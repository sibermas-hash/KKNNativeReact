...
export default function RekapitulasiList({ kelompokList }: Props) {
  ...
  const filteredList = kelompokList.filter(
    (k) =>
            ...
            <div className="px-4 py-2 bg-white border border-emerald-50 rounded-lg shadow-sm flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm font-semibold text-emerald-950">
                  {kelompokList.length} Unit Tersedia
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredList.length === 0 ? (
            ...
            filteredList.map((k) => (
              <div
                key={k.id}
                className="bg-white border border-emerald-50 rounded-xl shadow-sm hover:shadow-md transition-shadow flex flex-col h-full overflow-hidden relative"
              >
                ...
                <div className="p-5 flex-grow z-10 space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex flex-col">
                      <span className="text-xs font-medium text-[#0d9488] mb-1">
                        {k.periode || '-'}
                      </span>
                      <h3 className="text-lg font-bold text-emerald-950 leading-tight">
                        {k.nama_kelompok}
                      </h3>
                    </div>
                  </div>
