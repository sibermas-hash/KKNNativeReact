...
export function TahunAkademikClient({
  ...
  const isReadOnly = Boolean(loadError);
      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="lg:col-span-2">
        <div className="rounded-3xl border border-slate-100 bg-white shadow-sm overflow-hidden">
                        ...
                        {expandedId === ay.id && (
                          <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                            <td colSpan={3} className="p-0 border-none">
                              <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} transition={{ duration: 0.3, ease: 'easeInOut' }} className="overflow-hidden bg-[#F8FAF9]">
                                <div className="p-8 border-y border-cyan-100/50 grid grid-cols-1 md:grid-cols-2 gap-8">
                                  ...
                                  <div className="flex items-start gap-4">
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <p className="text-xs font-bold text-cyan-950 uppercase">Checksum: Verified (System ID {ay.id})</p>
                                    </div>
                                  </div>
                                </div>
                              </motion.div>
          ...
          <div className="px-6 py-4 border-t border-slate-50">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] tabular-nums">{rows.length} RECORDS FOUND</span>
          </div>
        </div>
      </motion.div>
    </div>
