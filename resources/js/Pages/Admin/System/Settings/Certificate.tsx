            ...
            >
                ...
                <motion.div variants={itemVariants} className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
                    <div className="space-y-6">
                        ...
                        <p className="text-lg font-bold text-slate-400 tracking-tight leading-relaxed max-w-2xl uppercase italic opacity-80">
                            ...
                            <span className="text-slate-900 not-italic">Otorisasi aset visual, narasi resmi, dan pemetaan metadata untuk penerbitan sertifikat digital KKN secara masif.</span>
                        </p>
                    </div>
                    ...
                    <div className="hidden xl:flex items-center gap-6">
                         <div className="h-24 px-10 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex items-center gap-8 shadow-sm relative overflow-hidden group">
                              ...
                              <div className="flex flex-col">
                                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Asset Status</span>
                                   <span className="text-lg font-black text-slate-900 uppercase tracking-widest italic">VERIFIED_SRC</span>
                              </div>
                         </div>
                    </div>
                </motion.div>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-12 gap-10 items-stretch">
                            ...
                            <div className="p-12 space-y-12">
                                ...
                                <div className="bg-slate-50 border border-slate-100 rounded-[2.5rem] p-10 space-y-8">
                                    <div className="flex items-center gap-6">
                                        <div className="h-10 w-10 bg-slate-900 text-emerald-500 rounded-xl flex items-center justify-center shadow-xl">
                                             <Binary size={18} />
                                        </div>
                                        <div className="space-y-1">
                                             <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Runtime Metadata Tags</h3>
                                             <p className="text-sm font-black text-slate-900 uppercase tracking-widest italic">Dynamic Injectors</p>
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap gap-3">
                                        {['[StudentName]', '[NIM]', '[LOKASI]', '[PERIODE]'].map((tag) => (
                                            <span key={tag} className="px-5 py-2.5 bg-white rounded-2xl text-[10px] font-black text-slate-600 border border-slate-100 shadow-sm uppercase tracking-widest italic">{tag}</span>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    {configs.filter((c) => c.type !== 'gambar').map((config) => (
                                        <div key={config.id} className="space-y-4">
                                             <div className="flex items-center justify-between px-2">
                                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic mb-2 block">{config.label}</label>
                                            ...
                                            {config.type === 'longtext' ? (
                                                <div className="relative group/input">
                                                     <textarea 
                                                        ...
                                                        className="w-full bg-slate-50 border-none rounded-[2rem] px-10 py-10 text-[13px] font-black text-slate-900 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all leading-relaxed italic placeholder:text-slate-200" 
                                                ...
                                                <input 
                                                    ...
                                                    value={getValue(config.id)} 
                                                    onChange={(e) => updateValue(config.id, e.target.value)} 
                                                    className="w-full h-18 bg-slate-50 border-none rounded-[1.5rem] px-10 text-[13px] font-black text-slate-900 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all italic" 
                    ...
                    <motion.div variants={itemVariants} className="xl:col-span-4 space-y-10 flex flex-col h-full">
                        <div className="bg-white border border-slate-100 rounded-[3.5rem] overflow-hidden shadow-2xl shadow-slate-200/50 group/section flex-1">
                            <div className="px-12 py-12 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                                 <div className="flex items-center gap-8">
                                      ...
                                      <div className="space-y-1">
                                           <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Visual Identity</h3>
                                           <p className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic leading-none">Graphic Vault</p>
                                      </div>
                                 </div>
                            </div>
                            ...
                            <div className="p-12 space-y-10">
                                {configs.filter((c) => c.type === 'gambar').map((config) => (
                                    <div key={config.id} className="space-y-4">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic block ml-2">{config.label}</label>
                                        <div className="relative group/input">
                                            ...
                                            <input 
                                                ...
                                                onChange={(e) => updateValue(config.id, e.target.value)} 
                                                placeholder="https://..." 
                                                className="w-full h-18 pl-18 pr-8 bg-slate-50 border-none rounded-[1.5rem] text-[11px] font-black text-slate-900 focus:bg-white focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all italic font-mono tracking-widest placeholder:text-slate-200" 
                                            />
                                        </div>
                                    </div>
                                ))}
                                
                                <div className="p-10 bg-slate-950 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-6 shadow-2xl relative overflow-hidden group/iv">
                                    ...
                                    <div className="space-y-2">
                                         ...
                                         <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic leading-relaxed max-w-[180px]">EXTERNAL_CDN LINKED. <br /> ASSETS LOADED ON RUNTIME ONLY.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Control Bar */}
                        <div className="bg-slate-900 rounded-[3.5rem] p-10 flex flex-col gap-10 shadow-2xl relative overflow-hidden group/save">
                ...
                {/* --- FOOTER GOVERNANCE --- */}
                <motion.div variants={itemVariants} className="bg-white border border-slate-100 rounded-[3.5rem] p-16 text-slate-900 relative overflow-hidden group/f shadow-2xl shadow-slate-200/50">
                    ...
                    <div className="flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10">
                        <div className="space-y-6 flex-1">
                             <div className="flex items-center gap-5">
                                  ...
                                  <div className="space-y-1">
                                       ...
                                       <h3 className="text-3xl font-black tracking-tighter uppercase italic leading-none text-slate-900">Institutional Credentialing Standard</h3>
                                  </div>
                             </div>
                        ...
                        <div className="px-12 py-6 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center gap-2 shadow-sm">
                             ...
                             <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Validation Stream Nominal</span>
                        </div>
                    </div>
                </motion.div>
            </motion.div>
