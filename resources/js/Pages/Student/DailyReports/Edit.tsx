                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-100 pb-10 gap-8">
                    <div className="space-y-5">
                        ...
                        <h1 className="text-4xl font-extrabold text-slate-900  uppercase italic leading-none">
                            Kalibrasi <span className="text-primary italic">Laporan</span>
                        </h1>
                    ...
                    <div className="flex items-center gap-5 bg-white border border-slate-100 p-6rounded-lg min-w-[220px]">
                        <div className="h-14 w-14 rounded-lg bg-slate-900 text-primary flex items-center justify-center
                            <History className="h-7 w-7" />
                        </div>
                        <div>
                            <span className="text-[9px] font-bold text-slate-400 uppercase  block mb-1.5 italic">ID Laporan</span>
                            <span className="text-sm font-black text-slate-900 uppercase italic  leading-none">Entry #{report.id.toString().padStart(4, '0')}</span>
                        </div>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-slate-100 p-12 space-y-12 relative overflow-hidden group">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="relative group/input">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-slate-50 border-slate-100 rounded-lg pl-16 py-5 text-sm font-black text-slate-900 focus:ring-4 focus:ring-primary/5 focus:border-primary/20 transition-all uppercase italic outline-none"
                                            required
                                        />
                                    </div>
                            ...
                            <div className="space-y-4 relative z-10">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-slate-50 border-slate-100 rounded-lg px-8 py-5 text-sm font-black text-slate-900 focus:ring-4 focus:ring-primary/5 focus:border-primary/20 transition-all uppercase italic outline-none"
                                />
                                {form.errors.output && <p className="text-[10px] font-black text-rose-500 uppercase  ml-2 italic">{form.errors.output}</p>}
                            </div>

                            <div className="flex pt-8 relative z-10 border-t border-slate-50 justify-between items-center">
                                <button 
                                    type="button"
                                    onClick={() => window.history.back()}
                                    className="text-[10px] font-black text-slate-400 uppercase  hover:text-slate-950 transition-colors italic"
                                >
                                    Batalkan Perubahan
                                </button>
                                <button 
                                ...
                                >
                                    <RotateCcw className={clsx("w-6 h-6 text-primary transition-all", form.processing ? "animate-spin" : "group-hover/submit:rotate-180")} />
                    ...
                    <aside className="space-y-12">
                        <section className="bg-slate-900 rounded-lg p-12 border border-slate-900 text-white relative overflow-hidden group">
                             ...
                             <h4 className="text-[11px] font-black uppercase  mb-8 flex items-center gap-3 italic">
                                 <Info className="h-4 w-4 text-primary" />
                                 Prosedur Revisi
                             </h4>
                             <div className="space-y-8 relative z-10 italic">
                                <div className="space-y-2">
                                    <p className="text-[10px] font-black text-primary uppercase  Data</p>
                                    <p className="text-[11px] font-bold text-slate-400 leading-relaxed uppercase opacity-80">Pastikan seluruh data yang diperbaiki sudah sesuai dengan draf fisik atau instruksi dari supervisor lapangan.</p>
                                </div>
                                <div className="space-y-2 pt-6 border-t border-white/5">
                                    <p className="text-[10px] font-black text-slate-400 uppercase  italic leading-none">Verifikasi Ulang</p>
                                    <p className="text-[11px] font-bold text-slate-400 leading-relaxed uppercase italic opacity-70">Laporan yang telah diperbaiki akan masuk kembali ke antrian verifikasi DPL untuk divalidasi statusnya.</p>
                                </div>
                             </div>
                        </section>
                        ...
                        <div className="bg-white border border-slate-100 rounded-lg p-10 relative overflow-hidden italic group">
                            ...
                            <h4 className="text-[10px] font-black mb-10 flex items-center gap-3 uppercase  text-slate-400 italic">
                                <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                                Integritas Laporan
                            </h4>
                            <p className="text-[12px] font-bold leading-relaxed uppercase italic  relative z-10 text-slate-500 opacity-80 mb-6">
                            ...
                            <div className="flex -space-x-3 italic">
                                {[1,2,3].map(i => (
                                    <div key={i} className="h-8 w-8 rounded-full border-4 border-white bg-slate-100 transition-transform hover:translate-y-[-4px] hover:z-20 cursor-default" />
                                ))}
                            </div>
                        </div>
                    </aside>
