...
export default function ProfileShow() {
  ...
  const biodataComplete = isStudent
    ? (student?.biodata_complete ?? true)
    : (lecturer?.biodata_complete ?? true);
  const missingFields = isStudent
    ? (student?.missing_biodata_fields ?? [])
    : (lecturer?.missing_biodata_fields ?? []);
      ...
      <div className="max-w-5xl mx-auto space-y-6 sm:px-6 lg:px-8 font-sans pb-12">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-emerald-50 pt-6">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-bold text-emerald-950 tracking-tight leading-tight">
              Profil Saya
            </h1>
            <p className="text-sm text-emerald-700 max-w-xl">
              {isLecturer
                ? 'Pastikan data kepegawaian Anda valid untuk keperluan pembimbingan KKN.'
                : 'Lengkapi data pribadi Anda untuk kelancaran proses pendaftaran dan penempatan KKN.'}
            </p>
          </div>
        </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-amber-700">
                Informasi berikut wajib diisi:{' '}
                <strong>{missingFields.map((f) => FIELD_LABELS[f] ?? f).join(', ')}</strong>.
              </p>
            </div>
          </div>
        )}
              ...
              >
                <div className="h-24 w-24 rounded-full border-2 border-emerald-50 overflow-hidden bg-gray-100 relative">
                  {avatarUrl ? (
                    ...
                    <div className="h-full w-full flex items-center justify-center text-3xl font-bold text-emerald-600">
                      {user.name.charAt(0)}
                    </div>
                  )}
              ...
              {isLecturer && (
                <div className="mt-2 p-3 bg-emerald-50 rounded-lg space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider text-emerald-950 uppercase">
                      Kualifikasi DPL
                    </span>
                  </div>
                  ...
                  {lecturer.is_tugas_belajar && (
                    <p className="text-[9px] text-rose-600 italic leading-tight">
                      * Dosen tugas belajar tidak diperbolehkan mendaftar sebagai DPL.
                    </p>
                  )}
                </div>
              )}
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      {lecturer.golongan && (
                        <div>
                          ...
                          <span className="font-medium text-gray-900">{lecturer.golongan}</span>
                        </div>
                      )}
                      {lecturer.no_rekening && (
                        <div>
                          <span className="text-gray-500">No. Rekening:</span>{' '}
                          <span className="font-medium text-gray-900">{lecturer.no_rekening}</span>
                        </div>
                      )}
                      {lecturer.nama_bank && (
                        <div>
                          <span className="text-gray-500">Nama Bank:</span>{' '}
                          <span className="font-medium text-gray-900">{lecturer.nama_bank}</span>
                        </div>
                      )}
                      {lecturer.npwp && (
                        <div>
                          <span className="text-gray-500">NPWP:</span>{' '}
                          <span className="font-medium text-gray-900">{lecturer.npwp}</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <form onSubmit={handleProfileSubmit} className="p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {isLecturer && (
                      <div className="space-y-1.5">
                        ...
                        <FormInput
                          value={profileForm.data.jabatan}
                          onChange={(e) => profileForm.setData('jabatan', e.target.value)}
                          error={profileForm.errors.jabatan}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          ...
                          <FormInput
                            ...
                            error={profileForm.errors.golongan}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <FormInput
                            value={profileForm.data.no_rekening}
                            onChange={(e) => profileForm.setData('no_rekening', e.target.value)}
                            error={profileForm.errors.no_rekening}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <FormInput
                            value={profileForm.data.nama_bank}
                            onChange={(e) => profileForm.setData('nama_bank', e.target.value)}
                            error={profileForm.errors.nama_bank}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        className="w-full h-10 pl-3 pr-8 rounded-md border-gray-300 bg-white text-sm text-emerald-800 focus:border-[#0d9488] focus:ring-emerald-500 shadow-sm transition-colors"
                      >
                      ...
                      {profileForm.errors.gender && (
                        <p className="text-xs text-rose-600">{profileForm.errors.gender}</p>
                      )}
                    </div>
                    ...
                    {isStudent && (
                      <div className="space-y-1.5">
                        <label className="text-sm font-medium text-emerald-800 flex items-center gap-1">
                          Ukuran Baju / Jaket <span className="text-rose-500">*</span>
                        </label>
                        <select
                    ...
                    <div className="space-y-4">
                      ...
                      <label
                        className={clsx(
                          ...
                          profileForm.data.address_verified
                            ? 'bg-emerald-50 border-emerald-200'
                            : 'bg-gray-50 border-emerald-50 hover:border-emerald-200',
                        )}
                      >
                        <input
                          type="checkbox"
                        ...
                        <div className="space-y-1">
                          <p className="text-sm font-medium text-emerald-950">
                            Saya menyatakan alamat domisili di atas benar adanya
                          </p>
                          <p className="text-xs text-emerald-700">
                            Alamat ini digunakan sebagai acuan penempatan otomatis KKN. Sistem tidak
                            akan menempatkan Anda di kabupaten/kota yang sama.
                          </p>
                          {student &&
                          ...
                          {student?.domicile_verified_at && (
                            <div className="flex items-center gap-1.5 pt-1 text-xs text-emerald-600">
                              <UserCheck size={12} /> Dikonfirmasi{' '}
                              {new Date(student.domicile_verified_at).toLocaleDateString('id-ID')}
                            </div>
                          )}
                        </div>
                      </label>
                ...
                {isLecturer && (
                  <div className="p-4 bg-emerald-50/50 border border-emerald-100 rounded-xl flex items-start gap-3">
                    ...
                    <p className="text-xs text-emerald-800 leading-relaxed">
                      Beberapa data kepegawaian (seperti Fakultas) ditarik otomatis dari integrasi
                      data master universitas. Hubungi Admin Kepegawaian jika terdapat
                ...
                <div className="flex justify-end pt-2 border-t border-gray-100">
                  <button
                    ...
                    className="h-10 px-6 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 transition-colors shadow-sm disabled:opacity-50 flex items-center gap-2"
                  >
    ...
    <div className="flex items-center justify-between gap-2">
      ...
      <span
        className={clsx(
          ...
          complete ? 'bg-emerald-100 text-emerald-950' : 'bg-amber-100 text-amber-800',
        )}
      >
