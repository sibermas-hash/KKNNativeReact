...
interface PaginatedDispensasi {
  data: Dispensasi[];
  meta: PaginationMeta;
}
interface PaginatedIzin {
  data: Izin[];
  meta: PaginationMeta;
}
interface Props {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pt-10">
          ...
          <div className="bg-white border border-emerald-100 rounded-2xl p-5 flex items-center gap-5 shadow-sm">
            ...
            <div>
              ...
              <h4 className="text-2xl font-black text-emerald-950 tabular-nums leading-none">
                {dispensasi?.meta?.total} Kasus
              </h4>
            </div>
          </div>
        </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-emerald-950 uppercase tracking-widest pl-1">
                    Mahasiswa [NIM] <span className="text-rose-500">*</span>
                  </label>
                  <input
                    value={form.data.nim}
                    onChange={(e) => form.setData('nim', e.target.value)}
                    className="w-full h-12 px-5 bg-gray-50 border border-emerald-50 rounded-xl text-sm font-bold text-emerald-950 focus:bg-white focus:border-[#f3f4f6]0 outline-none transition-all placeholder:text-black"
                    placeholder="Masukkan NIM..."
                    required
                  />
                  {form.errors.nim && (
                    <p className="text-xs font-bold text-rose-500">{form.errors.nim}</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full h-12 px-5 bg-gray-50 border border-emerald-50 rounded-xl text-sm font-bold text-emerald-950 focus:bg-white focus:border-[#f3f4f6]0 outline-none transition-all"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-emerald-950 uppercase tracking-widest pl-1">
                    Justifikasi <span className="text-rose-500">*</span>
                  </label>
                  <textarea
                    value={form.data.alasan}
                    onChange={(e) => form.setData('alasan', e.target.value)}
                    className="w-full h-24 px-5 py-4 bg-gray-50 border border-emerald-50 rounded-xl text-sm font-bold text-emerald-950 focus:bg-white focus:border-[#f3f4f6]0 outline-none transition-all resize-none"
                        ...
                        <button
                          ...
                          className={clsx(
                            ...
                            isChecked
                              ...
                              : 'bg-white border-emerald-50 text-emerald-950 hover:bg-gray-50',
                          )}
                        >
                          <div
                            className={clsx(
                              ...
                              isChecked
                                ? 'bg-white border-white text-[#0d9488]'
            ...
            <div className="bg-white border border-emerald-50 rounded-xl overflow-hidden shadow-sm flex flex-col min-h-[800px]">
              <div className="p-6 border-b border-[#f3f4f6] bg-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <form onSubmit={handleSearch} className="relative w-full md:w-64">
                  ...
                  <input
                    ...
                    className="w-full h-10 pl-11 pr-4 bg-white border border-emerald-50 rounded-xl text-xs font-bold text-emerald-950 focus:border-[#f3f4f6]0 outline-none transition-all placeholder:text-black"
              ...
              <div className="flex-1 overflow-x-auto">
                <AnimatePresence mode="wait">
                  {activeTab === 'dispensasi' ? (
                    ...
                    >
                      <thead className="bg-gray-50/50 text-emerald-950 border-b border-[#f3f4f6]">
                        <tr>
                          <th className="px-8 py-5 text-xs font-bold uppercase tracking-widest">
                            Identitas NIM
                          </th>
                          ...
                          <th className="px-8 py-5 text-right text-xs font-bold uppercase tracking-widest">
                            Tindakan
                          </th>
                        </tr>
                      </thead>
                            ...
                            <tr key={item.id} className="group hover:bg-gray-50/20 transition-all">
                              <td className="px-8 py-6">
                                <div className="flex flex-col gap-1">
                                  ...
                                  <span
                                    className="text-xs font-bold text-[#0d9488] truncate max-w-[200px]"
                                    title={item.alasan}
                                  >
                                    {item.alasan}
                                  </span>
                              ...
                              <td className="px-6 py-6 text-xs font-bold text-emerald-800">
                                {item.periode?.name || 'GLOBAL'}
                              </td>
                              <td className="px-6 py-6">
                                <div className="flex flex-wrap gap-1">
                                  {item.bypassed_requirements?.map((r) => (
                                    <span
                                      key={r}
                                      className="px-1.5 py-0.5 bg-white border border-emerald-50 text-[#0d9488] rounded text-[9px] font-bold"
                                    >
                                      {r.toUpperCase()}
                                    </span>
                                  )) || (
                    ...
                    >
                      ...
                      <tbody className="divide-y divide-[#f3f4f6]">
                        {izins?.data?.length === 0 ? (
                          <EmptyState label="Data Izin Nihil" />
                        ) : (
                          izins?.data?.map((item) => (
                            <tr key={item.id} className="group hover:bg-gray-50/20 transition-all">
                              <td className="px-8 py-6">
                                <div className="flex flex-col gap-0.5">
                                  ...
                                  <span className="text-xs font-bold text-[#0d9488] font-mono">
                                    {item.mahasiswa?.nim}
                                  </span>
                                </div>
                              </td>
                              <td className="px-6 py-6">
                                <div className="flex flex-col gap-1">
                                  <span className="text-xs font-bold text-emerald-800">
                                    {item.tanggal_mulai} — {item.tanggal_kembali}
                                  </span>
                                  <span
                                    className="text-[9px] font-bold text-[#0d9488] italic truncate max-w-[200px]"
                                    title={item.alasan}
                                  >
                                    "{item.alasan}"
                                  </span>
                                ...
                                <span
                                  className={clsx(
                                    ...
                                    item.status === 'disetujui'
                                      ? 'bg-[#f0fdfa] text-[#0d9488] border border-emerald-200'
                                      : item.status === 'ditolak'
              ...
              <div className="p-6 border-t border-[#f3f4f6] bg-gray-50/50 flex items-center justify-between">
                <p className="text-xs font-bold text-emerald-950/60 uppercase tracking-widest">
                  TOTAL {activeTab === 'dispensasi' ? dispensasi?.meta?.total : izins?.meta?.total}{' '}
                  BARIS DATA
                </p>
                <Pagination
                  meta={(activeTab === 'dispensasi' ? dispensasi?.meta : izins?.meta) as any}
                />
              </div>
            </div>
          </motion.div>
