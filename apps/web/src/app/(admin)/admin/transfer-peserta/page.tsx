...
type PeriodeOption = { id: number; name: string; periode: number; is_active?: boolean; jenis_kkn?: { id: number; name: string; requires_interview?: boolean } };
...
export default function TransferPesertaPage(): React.JSX.Element {
  ...
  const { data: allPeriodes = [] } = useQuery<{ id: number; periode: number; is_active?: boolean }[]>({
    queryKey: ['admin', 'all-periodes', 'active-only'],
    queryFn: async () => {
      ...
      return ((Array.isArray(payload) ? payload : (payload as { data?: unknown[] }).data ?? []) as { id: number; periode: number; is_active?: boolean }[]).filter((p) => p.is_active === true);
    },
  });

  // Extract unique angkatans
  const uniqueAngkatans = Array.from(
    new Set((allPeriodes ?? []).map(p => p.periode).filter((v): v is number => v !== null && v !== undefined).map(v => String(v)))
  ).sort((a, b) => b.localeCompare(a));

  useEffect(() => {
    if (uniqueAngkatans.length > 0 && (!angkatan || !uniqueAngkatans.includes(angkatan))) setAngkatan(uniqueAngkatans[0]);
  }, [angkatan, uniqueAngkatans]);

  // Fetch peserta yang bisa di-transfer (interview_failed + approved yang mau dipindah)
  const { data, isLoading, isError, refetch } = useQuery<{ data: Peserta[]; meta: { current_page: number; last_page: number; total: number } }>({
  ...
  const { data: periodesData } = useQuery<PeriodeOption[]>({
    ...
    queryFn: async () => {
      ...
      return ((Array.isArray(payload) ? payload : []) as PeriodeOption[]).filter((p) => p.is_active !== false);
    },
  });
    ...
    <div className="space-y-6">
      ...
      {isError ? (
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-rose-100">
          <p className="text-sm font-bold text-rose-600">Gagal memuat data transfer peserta.</p>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="mt-4">
              ...
              <select value={targetPeriodeId} onChange={e => setTargetPeriodeId(e.target.value)} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-600">
                ...
                {(Array.isArray(periodes) ? periodes : []).filter(p => p.id !== transferTarget.periode?.id).map(p => (
