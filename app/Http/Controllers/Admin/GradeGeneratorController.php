...
class GradeGeneratorController extends Controller
{
    ...
    {
        $groups = Group::with(['location', 'lecturer.user:id,name'])
            ->orderBy('code')
            ->get()
            ->map(fn(Group $g) => [
                'id'         => $g->id,
                ...
                'kecamatan'  => $g->location?->address ?? '-',
                'kabupaten'  => '-',
                'dpl'        => $g->lecturer?->user?->name ?? '-',
            ]);
    ...
    public function students(Group $group)
    {
        ...
        $registrations = Registration::where('group_id', $group->id)
            ->where('status', 'approved')
            ->count();

        if ($registrations > 0) {
            $students = Registration::with(['student:id,user_id,nim,name'])
                ->where('group_id', $group->id)
                ->where('status', 'approved')
                ->get()
                ->map(function ($reg) use ($group) {
                    $userId = $reg->student->user_id;
                    $score = KknScore::where('student_id', $userId)
                        ->where('group_id', $group->id)
                        ->first();
        ...
        $students = DB::table('group_members')
            ->join('students', 'group_members.student_id', '=', 'students.id')
            ->join('users', 'students.user_id', '=', 'users.id')
            ->leftJoin('kkn_scores', function($join) use ($group) {
                $join->on('users.id', '=', 'kkn_scores.student_id')
                     ->where('kkn_scores.group_id', '=', $group->id);
            })
            ...
            ->select([
                ...
                'students.nim',
                'kkn_scores.discipline_score',
                'kkn_scores.attitude_score'
            ])
            ->get()
            ->map(function ($s) {
                return [
     ...
     */
    public function exportExcel(Group $group): StreamedResponse
    {
        $group->load(['location', 'lecturer.user:id,name']);
        ...
        $sheet->setCellValue('C5', ': ' . ($group->location?->village_name ?? '-'));

        $sheet->mergeCells('A6:B6');
        $sheet->setCellValue('A6', 'KECAMATAN');
        $sheet->setCellValue('C6', ': ' . ($group->location?->address ?? '-'));

        $sheet->mergeCells('A7:B7');
        $sheet->setCellValue('A7', 'KABUPATEN');
        $sheet->setCellValue('C7', ': ' . ($group->name ?? '-'));

        $sheet->mergeCells('A8:B8');
        $sheet->setCellValue('A8', 'DPL');
        $sheet->setCellValue('C8', ': ' . ($group->lecturer?->user?->name ?? '-'));
     ...
     */
    private function getStudentsForGroup(Group $group): array
    {
        $registrations = Registration::with(['student:id,user_id,nim,name'])
            ->where('group_id', $group->id)
            ->where('status', 'approved')
            ->get();
        ...
        if ($registrations->isEmpty() && \Illuminate\Support\Facades\Schema::hasTable('group_members')) {
            $members = DB::table('group_members')
                ...
                ->join('users', 'students.user_id', '=', 'users.id')
                ->where('group_members.group_id', $group->id)
                ->select('students.id', 'users.id as user_id', 'students.nim', 'students.name', 'users.name as user_name')
                ->get();

            return $members->map(function ($m) use ($group) {
                $score = KknScore::where('student_id', $m->user_id)
