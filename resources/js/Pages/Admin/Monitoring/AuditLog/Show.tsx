          ...
          <div className="xl:col-span-8 space-y-8">
            ...
            >
              <div className="space-y-6 py-4">
                ...
                <div className="flex items-center gap-3 text-emerald-600/50">
                  ...
                  <span className="text-[12px] font-black tabular-nums uppercase tracking-widest">
                    {log.created_at}
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border-2 border-slate-50 rounded-[2rem] p-8 shadow-sm group hover:border-emerald-600 transition-all">
                ...
                <p className="text-xl font-black text-emerald-950 uppercase tracking-tight leading-none mb-2">
                  {log.causer?.name || 'SYSTEM_DAEMON'}
                </p>
          ...
          <div className="xl:col-span-4 space-y-8">
            ...
            >
              <div className="space-y-4 py-2">
                <MetaRow
                  label="SUBJECT_ID"
                  value={log.subject_id ? `#${log.subject_id.toString().padStart(4, '0')}` : 'NULL'}
                />
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between py-3 border-b-2 border-slate-50 last:border-0 group">
      <span className="text-[10px] font-black text-emerald-950/30 uppercase tracking-widest group-hover:text-emerald-950 transition-colors">
        {label}
      </span>
      <span className="text-[10px] font-black text-emerald-900 bg-emerald-50 px-3 py-1 rounded-lg border border-emerald-100 uppercase">
        {value}
      </span>
    </div>
  );
