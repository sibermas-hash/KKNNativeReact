  ...
  return (
    <AppLayout title="Pusat Komando Taktis">
      ...
      <div className="space-y-12 pb-24">
        ...
        <section className="relative overflow-hidden rounded-lg bg-white from-[#10a853] via-[#0d8c45] to-[#043d23] p-12 md:p-20 border border-white/20 group">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-16">
            <div className="space-y-10 max-w-4xl text-center lg:text-left">
              <div className="inline-flex items-center gap-4 px-6 py-2.5 bg-white/10 rounded-lg border border-white/20 backdrop-blur-xl">
              ...
              <p className="text-emerald-50/70 text-xl font-bold italic leading-relaxed max-w-2xl">
              ...
              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-8 pt-4">
                <button className="flex items-center gap-4 px-12 py-5 bg-white text-emerald-900rounded-lg font-black text-xs uppercase  transition-all hover:scale-105 active:scale-95 group/btn">
                  Kalibrasi Data
            ...
            <div className="relative z-10 p-1.5 bg-white/10 rounded-lg border border-white/20 hidden xl:block">
               <div className="bg-slate-900/40 p-12 rounded-[3.3rem] border border-white/10 flex flex-col items-center gap-8 text-center min-w-[320px]">
                  ...
                  <div className="space-y-3">
                     <span className="block text-[11px] font-black text-emerald-400 uppercase  italic">System Status</span>
                     <span className="text-4xl font-black text-white italic">OPERATIONAL_OK</span>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-lg border border-slate-100 p-12">
            <div className="flex items-center justify-between mb-12">
               <div className="flex items-center gap-6">
                  ...
                  <h3 className="text-2xl font-black text-slate-900  uppercase italic leading-none">Telemetri Aktivitas</h3>
               </div>
               <div className="px-6 py-2.5 rounded-full bg-emerald-50 text-emerald-600 text-[11px] font-black uppercase  italic border border-emerald-100 italic">Live_Feed</div>
            </div>
            
            <div className="h-64 flex items-end gap-4 px-2">
              {[45, 60, 45, 80, 55, 90, 40, 75, 65, 85, 95].map((h, i) => (
                <div key={i} className="flex-1 bg-slate-50 rounded-t-[2rem] relative group/bar cursor-default h-full">
                  <div 
                    className="absolute bottom-0 left-0 w-full bg-primary group-hover/bar:brightness-110 transition-all rounded-t-[2rem]
                    style={{ height: `${h}%` }}
                  />
                </div>
              ))}
        ...
        <div className="p-16 bg-[#043d23] rounded-[4rem] border border-white/10 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-12">
               <div className="space-y-6">
                  <div className="flex items-center justify-center md:justify-start gap-5">
                      <Globe className="h-8 w-8 text-emerald-300" />
                      <h4 className="text-[12px] font-black text-white uppercase  italic">GLOBAL_INTEGRITY_PROTOCOL</h4>
                  </div>
                  <p className="text-lg text-emerald-50/60 font-medium max-w-3xl italic">
  ...
  return (
    <div className="bg-white rounded-lg p-10 border border-slate-100 hover:shadow-3xl hover:-translate-y-2 transition-all group cursor-default">
      <div className="flex items-center gap-5 mb-8">
        ...
        <div className="text-[10px] font-black text-slate-400 italic uppercase  leading-none">{label}</div>
      </div>

      <div className="flex items-baseline gap-3 mb-2">
         <h4 className="text-5xl font-black text-slate-900  italic leading-none">
           {value}
         </h4>
         {unit && <span className="text-lg font-black text-slate-300 italic">{unit}</span>}
      </div>
      
      <span className="text-[10px] font-black text-emerald-500 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100 italic">{trend}</span>
    </div>
  );
}
...
function FeedItem({ name, action, status }: any) {
    return (
        <div className="flex items-center gap-6 group cursor-pointer p-2rounded-lg hover:bg-slate-50/50 transition-all">
            <div className="h-14 w-14 bg-white border border-slate-100 rounded-lg flex items-center justify-center text-primary font-black italic">
               {name.charAt(0)}
            </div>
            <div className="flex-1 space-y-1">
                <h5 className="font-black italic uppercase  text-xs text-slate-900 leading-none">{name}</h5>
                <p className="text-[10px] font-bold text-slate-400 italic leading-none">{action}</p>
            </div>
            <div className={clsx(
                "px-3 py-1 rounded-lg text-[9px] font-black italic uppercase 
