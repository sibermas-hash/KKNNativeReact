...
import {
 Download, MapPin, Pencil, Plus, Search, Trash2, Users, UserCheck, Filter, CheckCircle2, Activity, Layers, ArrowRight, Save, X, RefreshCw
} from 'lucide-react';
...
export default function GroupsIndex({
 ...
 <span className="text-sm font-medium text-gray-700">Kelompok & Penugasan</span>
 </div>
 <h1 className="text-2xl font-bold text-gray-900 leading-tight">Manajemen Kelompok</h1>
 ...
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-1.5">Periode Program</label>
 <select
 value={form.data.period_id}
 onChange={(e) => form.setData('period_id', e.target.value)}
 className="w-full px-3.5 py-2.5 rounded-lg border border-gray-300 bg-white text-sm text-gray-900 focus:border-[#1a7a4a] focus:ring-1 focus:ring-[#1a7a4a] outline-none transition-all"
 required
 >
 ...
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-1.5">Wilayah Penugasan</label>
 <select
 ...
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-1.5">Nama Kelompok</label>
 <input
 type="text"
 ...
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-1.5">Daya Tampung</label>
 <input
 type="number"
 min="1"
 ...
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-1.5">DPL Utama (Ketua)</label>
 <select
 ...
 <div>
 <p className="text-xl font-bold text-gray-900 tabular-nums">{summary?.groups_without_main_lecturer ?? 0}</p>
 <p className="text-xs font-medium text-gray-700">Tanpa DPL</p>
 </div>
 </div>
 <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3">
 ...
 <div>
 <p className="text-xl font-bold text-gray-900 tabular-nums">{summary?.total_available_slots ?? 0}</p>
 <p className="text-xs font-medium text-gray-700">Sisa Kapasitas</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div>
 <h3 className="text-sm font-bold text-gray-900">Daftar Kelompok</h3>
 <p className="text-xs text-gray-700">Total: {groups.meta.total} entri data</p>
 </div>
 ...
 {/* Filter row */}
 <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex flex-wrap items-center gap-3">
 <select
 value={periodId}
 onChange={(e) => setPeriodId(e.target.value)}
 className="h-9 px-3 rounded-lg border border-gray-300 bg-white text-xs font-medium text-gray-700 focus:border-[#1a7a4a] outline-none transition-all min-w-[150px]"
 >
 ...
 className="h-9 px-3 rounded-lg border border-gray-300 bg-white text-xs font-medium text-gray-700 focus:border-[#1a7a4a] outline-none transition-all"
 >
 ...
 <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">ID & Nama</th>
 <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Lokasi</th>
 <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Kapasitas</th>
 ...
 <p className="text-sm font-medium text-gray-700">Data Tidak Ditemukan</p>
 <p className="text-xs text-gray-700 mt-1">Silakan periksa filter atau buat entri baru</p>
 </td>
 </tr>
 ) : (
 ...
 <span className="text-xs font-mono bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded">{group.code}</span>
 <span className={clsx(
 'text-xs font-medium px-2 py-0.5 rounded-full',
 group.status === 'active' ? 'bg-[#dcfce7] text-[#15803d]' : 
 group.status === 'closed' ? 'bg-gray-100 text-gray-700' : 'bg-amber-50 text-amber-700'
 )}>
 ...
 <div className="flex flex-col">
 <span className="text-sm font-medium text-gray-900 line-clamp-1">{group.location?.full_name || '-'}</span>
 <span className="text-xs text-gray-700 mt-1">{group.period?.name || '-'}</span>
 </div>
 </td>
 <td className="px-6 py-4 align-top">
 ...
 className="inline-flex items-center gap-1.5 h-8 px-3.5 rounded-md text-sm font-medium border border-gray-300 text-gray-700 bg-white hover:bg-gray-50 transition-colors"
 >
 Detail
 </Link>
 {canManage && (
 <>
 <button onClick={() => openEditForm(group)} className="h-8 w-8 flex items-center justify-center text-gray-700 hover:text-gray-900 hover:bg-gray-50 rounded-md transition-colors" title="Edit">
 <Pencil size={15} />
 </button>
 ...
 <span className="text-xs text-gray-700">Menampilkan {groups.data.length} dari {groups.meta.total} baris</span>
