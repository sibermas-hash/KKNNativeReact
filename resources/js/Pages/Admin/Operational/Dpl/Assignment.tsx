        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pt-10">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-medium text-gray-700 max-w-2xl">
            ...
            <button
              ...
              className={clsx(
                ...
                activeFormMode === 'import' 
                  ...
                  : "bg-white border border-gray-200 text-[#1a7a4a] hover:bg-gray-50"
              )}
            >
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <div className="p-6 border-b border-[#f3f4f6] bg-gray-50 flex items-center justify-between">
                ...
                {activeFormMode === 'import' && (
                  <button onClick={() => setActiveFormMode('default')} className="h-8 w-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-600 hover:text-rose-500 hover:bg-rose-50 transition-colors">
              ...
              {activeFormMode === 'import' && (
                <form onSubmit={submitImport} className="p-8 space-y-6">
                   <div className="border-4 border-dashed border-[#f3f4f6] rounded-xl p-8 flex flex-col items-center justify-center gap-6 hover:bg-gray-50 transition-all relative group/upload">
                    <div className="h-16 w-16 rounded-xl bg-white border border-gray-200 shadow-sm flex items-center justify-center text-[#1a7a4a] group-hover/upload:scale-110 group-hover/upload:rotate-12 transition-all">
                      <Upload size={24} strokeWidth={2.5} />
                    </div>
                    <div className="flex flex-col items-center text-center gap-2">
                      {importForm.data.file ? (
                        <>
                          <span className="text-sm font-bold text-gray-900">{importForm.data.file.name}</span>
                          <span className="text-xs font-bold text-[#1a7a4a] uppercase">File Siap Ditransmisikan</span>
                        </>
                      ) : (
                        <>
                          <p className="text-xs font-bold text-gray-900">Pilih Berkas Spreadsheet</p>
                  ...
                  <div className="bg-[#e8f5ee] rounded-xl p-6 flex flex-col gap-2">
                    <span className="text-xs font-bold text-gray-800 uppercase tracking-widest">Informasi Sinkronisasi</span>
              ...
              {activeFormMode === 'default' && activeTab === 'assignments' && (
                <form onSubmit={submitPeriodAssignment} className="p-8 space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-gray-900 uppercase tracking-widest pl-1 mb-2">Dosen Pengampu <span className="text-rose-500">*</span></label>
                    <select value={periodForm.data.dosen_id} onChange={(e) => periodForm.setData('dosen_id', e.target.value)} className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:border-[#f3f4f6]0 outline-none transition-all shadow-sm appearance-none" required>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-900 uppercase tracking-widest pl-1 mb-2">Target Periode <span className="text-rose-500">*</span></label>
                    <select value={periodForm.data.period_id} onChange={(e) => periodForm.setData('period_id', e.target.value)} className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:border-[#f3f4f6]0 outline-none transition-all appearance-none" required>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-900 uppercase tracking-widest pl-1 mb-2">Kuota Bimbingan <span className="text-rose-500">*</span></label>
                    <input type="number" min="1" value={periodForm.data.max_groups} onChange={(e) => periodForm.setData('max_groups', parseInt(e.target.value))} className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-900 text-center focus:border-[#f3f4f6]0 outline-none transition-all shadow-sm tabular-nums" required />
                  </div>
              ...
              {activeFormMode === 'default' && activeTab === 'groups' && (
                <form onSubmit={submitGroupAssignment} className="p-8 space-y-6">
                  {selectedGroup ? (
                    <div className="bg-[#e8f5ee] p-5 rounded-xl border border-gray-200 flex items-center justify-between shadow-sm">
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-[#1a7a4a] uppercase tracking-widest leading-none mb-2">UNIT TERPILIH</span>
                        <span className="text-sm font-bold text-gray-900">{selectedGroup.name}</span>
                      </div>
                      <span className="text-xs font-bold text-gray-800 bg-white px-2 py-1 rounded border border-gray-200 shadow-sm font-mono">{selectedGroup.code}</span>
                    </div>
                  ) : (
                    <div className="bg-gray-50 bg-dashed border-2 border-dashed border-gray-200 p-6 rounded-xl flex flex-col items-center justify-center gap-3 text-center">
                       <Target size={24} className="text-gray-600" />
                       <span className="text-xs font-bold text-gray-700 uppercase tracking-wider">Silakan Pilih Unit Anda<br/>Dari Tabel di Sebelah Kanan</span>
                    </div>
                  )}

                  <div>
                    <label className="block text-xs font-bold text-gray-900 uppercase tracking-widest pl-1 mb-2">Alokasi DPL <span className="text-rose-500">*</span></label>
                    <select value={groupForm.data.dpl_period_id} onChange={(e) => groupForm.setData('dpl_period_id', e.target.value)} disabled={!selectedGroup} className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:border-[#f3f4f6]0 outline-none transition-all shadow-sm appearance-none disabled:opacity-50" required>
                  ...
                  <div className="pt-2 border-t border-[#f3f4f6]">
                    ...
                    {selectedGroup && (
                      <button type="button" onClick={() => groupForm.reset()} className="mt-2 w-full h-10 text-xs font-bold text-gray-600 hover:text-rose-500 transition-colors uppercase tracking-widest">
                        Batalkan Pilihan
                      </button>
                    )}
                  </div>
              ...
              {activeFormMode === 'default' && activeTab === 'regions' && (
                <form onSubmit={submitDistrictCoordinator} className="p-8 space-y-6">
                  <div>
                    <label className="block text-xs font-bold text-gray-900 uppercase tracking-widest pl-1 mb-2">Wilayah Kecamatan <span className="text-rose-500">*</span></label>
                    <select value={coordForm.data.district_id} onChange={(e) => coordForm.setData('district_id', e.target.value)} className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:border-[#f3f4f6]0 outline-none transition-all shadow-sm appearance-none" required>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-900 uppercase tracking-widest pl-1 mb-2">Otoritas Korwil <span className="text-rose-500">*</span></label>
                    <select value={coordForm.data.dpl_period_id} onChange={(e) => coordForm.setData('dpl_period_id', e.target.value)} className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:border-[#f3f4f6]0 outline-none transition-all shadow-sm appearance-none" required>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5 flex items-center gap-4 shadow-sm">
                ...
                <div>
                  <p className="text-2xl font-black text-gray-900 leading-none tabular-nums mt-1">{summary?.active_assignments}</p>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5 flex items-center gap-4 shadow-sm">
                ...
                <div>
                  <p className="text-2xl font-black text-gray-900 leading-none tabular-nums mt-1">{summary?.groups_total}</p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-black text-gray-900 leading-none tabular-nums mt-1">{summary?.district_coordinators} Korwil Terdata</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 flex flex-col h-full">
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col h-full overflow-hidden">
              ...
              <div className="p-6 border-b border-[#f3f4f6] bg-gray-50 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                <div className="flex items-center p-1.5 bg-white border border-gray-200 rounded-xl overflow-x-auto shadow-sm shadow-emerald-900/5">
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleApplyFilters(); }} className="relative w-full xl:w-72 shrink-0">
                  ...
                  <input
                    ...
                    className="w-full h-10 pl-9 pr-3 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-900 placeholder:text-gray-400 focus:border-[#f3f4f6]0 outline-none transition-all shadow-sm"
              ...
              <div className="overflow-x-auto min-h-[500px]">
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-gray-200 bg-white">
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase">Identitas Dosen [NIP]</th>
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase text-center">Periode Akademik</th>
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase text-center">Beban Kerja Bimbingan</th>
                        <th className="px-6 py-4 text-right text-xs font-extrabold text-gray-900 tracking-widest uppercase">Otoritas</th>
                      </tr>
                    </thead>
                        ...
                        assignments.map((a) => (
                          <tr key={a.id} className="group hover:bg-gray-50 transition-all">
                            <td className="px-6 py-4 align-top">
                              <div className="flex flex-col">
                                <span className="text-sm font-bold text-gray-900 mb-1">{a.dosen?.nama || 'N/A'}</span>
                                <span className="text-xs font-bold text-gray-700 font-mono">NIP: {a.dosen?.nip || '-'}</span>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4 align-top">
                              <div className="flex flex-col items-center gap-1.5 w-24 mx-auto">
                                <div className="flex justify-between text-xs font-bold w-full uppercase">
                                  <span className="text-gray-900 tabular-nums">{a.current_groups || 0}</span>
                                  <span className="text-gray-600 tabular-nums">/ {a.max_groups || 0} MAX</span>
                                </div>
                ...
                {activeTab === 'groups' && (
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-gray-200 bg-white">
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase">Identitas Kelompok [KODE]</th>
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase">Titik Lokasi Wilayah</th>
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase">Otoritas DPL Pengampu</th>
                        <th className="px-6 py-4 text-right text-xs font-extrabold text-gray-900 tracking-widest uppercase">Aksi</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#f3f4f6]">
                      {(!groups || groups.length === 0) ? (
                        ...
                        groups.map((g) => (
                          <tr key={g.id} className={clsx("group transition-all", groupForm.data.group_id === String(g.id) ? "bg-emerald-50/70" : "hover:bg-gray-50")}>
                            <td className="px-6 py-4 align-top">
                              <div className="flex flex-col">
                                <span className="text-sm font-bold text-gray-900 mb-1">{g.name}</span>
                                <span className="text-xs font-bold text-gray-700 font-mono">CODE: {g.code} &middot; {g.status.toUpperCase()}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4 align-top">
                              <div className="flex flex-col">
                                <span className="text-xs font-bold text-gray-900 truncate max-w-xs mb-1">{g.location?.district_name || 'UMUM'}</span>
                                <span className="text-xs font-bold text-gray-600">{g.location?.village_name || 'BELUM TERPLOT'}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4 align-top">
                              {g.dpl ? (
                                <div className="flex flex-col">
                                  <span className="text-xs font-bold text-gray-900 leading-tight mb-1">{g.dpl.nama}</span>
                                  <span className="text-xs font-bold text-gray-700 font-mono text-ellipsis overflow-hidden">NIP: {g.dpl.nip}</span>
                                </div>
                              ) : (
                              ...
                              <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                  ...
                                  className={clsx(
                                    ...
                                    g.dpl ? "bg-[#e8f5ee] text-[#1a7a4a] border-gray-200 hover:bg-[#e8f5ee]" : "bg-white text-[#1a7a4a] border-gray-200 hover:bg-gray-50 shadow-sm"
                                  )}
                                >
                ...
                {activeTab === 'regions' && (
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-gray-200 bg-white">
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase">Kecamatan Administratif</th>
                        <th className="px-6 py-4 text-xs font-extrabold text-gray-900 tracking-widest uppercase">Otoritas Koordinator Wilayah</th>
                        <th className="px-6 py-4 text-right text-xs font-extrabold text-gray-900 tracking-widest uppercase">Aksi</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#f3f4f6]">
                      {(!districtCoordinators || districtCoordinators.length === 0) ? (
                        ...
                        districtCoordinators.map((c) => (
                          <tr key={c.id} className="group hover:bg-gray-50 transition-all">
                            <td className="px-6 py-4 align-top">
                              <div className="flex flex-col">
                                <span className="text-sm font-bold text-gray-900 mb-1">{c.district_name || 'N/A'}</span>
                                <span className="text-xs font-bold text-gray-600 uppercase tracking-widest">{c.regency_name || 'BANYUMAS'}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4 align-top">
                              <div className="flex flex-col">
                                <span className="text-xs font-bold text-gray-900 mb-1">{c.dosen?.nama || 'N/A'}</span>
                                <span className="text-xs font-bold text-gray-700 font-mono">NIP: {c.dosen?.nip || '-'}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4 text-right align-top">
                              <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        active ? 'bg-[#16a34a] text-white shadow-sm' : 'text-gray-700 hover:text-[#1a7a4a] hover:bg-gray-50'
      )}
    >
