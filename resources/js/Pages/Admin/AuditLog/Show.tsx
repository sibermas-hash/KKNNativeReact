        ...
        <AppLayout title="Detail Log Aktivitas">
            ...
            <div className="max-w-7xl mx-auto space-y-10 pb-24">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1.5 font-bold">
                             ...
                             <p className="text-[10px] text-slate-400 uppercase  leading-none">Riwayat Sistem</p>
                        </div>
                        <h1 className="text-3xl font-extrabold text-slate-900  leading-none">Detail <span className="text-primary italic">Aktivitas</span></h1>
                    </div>
                </div>
                ...
                <div className="relative bg-white rounded-lg border border-slate-100 overflow-hidden group">
                    ...
                    <div className="p-10 flex flex-col xl:flex-row xl:items-center justify-between gap-10 border-b border-slate-50">
                        <div className="flex items-center gap-8">
                            <div className="w-20 h-20 rounded-lg bg-slate-50 flex items-center justify-center text-4xl border border-slate-100">
                                {log.action === 'DELETE' ? '🗑️' : log.action === 'LOGIN' ? '🔐' : '📝'}
                            </div>
                            <div className="space-y-3">
                                <div className="flex items-center gap-4">
                                    <h2 className="text-3xl font-black text-slate-900  uppercase leading-none">{log.action || 'AKTIVITAS'}</h2>
                                    <div className={clsx("px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase  border", severityMap[currentSeverity])}>
                                        {currentSeverity === 'high' ? 'PENTING' : 'REGULER'}
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 text-slate-400 font-bold text-[10px] uppercase 
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                            <div className="flex items-center gap-4 px-6 py-4 bg-primary/5 rounded-lg border border-primary/10">
                                ...
                                <div className="text-left">
                                    <p className="text-[9px] font-bold text-slate-400 uppercase  leading-none mb-1">Durasi Terakhir</p>
                                    <p className="text-sm font-bold text-slate-700 leading-none">{formatDistanceToNow(new Date(log.created_at), { addSuffix: true, locale: id })}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="p-10 grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10 bg-slate-50/30">
                        <AttributeItem 
                    ...
                    <div className="lg:col-span-1 space-y-8">
                        <section className="bg-white rounded-lg p-10 border border-slate-100 relative overflow-hidden group/spec">
                            ...
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <h3 className="text-sm font-bold text-slate-900 uppercase  Teknis</h3>
                            </div>
                            <div className="space-y-8 relative z-10">
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase  mb-3">Otorisasi / Kemampuan</p>
                                    <code className="block px-4 py-3 rounded-xl bg-slate-50 text-slate-600 font-mono text-[11px] font-bold border border-slate-100">
                                        {log.ability ?? 'SISTEM_GLOBAL'}
                                    </code>
                                </div>
                                <div className="h-px bg-slate-50" />
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase  mb-3">Deskripsi Operasi</p>
                                ...
                                <div className="pt-4 border-t border-slate-50">
                                     <div className="flex items-center gap-3 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                                        ...
                                        <p className="text-[9px] text-emerald-600 font-bold uppercase  Data Terverifikasi</p>
                                     </div>
                                </div>
                            </div>
                        </section>
                    ...
                    <div className="lg:col-span-2 bg-white rounded-lg p-10 border border-slate-100 group/mut relative overflow-hidden">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10 relative z-10">
                            <div className="flex items-center gap-4">
                                ...
                                <h3 className="text-xl font-extrabold text-slate-900  uppercase">Perbandingan Perubahan</h3>
                            </div>
                            
                            {log.model_type && (
                                <div className="flex flex-col items-end gap-1">
                                    <span className="text-[9px] font-bold text-slate-400 uppercase  Referensi:</span>
                                    <span className="px-4 py-1.5 rounded-lg bg-slate-900 text-white font-mono text-[10px] font-bold uppercase 
                        ...
                        <div className="bg-slate-50/50rounded-lg border border-slate-100 p-8
                        ...
                        <div className="mt-10 p-8 bg-slate-50rounded-lg border border-slate-100">
                             <div className="flex flex-col md:flex-row md:items-center gap-6">
                                <div className="p-3 bg-white rounded-xl border border-slate-100 text-primary">
                                    <Scale className="h-5 w-5" />
                                </div>
                                <div className="space-y-1">
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase  leading-none">Insight Analitik</h4>
...
function AttributeItem({ label, title, subtitle, icon: Icon }: any) {
    return (
        <div className="space-y-4 group/attr">
            ...
            <div className="pl-5 border-l-2 border-slate-100 group-hover/attr:border-primary transition-colors">
                <p className="text-slate-900 font-extrabold text-xl  leading-none mb-1">{title}</p>
