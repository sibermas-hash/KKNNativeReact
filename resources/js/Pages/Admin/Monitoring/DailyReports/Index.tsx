...
export default function AdminDailyReportsIndex({ reports, filters }: Props) {
  ...
  return (
    <AppLayout title="Monitoring Logbook Harian">
      <Head title="Audit Logbook Harian | SIBERDAYA" />
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-8 py-10 font-sans">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Transmisi" value={reports.meta.total} icon={FileText} variant="success" />
        ...
        >
          <PremiumTable
            headers={['Aktivitas Mahasiswa', 'Identitas Personel', 'Lokasi/Kelompok', 'Status Audit', 'Opsi']}
              ...
              <PremiumTableRow key={r.id} className="group">
                <PremiumTableCell>
                  <div className="flex flex-col gap-1 py-1">
                    <span className="text-[13px] font-black text-emerald-950 uppercase tracking-tight leading-none group-hover:text-emerald-700 transition-colors line-clamp-1">{r.title}</span>
                    <div className="flex items-center gap-1.5 text-emerald-700/40">
                      <Calendar size={10} strokeWidth={3} />
                      <span className="text-[9px] font-black uppercase tracking-widest">{r.date}</span>
                    </div>
                  </div>
                </PremiumTableCell>
                <PremiumTableCell>
                  <div className="flex flex-col gap-1">
                    <span className="text-[11px] font-black text-emerald-950 uppercase tracking-tight leading-none">{r.student.name}</span>
                    <span className="text-[9px] font-bold text-emerald-600/50 font-mono tracking-tighter uppercase">{r.student.nim}</span>
                  </div>
                </PremiumTableCell>
                <PremiumTableCell>
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#F8FAF9] border border-emerald-50 rounded-lg">
                    ...
                    <span className="text-[10px] font-black text-emerald-900 uppercase tracking-tight">{r.group.name}</span>
                  </div>
                </PremiumTableCell>
                <PremiumTableCell>
                  <StatusTag 
                    status={
                      r.status === 'disetujui' ? 'active' : 
                      r.status === 'revisi' ? 'error' : 
                      r.status === 'submitted' ? 'pending' : 'draft'
                    } 
                    label={
                      r.status === 'disetujui' ? 'VERIFIED' : 
