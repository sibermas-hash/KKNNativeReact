    ...
    <AppLayout title="Manajemen Kelompok">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-10 py-10 font-sans">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            label="Unit Aktif"
            value={summary?.active_groups ?? 0}
            icon={Users}
            variant="success"
          />
          <StatCard
            label="Butuh DPL"
            value={summary?.groups_without_main_lecturer ?? 0}
            icon={UserCheck}
            variant="warning"
          />
          <StatCard
            label="Siap Ploting"
            value={summary?.groups_ready_for_placement ?? 0}
            icon={Target}
            variant="info"
          />
          <StatCard
                  ...
                  <PremiumTableRow key={group.id}>
                    <PremiumTableCell>
                      <div className="flex flex-col py-1">
                        <span className="text-[13px] font-black text-emerald-950 uppercase tracking-tight leading-tight font-display">
                          {group.name}
                        </span>
                        <div className="flex items-center gap-2 mt-1.5">
                          <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-[8px] font-black uppercase tracking-widest border border-slate-200">
                            #{group.code}
                          </span>
                        ...
                        <div className="flex items-center gap-1.5 mt-2 text-[9px] font-black text-emerald-800 uppercase tracking-tighter opacity-60">
                          ...
                          <span className="truncate max-w-[200px]">
                            {group.main_lecturer?.name || 'BELUM ADA DPL'}
                          </span>
                        </div>
                      </div>
                    </PremiumTableCell>
                    ...
                    <PremiumTableCell>
                      <div className="flex flex-col gap-2 w-32">
                        <div className="flex justify-between items-baseline">
                          <span className="text-[11px] font-black text-emerald-950 tabular-nums">
                        ...
                        <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
                          <div
                            className={clsx(
                              ...
                              group.approved_participants_count / group.capacity >= 0.9
                                ? 'bg-amber-500'
                                : 'bg-emerald-600',
                            )}
                            style={{
                        ...
                        {canManage && (
                          <>
                            <button
                              ...
                              className="h-8 w-8 flex items-center justify-center text-slate-300 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all"
                              title="Edit"
                            >
                              <Pencil size={14} />
                            </button>
        ...
        <form onSubmit={handleSync} className="p-8 space-y-6">
          <div className="p-5 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-start gap-4">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black text-emerald-950 uppercase tracking-widest leading-none mb-2 font-display">
                Logika Sinkronisasi:
              </p>
              <p className="text-[10px] text-emerald-800 leading-relaxed font-bold uppercase tracking-tight opacity-70">
                Sistem akan membuat kelompok baru untuk setiap desa yang belum memiliki unit
                bimbingan pada periode terpilih.
              </p>
            </div>
          </div>
