...
export default function ProfileContentPage({ content }: Props) {
  ...
  return (
    <AppLayout title="Profil Lembaga">
      <Head title="Manajemen Identitas Lembaga | SIBERDAYA" />
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-8 py-10 font-sans">
            ...
            footer={
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-emerald-950/40 uppercase tracking-widest leading-none">Identity Control Matrix Active</span>
                </div>
                <button 
                  type="submit" 
                  disabled={processing} 
            ...
            <div className="space-y-8 py-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 ml-1">
                  ...
                  <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest">Deskripsi Lembaga (Tentang)</label>
                </div>
                <textarea
                  rows={8}
                  value={data.about}
                ...
                />
                {errors.about && <p className="text-[10px] font-black text-rose-500 ml-1 uppercase tracking-tighter italic">Error: {errors.about}</p>}
              </div>
              ...
              <div className="grid gap-8 lg:grid-cols-2">
                <div className="space-y-3">
                  <div className="flex items-center gap-3 ml-1">
                    ...
                    <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest">Visi Strategis</label>
                  </div>
                  <textarea
                    rows={6}
                    value={data.visi}
                  ...
                  />
                  {errors.visi && <p className="text-[10px] font-black text-rose-500 ml-1 uppercase tracking-tighter italic">Error: {errors.visi}</p>}
                </div>
                <div className="space-y-3">
                  <div className="flex items-center gap-3 ml-1">
                    <Activity size={14} className="text-emerald-600" />
                    <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest">Misi Operasional</label>
                  </div>
                  <textarea
                    rows={6}
                    value={data.misi}
                  ...
                  />
                  {errors.misi && <p className="text-[10px] font-black text-rose-500 ml-1 uppercase tracking-tighter italic">Error: {errors.misi}</p>}
                </div>
              </div>
            </div>
          </ContentPanel>
        ...
        <div className="flex justify-center pt-8">
          <div className="h-10 px-6 bg-emerald-50 rounded-full flex items-center gap-3 border border-emerald-100/50">
            ...
            <span className="text-[10px] font-black text-emerald-950/40 uppercase tracking-widest">Governance Loop Active &bull; Identity Control Matrix</span>
          </div>
        </div>
      </div>
    </AppLayout>
