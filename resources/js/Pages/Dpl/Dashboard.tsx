      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pt-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Selamat Datang, {auth.user.name.split(',')[0]}. 👋</h1>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatBox label="Wilayah Tugas" value={coordinatorAreas.length || '—'} icon={MapPin} color="rose" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    groups.map((group) => (
                      <tr key={group.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-6 py-5">
                          <span className="px-2 py-1 bg-slate-900 text-white text-[10px] font-black rounded uppercase tracking-wider">#{group.code}</span>
                        </td>
                        <td className="px-6 py-5">
                          <div className="text-sm font-black text-slate-900 uppercase tracking-tight">{group.name}</div>
                          <div className="text-[10px] font-bold text-emerald-600 uppercase tracking-tighter mt-0.5">{group.period_name}</div>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5 text-right">
                          <Link href={`/dpl/kelompok/${group.id}`} className="inline-flex h-9 items-center gap-2 px-4 bg-white ring-1 ring-slate-200 text-[10px] font-black text-slate-900 hover:bg-slate-900 hover:text-white hover:ring-slate-900 rounded-lg transition-all uppercase tracking-widest active:scale-95">
          ...
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="space-y-2">
                {atRiskStudents.length > 0 ? (
                  atRiskStudents.map((student) => (
                    <div key={student.id} className="flex items-center gap-4 p-4 bg-slate-50 border border-slate-100 rounded-lg group transition-all hover:bg-rose-50 hover:border-rose-100">
                      <div className="h-9 w-9 bg-white ring-1 ring-slate-200 text-slate-400 group-hover:text-rose-600 rounded-lg flex items-center justify-center font-black text-xs shadow-sm">{student.name.charAt(0)}</div>
                      <div className="min-w-0">
                        <p className="text-[11px] font-black text-slate-900 uppercase truncate leading-tight">{student.name}</p>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{student.nim} • {student.group_code}</p>
                      </div>
                      <Link href={`/dpl/mahasiswa/${student.id}`} className="ml-auto p-1.5 text-slate-300 hover:text-rose-600"><ArrowRight size={14} /></Link>
                    </div>
                  ))
                ) : (
                  <div className="text-center py-10 border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/50">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">Seluruh Mahasiswa<br/>Terpantau Aktif</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-xl p-6 shadow-xl text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 p-6 opacity-5 -mr-4 -mt-4"><Activity size={80} /></div>
              <div className="flex items-center gap-2 mb-6">
                <div className="p-1.5 bg-emerald-600 text-white rounded-lg shadow-lg shadow-emerald-900/40"><Activity size={16} /></div>
              ...
              <div className="space-y-4 relative z-10">
                {activityTrend.length > 0 ? (
                  activityTrend.slice(0, 4).map((item) => (
                    <div key={item.date} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-colors">
                      <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">{item.date}</span>
                      <span className="text-[11px] font-black text-white">{item.count} LAPORAN</span>
                    </div>
                  ))
                ) : (
                  <p className="text-[10px] font-black text-white/30 uppercase text-center py-6 tracking-widest">Sinkronisasi Data...</p>
                )}
              </div>
            </div>
...
function StatBox({ label, value, icon: Icon, color }: any) {
  ...
  return (
    <div className="bg-white p-5 rounded-xl ring-1 ring-slate-200 shadow-sm flex items-center gap-5 group hover:ring-slate-300 transition-all">
      <div className={clsx('h-12 w-12 rounded-lg flex items-center justify-center ring-1 shadow-inner group-hover:scale-110 transition-transform', colors[color])}>
