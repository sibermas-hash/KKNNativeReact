...
interface Props {
  students: { data: StudentRecord[]; meta: PaginationMeta };
  filters: Filters;
  ...
  syncInfo: { mode: string; source: string; last_synced_at: string | null };
}
...
function normalizeFilters(f: Filters): Required<Filters> {
  return {
    search: f.search ?? '',
...
function buildQuery(f: Required<Filters>): Record<string, string | number> {
...
export default function MahasiswaIndex({
  ...
  const visiblePrograms = formFilters.faculty_id
    ? programs.filter((p) => String(p.faculty_id) === String(formFilters.faculty_id))
    : programs;
  const submitFilters = () =>
  ...
  const activeFilterCount =
    Object.values(buildQuery(formFilters)).filter((v) => v !== '').length -
    (formFilters.search ? 1 : 0);
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-emerald-50 pt-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-emerald-800 max-w-2xl mt-1">
                Pengelolaan data kepesertaan dan sinkronisasi registrasi mahasiswa KKN.
              </p>
            </div>
          ...
          <div className="bg-white border border-emerald-50 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-emerald-50 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={clsx(
                    'h-10 px-4 rounded-lg text-sm font-medium flex items-center gap-2 border shadow-sm transition-colors',
                    showFilters
                      ? 'bg-[#0d9488] text-white border-emerald-600'
                      : 'bg-white border-gray-300 text-emerald-800 hover:bg-gray-50',
                  )}
                >
                  <Filter size={15} />
                ...
                <button
                  ...
                  className="h-10 px-4 bg-[#0d9488] text-white rounded-lg text-sm font-medium hover:bg-[#0f766e] transition-colors shadow-sm"
                >
                  Terapkan
                </button>
              </div>
            ...
            {showFilters && (
              <div className="px-5 py-4 border-b border-emerald-50 bg-gray-50 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <FilterGroup
                  ...
                  onChange={(v) =>
                    setFormFilters({ ...formFilters, faculty_id: v, program_id: '' })
                  }
                  options={[
                ...
                <FilterGroup
                  ...
                  onChange={(v) => setFormFilters({ ...formFilters, program_id: v })}
                  options={[
                    { value: '', label: 'Semua Prodi' },
                    ...visiblePrograms.map((p) => ({ value: String(p.id), label: p.name })),
                  ]}
                />
                <FilterGroup
                  label="Angkatan"
                  value={String(formFilters.batch_year)}
                  onChange={(v) => setFormFilters({ ...formFilters, batch_year: v })}
                  options={[
                    { value: '', label: 'Semua Angkatan' },
                    ...batchYears.map((y) => ({ value: String(y), label: String(y) })),
                  ]}
                />
                <FilterGroup
                  label="Status BTA-PPI"
                  value={formFilters.bta_ppi}
                  onChange={(v) => setFormFilters({ ...formFilters, bta_ppi: v })}
                  options={[
                ...
                <div className="sm:col-span-2 lg:col-span-4 flex justify-end pt-2 border-t border-emerald-50">
                  <button
                    ...
                    className="text-sm text-emerald-800 hover:text-rose-600 transition-colors font-medium"
                  >
                    Reset Semua Filter
                  </button>
                </div>
            ...
            <div className="overflow-x-auto min-h-[300px]">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-emerald-800">
                      Data Mahasiswa
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-emerald-800">
                      Afiliasi Studi
                    ...
                    <th className="px-6 py-3 text-center text-xs font-semibold text-emerald-800">
                      Status Akun
                    </th>
                    <th className="px-6 py-3 text-right text-xs font-semibold text-emerald-800">
                      Aksi
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {!students?.data ? (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-sm text-emerald-800">
                        Memuat data...
                      </td>
                    </tr>
                      ...
                      <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-emerald-800 mt-0.5">
                              NIM: {s.nim}
                              {s.account && ` · @${s.account.username}`}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <span className="text-sm text-[#1f2937] truncate max-w-xs">
                              {s.prodi?.nama || '—'}
                            </span>
                            <span className="text-xs text-emerald-800 mt-0.5">
                              {s.fakultas?.nama || '—'}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2 text-xs text-emerald-800">
                              <span>
                                SKS: <strong>{s.sks_completed ?? 0}</strong>
                              </span>
                              <span className="text-emerald-700">·</span>
                              <span>
                                GPA: <strong>{Number(s.gpa ?? 0).toFixed(2)}</strong>
                              </span>
                              <span className="text-emerald-700">·</span>
                              <span>
                                Angk. <strong>{s.batch_year || '—'}</strong>
                              </span>
                            </div>
                            <span
                              className={clsx(
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              onClick={() => s.account && setConfirmReset(s)}
                              disabled={!s.account}
                              className="p-1.5 text-emerald-800 hover:text-[#0d9488] hover:bg-gray-50 rounded-md transition-colors disabled:opacity-30"
                              title="Reset Password"
                            >
                              <KeyRound size={16} />
                            </button>
                            <button
                              onClick={() => s.account && setConfirmToggle(s)}
                              disabled={!s.account}
                              className="p-1.5 text-emerald-800 hover:text-amber-600 hover:bg-amber-50 rounded-md transition-colors disabled:opacity-30"
                              title={s.account?.is_active ? 'Nonaktifkan' : 'Aktifkan'}
                            >
            ...
            <div className="px-6 py-4 border-t border-emerald-50 bg-gray-50 flex items-center justify-between">
              <span className="text-xs text-emerald-800">
                Hal. {students.meta.current_page} — Total <strong>{students.meta.total}</strong>{' '}
                data
              </span>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (confirmReset?.account) {
            router.post(
              `/admin/pengguna/${confirmReset.account.id}/reset-password-sementara`,
              {},
              { preserveScroll: true },
            );
          }
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (confirmToggle?.account) {
            router.patch(
              `/admin/pengguna/${confirmToggle.account.id}/toggle-status`,
              {},
              { preserveScroll: true },
            );
          }
        ...
        message={
          confirmToggle?.account?.is_active
    ...
    <div className="bg-white border border-emerald-50 rounded-xl shadow-sm p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
      <div
        className={clsx(
          ...
          color === 'emerald' ? 'bg-gray-50 text-[#0d9488]' : 'bg-gray-100 text-emerald-800',
        )}
      >
        <Icon size={18} strokeWidth={2} />
      </div>
      <div>
        <p className="text-lg font-bold text-emerald-950 leading-tight tabular-nums">
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <ChevronDown
          size={14}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-emerald-800 pointer-events-none"
        />
      </div>
    </div>
  );
