...
export default function AuditLogIndex({ logs, filters, stats }: Props) {
 ...
 <span className="text-xs font-semibold text-gray-700 leading-none mb-2">Populasi Record</span>
 ...
 <Terminal size={28} className="text-gray-700 drop-shadow-sm"/>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="px-10 py-10 bg-gray-50 border-b-2 border-gray-200 space-y-8">
 <div className="flex items-center gap-4">
 <div className="h-12 w-12 bg-white rounded-xl border border-gray-200 flex items-center justify-center text-[#1a7a4a] shadow-sm">
 <Binary size={24} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <h3 className="text-lg font-semibold text-gray-900 leading-none mb-1">Audit Stream</h3>
 <p className="text-xs font-bold text-gray-700">Aliran Aktivitas Kronologis Real-Time</p>
 </div>
 </div>

 <form onSubmit={handleSearch} className="relative group">
 <Search className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-600 group-focus-within:text-[#1a7a4a] transition-colors"strokeWidth={3} />
 <input 
 type="text"
 value={search} 
...
"group hover:bg-gray-50 transition-all font-sans",
 ...
 <span className="text-xs font-semibold text-gray-500 group-hover:text-gray-700 transition-colors font-mono">#{log.id.toString().padStart(6, '0')}</span>
 </td>
...
"h-12 w-12 rounded-xl flex items-center justify-center transition-all shadow-sm border shrink-0 group-hover:rotate-6",
 ...
 'bg-gray-50 border-gray-200 text-[#1a7a4a] group-hover:bg-[#16a34a] group-hover:text-white'
 )}>
 {log.severity === 'high' ? <ShieldAlert size={22} strokeWidth={2.5} /> : <Activity size={22} strokeWidth={2.5} />}
 </div>
 <div className="flex flex-col gap-2">
 <span className="text-sm font-semibold text-gray-900 group-hover:text-gray-700 transition-colors leading-tight max-w-[400px] truncate">{log.description}</span>
 <span className={clsx(
"text-xs font-semibold",
 log.severity === 'high' ? 'text-rose-500' : log.severity === 'medium' ? 'text-amber-500' : 'text-gray-700'
 )}>{log.action}</span>
 </div>
 </div>
 </td>
 ...
 <span className="px-3 py-1 bg-white border border-gray-200 rounded-lg text-xs font-semibold text-gray-600">AUTHORIZED_ACTOR</span>
 </div>
 </td>
 <td className="px-8 py-8 text-center">
 <span className="h-8 px-4 inline-flex items-center bg-white border border-gray-200 rounded-xl text-xs font-semibold text-[#1a7a4a] shadow-sm">
 ...
 <Link href={route('admin.audit-log.show', log.id)} className="text-xs font-semibold text-gray-600 hover:text-gray-900 flex items-center gap-2 group-hover:translate-x-0 translate-x-4 transition-all opacity-0 group-hover:opacity-100">
 ...
 Transmission Stage <strong className="text-gray-900 text-xs tabular-nums">{logs.meta.current_page}</strong> OF <strong className="text-gray-900 text-xs tabular-nums">{logs.meta.last_page}</strong> Active Frames
 </span>
 ...
 <span className="text-xs font-semibold text-gray-700 opacity-80">Protokol Keamanan Data Transparan</span>
 </div>
 </div>
...
"h-14 w-14 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-6 transition-all shadow-sm border",
 type === 'danger' ? 'bg-rose-50 border-rose-100 text-rose-500' : 'bg-gray-50 border-gray-200 text-[#1a7a4a]'
 )}>
...
function EmptyState() {
 ...
 <span className="text-sm font-semibold text-gray-900">Audit Silence</span>
 <p className="text-xs font-semibold text-gray-700 leading-none opacity-60">Tidak ditemukan aliran transmisi log untuk parameter pencarian saat ini.</p>
 </div>
 </td>
 </tr>
 );
