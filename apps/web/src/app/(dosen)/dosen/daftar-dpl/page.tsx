    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl bg-white ring-1 ring-slate-200 p-6 space-y-5">
        <div>
          ...
          <select value={formData.periode_id} onChange={(e) => setFormData({ ...formData, periode_id: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" required>
            <option value="">Pilih periode...</option>
            {periods.map((period) => <option key={period.id} value={period.id}>{period.name}</option>)}
          </select>
          {periodsLoading && <p className="text-xs text-slate-400 mt-1">Memuat periode...</p>}
          <p className="text-xs text-slate-500 mt-1">Pilih periode KKN yang akan Anda ikuti sebagai calon DPL</p>
        </div>

        <div>
          <label className="block text-xs font-black text-slate-700 uppercase tracking-wider mb-2">Catatan (Opsional)</label>
          <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 min-h-[100px]" placeholder="Catatan tambahan untuk admin..." />
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-200 p-5">
              <div>
                <h2 className="text-lg font-black text-slate-900">Pernyataan Kesediaan Menjadi DPL</h2>
                <p className="text-sm text-slate-500">Baca dengan saksama sebelum mengirim pendaftaran.</p>
              </div>
              <button type="button" onClick={() => setShowStatement(false)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><X size={18} /></button>
            </div>
            ...
            <div className="space-y-5 p-6 text-sm leading-7 text-slate-700">
              ...
              <div className="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200">
                <p><span className="font-bold">Nama akun:</span> {user?.name ?? '-'}</p>
                <p><span className="font-bold">NIP/NIDN:</span> {user?.username ?? '-'}</p>
                <p><span className="font-bold">Periode:</span> {selectedPeriod?.name ?? '-'}</p>
              </div>
              ...
              <div className="grid gap-3 rounded-xl border border-slate-200 p-4 md:grid-cols-2">
                <label className="block text-xs font-black uppercase tracking-wider text-slate-700">Nama tanpa gelar <span className="text-red-500">*</span>
                  <input value={signature.nama} onChange={(e) => setSignature({ ...signature, nama: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-semibold normal-case tracking-normal focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" placeholder="Contoh: Shofiyulloh" />
                </label>
                <label className="block text-xs font-black uppercase tracking-wider text-slate-700">NIP/NIDN <span className="text-red-500">*</span>
                  <input value={signature.nip} onChange={(e) => setSignature({ ...signature, nip: e.target.value })} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-semibold normal-case tracking-normal focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" placeholder="NIP/NIDN" />
                </label>
              </div>
            </div>

            <div className="flex flex-col-reverse gap-3 border-t border-slate-200 p-5 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setShowStatement(false)} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50">Batal</button>
              <button type="button" onClick={handleConfirmStatement} disabled={submitMut.isPending} className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50">{submitMut.isPending ? 'Mengirim...' : 'Setuju dan Kirim Pendaftaran'}</button>
            </div>
          </div>
        </div>
      )}
