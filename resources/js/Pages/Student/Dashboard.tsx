...
export default function StudentDashboard({
  ...
  const phases = [
    { id: 1, label: 'Registrasi', done: isApproved, active: isPending || !registration },
    {
      id: 2,
      label: 'Persiapan',
      done: workProgramCount > 0,
      active: isApproved && workProgramCount === 0,
    },
    {
      id: 3,
      label: 'Pelaksanaan',
      done: dailyReportCount >= (registration?.period?.min_logbook ?? 30),
      active: workProgramCount > 0 && dailyReportCount < (registration?.period?.min_logbook ?? 30),
    },
    {
      id: 4,
      label: 'Pelaporan',
      done: !!finalReport,
      active: dailyReportCount >= (registration?.period?.min_logbook ?? 30) && !finalReport,
    },
    {
      id: 5,
      label: 'Penilaian',
      done: grade?.is_finalized,
      active: !!finalReport && !grade?.is_finalized,
    },
  ];
          ...
          {showPopup && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
              <motion.div
                ...
                className="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 border ring-1 ring-slate-200"
              >
                <div className="text-center">
                  ...
                  <p className="text-sm text-slate-500 mb-6 font-medium">
                    {isApproved
                      ? 'Selamat! Anda telah resmi terdaftar sebagai peserta KKN.'
                      : 'Maaf, berkas Anda memerlukan perbaikan.'}
                  </p>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900 italic leading-relaxed">
                          ...
                          {registration?.rejection_reason ||
                            'Periksa kembali kelengkapan berkas Anda.'}
                          "
                        </p>
                      </div>
                    )}
                ...
                <button
                  ...
                  className="w-full mt-8 h-12 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black uppercase tracking-widest rounded-lg shadow-lg shadow-emerald-600/20 transition-all active:scale-[0.98]"
                >
                  Selesai & Mengerti
                </button>
              </motion.div>
                    ...
                    <div
                      ...
                      className={clsx(
                        ...
                        phase.done
                          ...
                          : phase.active
                            ? 'bg-white border-slate-300 ring-1 ring-inset ring-slate-100'
                    ...
                    >
                      ...
                      <span
                        className={clsx(
                          ...
                          phase.done ? 'text-emerald-900' : 'text-slate-600',
                        )}
                      >
                        {phase.label}
                      </span>
              ...
              {!isApproved && (
                <div className="bg-slate-900 rounded-xl p-8 text-white relative overflow-hidden shadow-xl">
                  ...
                  <div className="relative z-10 space-y-6">
                      ...
                      <p className="text-sm font-medium text-slate-400 max-w-xl leading-relaxed">
                        {isRejected
                          ...
                          : isPending
                            ...
                            : 'Daftarkan diri Anda sekarang untuk mengikuti program KKN Periode 2026/2027.'}
                      </p>
                    </div>
                    <Link
                      href={
                        registration ? route('student.registration.status') : '/mahasiswa/daftar'
                      }
                    ...
                    >
                      {registration ? 'Cek Detail Status' : 'Mulai Pendaftaran'}{' '}
                      <ArrowRight size={16} />
                    </Link>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        {groupName} • {registration?.group?.code}
                      </p>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-white ring-1 ring-slate-200 rounded-xl p-6 shadow-sm">
                ...
                <div className="grid gap-2">
                  ...
                  />
                  <NavButton
                    href={route('student.rekapitulasi.index')}
                    icon={Activity}
              ...
              <div className="bg-emerald-50/50 ring-1 ring-emerald-100 rounded-xl p-6">
                ...
                <ul className="space-y-4">
                  <li className="flex gap-3">
                    <div className="h-5 w-5 bg-emerald-600 text-white rounded-full flex items-center justify-center shrink-0 text-[10px] font-bold">
                      1
                    </div>
                    <p className="text-xs font-semibold text-emerald-950 leading-relaxed">
                      Pastikan Logbook diisi setiap hari paling lambat pukul 23:59 WIB.
                    </p>
                  </li>
                  <li className="flex gap-3">
                    <div className="h-5 w-5 bg-emerald-600 text-white rounded-full flex items-center justify-center shrink-0 text-[10px] font-bold">
                    ...
                    <p className="text-xs font-semibold text-emerald-950 leading-relaxed">
                      Minimal 30 laporan harian yang divalidasi DPL untuk syarat kelulusan.
                    </p>
                  </li>
                </ul>
              </div>
...
function NavButton({ href, icon: Icon, label }: any) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 p-3 rounded-lg border border-transparent hover:border-emerald-100 hover:bg-emerald-50 transition-all group"
    >
      <div className="p-2 bg-slate-50 text-slate-400 rounded-md group-hover:bg-emerald-600 group-hover:text-white transition-all">
        <Icon size={16} />
      </div>
      <span className="text-xs font-bold text-slate-700 group-hover:text-emerald-900 transition-colors uppercase tracking-tight">
        {label}
      </span>
      <ArrowRight
        size={14}
        className="ml-auto text-slate-200 group-hover:text-emerald-600 group-hover:translate-x-1 transition-all"
      />
    </Link>
  );
}
