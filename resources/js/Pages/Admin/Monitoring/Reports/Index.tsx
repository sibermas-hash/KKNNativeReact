...
export default function ReportsIndex({ reports, summary }: Props) {
 ...
 <span className="text-xs font-semibold text-emerald-800 leading-none mb-2">Total Aset KKN</span>
 ...
 <div className="px-10 py-10 bg-gray-50 border-b-2 border-emerald-50 flex flex-col lg:flex-row items-center justify-between gap-8">
 ...
 <div className="flex flex-col">
 <h3 className="text-xl font-semibold text-emerald-950 leading-none mb-1.5">Manifest Komponen Riset</h3>
 <p className="text-xs font-bold text-emerald-800 leading-none">Direktori Inventori Produk Akademik KKN</p>
 </div>
 </div>

 <div className="flex items-center gap-4 w-full lg:w-auto">
 <div className="relative flex-1 lg:w-72 group">
 <Search size={18} className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-800 group-focus-within:text-[#1a7a4a] transition-colors"strokeWidth={3} />
 <input 
 value={search} 
 ...
 <div className="h-12 w-12 bg-white border border-emerald-50 text-emerald-700 rounded-xl flex items-center justify-center group-hover:bg-[#16a34a] group-hover:text-white group-hover:border-emerald-600 transition-all shadow-sm">
 <FileText size={22} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <span className="text-sm font-semibold text-emerald-950 leading-tight group-hover:text-emerald-800 transition-colors max-w-[400px] truncate mb-2">{report.title}</span>
 <span className="text-xs font-semibold text-emerald-800 font-mono leading-none">{report.file_name}</span>
 </div>
 </div>
 </td>
 <td className="px-8 py-8 text-center text-center">
 <div className="flex flex-col items-center gap-1.5">
 <span className="text-xs font-semibold text-emerald-950 leading-none">{report.group.name}</span>
 <span className="text-xs font-semibold text-emerald-800">{report.user.name}</span>
 </div>
 </td>
 <td className="px-8 py-8 text-center">
 <span className="inline-flex px-4 py-1.5 bg-white border border-emerald-50 text-[#1a7a4a] text-xs font-semibold rounded-xl shadow-sm">
 ...
 className="h-10 px-5 bg-white border border-emerald-50 text-emerald-950 hover:bg-white hover:text-white rounded-xl text-xs font-semibold transition-all active:scale-95 flex items-center gap-3 shadow-sm group-hover:border-gray-300"
 >
 INJECT_ASSET <ArrowDownToLine size={14} strokeWidth={3} />
 </Link>
 <button className="h-10 w-10 bg-white border border-emerald-50 text-emerald-800 hover:text-emerald-950 rounded-xl flex items-center justify-center transition-all active:scale-90 shadow-sm border">
 ...
 Data Halaman <strong className="text-emerald-950 text-xs tabular-nums">{reports.meta?.current_page || 1}</strong> Per <strong className="text-emerald-950 text-xs tabular-nums">{(summary.total_reports || 0).toLocaleString('id-ID')}</strong> Entitas Pustaka
 </span>
 ...
 <span className="text-xs font-semibold text-emerald-800 opacity-80">Protokol Verifikasi Produk Digital</span>
 </div>
 </div>
 ...
 <span className="text-xs font-semibold text-emerald-800 mb-1 opacity-60">STORAGE SECURITY</span>
 <span className="text-2xl font-semibold text-white italic er text-emerald-800">AMANKAN</span>
 </div>
 </div>
 </div>
 </div>
...
"h-14 w-14 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-6 transition-all shadow-sm border",
 type === 'warning' ? 'bg-amber-50 border-amber-100 text-amber-500' : 'bg-gray-50 border-emerald-50 text-[#1a7a4a]'
 )}>
...
function EmptyState() {
 ...
 <span className="text-sm font-semibold text-emerald-950">Vault Buffer Nihil</span>
 <p className="text-xs font-semibold text-emerald-800 leading-none opacity-60">Tidak ditemukan aliran data aset untuk parameter filter saat ini.</p>
 </div>
 </td>
 </tr>
 );
