...
sudo chmod -R u+rwX,g+rX,o+rX app database config routes bootstrap/app.php bootstrap/providers.php
...
echo "[4/4] verify"
unreadable_count=$(find app database config routes bootstrap/app.php bootstrap/providers.php -type f ! -perm -004 | wc -l | tr -d ' ')
...
if [ "$unreadable_count" != "0" ]; then
  ...
  find app database config routes bootstrap/app.php bootstrap/providers.php -type f ! -perm -004 -ls | head -50 >&2
  exit 1
fi
