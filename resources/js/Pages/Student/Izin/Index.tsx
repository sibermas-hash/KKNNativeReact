...
export default function StudentIzinIndex({ izins, akumulasiTanpaKeterangan }: Props) {
    ...
    const statusColors: Record<string, { bg: string, text: string, ring: string, dot: string }> = {
        menunggu: { bg: 'bg-amber-50', text: 'text-amber-700', ring: 'ring-amber-200/50', dot: 'bg-amber-500' },
        disetujui: { bg: 'bg-emerald-50', text: 'text-emerald-700', ring: 'ring-emerald-200/50', dot: 'bg-emerald-500' },
        ditolak: { bg: 'bg-rose-50', text: 'text-rose-700', ring: 'ring-rose-200/50', dot: 'bg-rose-500' },
    };
            ...
            <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12 font-sans">
                        ...
                        <div className="space-y-8 max-w-2xl">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-1">
                                    <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.4em]">Section 04 / Clearance Protocol</h4>
                            ...
                            <p className="text-lg font-bold text-slate-400 tracking-tight leading-relaxed">
                                ...
                                <span className="text-slate-900 italic">"Disiplin adalah fondasi pengabdian yang bermartabat."</span>
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className={clsx(
                        ...
                        ? "bg-rose-950 border-rose-900 text-white shadow-2xl shadow-rose-900/20" 
                        ...
                        <div className="h-40 w-40 shrink-0 bg-white/10 rounded-[3rem] flex items-center justify-center shadow-2xl">
                             <div className="text-center">
                                ...
                                <span className="block text-[10px] font-black uppercase tracking-widest opacity-60">Hari</span>
                             </div>
                        </div>
                        <div className="space-y-6 relative z-10">
                            <div>
                                ...
                                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500 opacity-80">Absensi Tanpa Keterangan (Alpha)</p>
                            </div>
                            <p className="text-sm font-bold opacity-70 leading-relaxed uppercase tracking-tight max-w-xl">
                                Akumulasi ketidakhadiran tanpa izin resmi telah mencapai batas pantau. {akumulasiTanpaKeterangan >= 3 
                    ...
                    <div className="bg-emerald-50 rounded-[3.5rem] p-12 border border-emerald-100 flex flex-col justify-between group">
                         ...
                         <div className="space-y-2">
                            <h4 className="text-5xl font-black text-slate-950 tracking-tighter uppercase leading-none">{izins.data.length}</h4>
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em]">Total Clearance Log</p>
                         </div>
                    </div>
                </div>
                ...
                <div className="space-y-10">
                    <div className="flex items-center justify-between px-4">
                        <div className="flex items-center gap-4">
                            <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.4em]">Clearance History</h3>
                    ...
                    >
                        {izins.data.length > 0 ? izins.data.map((izin) => (
                            <motion.div 
                                key={izin.id}
                            ...
                            >
                                ...
                                <div className="xl:w-64 shrink-0 space-y-6">
                                    <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100 group-hover:border-emerald-200 transition-colors">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Duration: {izin.durasi_hari} Days</p>
                                        <div className="space-y-1">
                                            <p className="text-lg font-black text-slate-900 tracking-tight uppercase">{new Date(izin.tanggal_mulai).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</p>
                                            <div className="h-4 w-px bg-slate-200 ml-4 my-1" />
                                            <p className="text-lg font-black text-slate-900 tracking-tight uppercase">{new Date(izin.tanggal_kembali).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                                        </div>
                                    </div>
                                    <div className={clsx(
                                        ...
                                        <div className={clsx("w-2 h-2 rounded-full", statusColors[izin.status].dot)} />
                                ...
                                <div className="flex-1 space-y-8">
                                    <div className="space-y-4">
                                        <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em]">Justification Log</h4>
                                        <div className="p-8 bg-slate-50 border border-slate-100 rounded-[2rem] relative group/reason">
                                            <p className="text-sm font-bold text-slate-600 leading-relaxed uppercase tracking-tight italic">
                                                "{izin.alasan}"
                                            </p>
                                            <FileText className="absolute right-8 top-8 text-slate-200 opacity-20 group-hover/reason:opacity-100 transition-opacity" size={40} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-10">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{izin.kelompok.nama_kelompok}</span>
                                        </div>
                                        {izin.diproses_oleh && (
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Verified by {izin.diproses_oleh.name}</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {izin.catatan_dpl && (
                                        <div className="p-8 bg-emerald-50/50 border border-emerald-100/50 rounded-[2rem]">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">DPL Feedback Protocol</span>
                                            </div>
                                            <p className="text-xs font-bold text-emerald-900 leading-relaxed uppercase tracking-tight">{izin.catatan_dpl}</p>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
                            ...
                            <div className="py-32 flex flex-col items-center gap-8 text-center bg-slate-50 rounded-[4rem] border border-dashed border-slate-200">
                                ...
                                <div className="space-y-2">
                                    <h4 className="text-2xl font-black text-slate-300 uppercase tracking-tighter">Zero Clearance Issued</h4>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Anda belum memiliki riwayat pengajuan izin lokasi.</p>
                                </div>
                            </div>
                        )}
                    </motion.div>
