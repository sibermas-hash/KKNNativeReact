...
const QuillEditor = ({ value, onChange, placeholder }: { value: string; onChange: (val: string) => void; placeholder?: string; }) => {
 ...
 return <div className="bg-white border border-emerald-100/60 rounded-xl overflow-hidden min-h-[400px]"><div ref={editorRef} /></div>;
};
...
export default function AnnouncementIndex({ announcements }: Props) {
 ...
 <Megaphone size={18} />
 <span className="text-xs font-bold tracking-[0.25em] opacity-80">Konten Publik & Komunikasi</span>
 </div>
 ...
 className="h-14 px-8 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold shadow-xl shadow-emerald-100 flex items-center gap-3 text-sm transition-all active:scale-95 tracking-wider"
 >
 ...
 <tr key={a.id} className="group hover:bg-emerald-50/30/50 transition-all">
 <td className="px-6 py-4">
 <div className="flex items-center gap-4">
 {a.image ? (
 <img src={`/storage/${a.image}`} className="h-10 w-16 rounded-lg object-cover border border-emerald-100/60 shadow-sm" alt="" />
 ) : (
 <div className="h-10 w-16 bg-emerald-50/30 border border-emerald-100/60 text-slate-200 rounded-lg flex items-center justify-center"><ImageIcon size={16} /></div>
 )}
 <div className="flex flex-col">
 <span className="text-sm font-bold text-black group-hover:text-emerald-700 transition-colors truncate max-w-[300px]">{a.title}</span>
 ...
 <span className="inline-flex h-6 items-center px-3 bg-emerald-600 text-white rounded-md text-sm font-bold font-semibold text-xs">{a.category}</span>
 </td>
 <td className="px-6 py-4 text-center">
 <div className={clsx('inline-flex h-6 items-center px-3 rounded-md text-sm font-bold font-semibold text-xs border', a.meta_title ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-emerald-50/30 text-slate-300 border-emerald-100/60')}>
 ...
 <button onClick={() => openEditModal(a)} className="h-8 w-8 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 text-emerald-950 hover:text-emerald-600 hover:border-emerald-200 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-emerald-900/5 transition-all"><Edit2 size={14} /></button>
 <button onClick={() => { if(confirm('Hapus warta?')) router.delete(route('admin.warta-utama.destroy', a.id)) }} className="h-8 w-8 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 text-emerald-950 hover:text-rose-500 hover:border-rose-200 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-emerald-900/5 transition-all"><Trash2 size={14} /></button>
 </div>
 </td>
 </tr>
 ))}
 ...
 <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-5xl bg-white shadow-2xl overflow-hidden rounded-xl border border-emerald-100/60 flex flex-col max-h-[90vh]">
 ...
 <h3 className="text-sm font-bold font-semibold text-xs">{editingAnnouncement ? 'Revise_Narrative' : 'Initial_Narrative'}</h3>
 </div>
 ...
 <input value={data.title} onChange={e => { setData('title', e.target.value); if(!editingAnnouncement) generateSlug(e.target.value); }} className="w-full text-2xl font-bold text-black placeholder:text-slate-100 bg-transparent border-none focus:ring-0 outline-none p-0 tracking-tight leading-none" placeholder="ENTER_TITLE..." required />
 <div className="flex items-center gap-2 mt-2 px-3 py-1 bg-emerald-50/30 rounded-md border border-emerald-100/60 w-fit">
 <span className="text-sm font-bold text-emerald-950 font-semibold text-xs">/warta/</span>
 <input value={data.slug} onChange={e => setData('slug', e.target.value)} className="bg-transparent border-none p-0 focus:ring-0 text-sm font-bold text-emerald-600 w-48 font-semibold text-xs" />
 </div>
 </div>
 <div className="space-y-1">
 ...
 <div className="bg-emerald-50/30 rounded-xl p-4 border border-emerald-100/60 space-y-4">
 <div className="space-y-2">
 <label className="text-sm font-bold text-emerald-950 font-semibold text-xs block ml-1">Asset_Proxy</label>
 <div className="relative h-32 w-full bg-white rounded-lg border-2 border-dashed border-emerald-100/60 flex flex-col items-center justify-center gap-2 overflow-hidden group/img">
 ...
 <input type="datetime-local" value={data.published_at} onChange={e => setData('published_at', e.target.value)} className="w-full h-9 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 rounded-2xl px-3 text-sm font-bold outline-none focus:border-emerald-500 shadow-xl shadow-emerald-900/5 transition-all" />
 </div>
 <div className="flex items-center justify-between gap-2 px-1">
 <span className="text-sm font-bold text-emerald-950 font-semibold text-xs">Matrix_Active</span>
 <input type="checkbox" checked={data.is_active} onChange={e => setData('is_active', e.target.checked)} className="h-4 w-4 rounded border-emerald-100/60 text-emerald-600 focus:ring-emerald-500 shadow-sm" />
 </div>
 </div>
 <div className="bg-white border border-emerald-100/60 rounded-xl p-4 space-y-4">
 <div className="space-y-2">
 <label className="text-sm font-bold text-emerald-950 font-semibold text-xs block ml-1">SEO_Title</label>
 <input value={data.meta_title} onChange={e => setData('meta_title', e.target.value)} className="w-full h-9 bg-emerald-50/30 border border-emerald-100/60 rounded-lg px-3 text-sm font-bold outline-none focus:border-emerald-500" placeholder="Meta title..." />
 </div>
 <div className="space-y-2">
 <label className="text-sm font-bold text-emerald-950 font-semibold text-xs block ml-1">SEO_Desc</label>
 <textarea value={data.meta_description} onChange={e => setData('meta_description', e.target.value)} rows={3} className="w-full bg-emerald-50/30 border border-emerald-100/60 rounded-lg px-3 py-2 text-sm font-bold outline-none focus:border-emerald-500 resize-none" placeholder="Meta description..." />
 </div>
 </div>
 </aside>
 </form>
 </div>
 <div className="px-6 py-4 border-t border-slate-50 flex items-center justify-between bg-emerald-50/30/20 shrink-0">
 <span className="text-sm font-bold text-slate-300 font-semibold text-xs">Narrative Stream Buffer v2.5</span>
 <Button form="cms-form" type="submit" disabled={processing} className="h-10 px-8 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold font-semibold text-xs shadow-xl shadow-emerald-100 flex items-center gap-3 group active:scale-95">{processing ? <RefreshCw size={14} className="animate-spin" /> : <Save size={14} className="text-white" />} COMMIT_BROADCAST</Button>
 </div>
 </motion.div>
 </div>
 )}
