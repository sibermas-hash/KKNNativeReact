...
export default function AdminDailyReportsIndex({ reports, filters }: Props) {
 ...
 <div className="max-w-7xl mx-auto space-y-6 pb-12 font-sans px-4 sm:px-6 lg:px-8 text-gray-900">
 ...
 <span className="text-xs font-bold text-gray-700 tracking-wide opacity-90">Total Transmisi</span>
 ...
 {/* --- DATA TABLE CARD --- */}
 <section className="bg-white border text-sm border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
 <div className="px-6 py-5 bg-gray-50 border-b border-gray-200 space-y-4">
 <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
 <div className="md:col-span-5 relative group">
 <label className="text-xs font-bold text-gray-700 tracking-wide mb-1.5 block">Cari Laporan</label>
 <div className="relative">
 <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-700 group-focus-within:text-[#1a7a4a]"/>
 <input 
 type="text"
 value={search} 
 onChange={(e) => setSearch(e.target.value)} 
 onKeyDown={(e) => e.key === 'Enter' && handleApplyFilters()} 
 className="w-full h-10 pl-10 pr-4 bg-white border border-gray-300 rounded-lg text-xs font-semibold text-gray-900 focus:border-[#1a7a4a] focus:ring-1 focus:ring-[#1a7a4a] outline-none transition-all placeholder:text-gray-600"
 ...
 <label className="text-xs font-bold text-gray-700 tracking-wide mb-1.5 block">Status Audit</label>
 <div className="relative">
 <select 
 value={status} 
 onChange={(e) => setStatus(e.target.value)} 
 className="w-full h-10 pl-4 pr-10 bg-white border border-gray-300 rounded-lg text-xs font-semibold text-gray-900 focus:border-[#1a7a4a] focus:ring-1 focus:ring-[#1a7a4a] outline-none transition-all appearance-none"
 >
 ...
 <ChevronRight size={14} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-700 rotate-90 pointer-events-none"/>
 </div>
 </div>
 ...
 <span className="text-sm font-semibold text-[#1f2937] group-hover:text-gray-700 max-w-[300px] truncate"title={r.title}>{r.title}</span>
 ...
 <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 border border-gray-200 rounded-md text-gray-700">
 ...
 className="px-3 py-1.5 hover:bg-[#e8f5ee] text-gray-700 hover:text-gray-700 rounded-md text-xs font-bold transition-colors inline-flex items-center gap-1"
 >
 ...
 Total: <strong className="text-gray-900">{reports.meta.total}</strong> Laporan
 </span>
 ...
 <ShieldCheck size={28} className="text-gray-700 shrink-0 mt-0.5"/>
 <div>
 ...
 <div className="text-xs font-bold text-gray-700 bg-white px-3 py-1.5 rounded border border-emerald-800 shrink-0 hidden sm:block">
 SISTEM AKTIF
 </div>
 </div>
 </div>
...
function MetricCard({ label, value, icon: Icon, desc }: { label: string; value: any; icon: any; desc: string }) {
 return (
 <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4 shadow-sm relative">
 <div className="h-10 w-10 shrink-0 rounded-lg flex items-center justify-center text-[#1a7a4a] bg-gray-50 border border-gray-200">
 <Icon size={18} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <span className="text-xs font-bold text-[#1a7a4a]">{label}</span>
 <span className="text-lg font-semibold text-gray-900 leading-tight truncate">{value}</span>
 </div>
 </div>
 );
}
...
function EmptyState() {
 ...
 <div className="h-16 w-16 bg-gray-50 rounded-xl flex items-center justify-center text-gray-600 border border-gray-200">
