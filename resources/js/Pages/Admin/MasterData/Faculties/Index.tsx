...
export default function FacultiesIndex({ faculties = { data: [], meta: { total: 0, current_page: 1, last_page: 1 } }, filters = {} }: Props) {
 ...
 <span className="text-sm font-bold tracking-wider text-xs font-semibold leading-none">Institusional &middot; Arsitektur Fakultas</span>
 </div>
 ...
 <span className="text-sm font-bold text-emerald-700/40 font-semibold text-xs leading-none mb-2">Total Fakultas</span>
 <span className="text-xl font-bold text-black tabular-nums leading-none tracking-tight">{(faculties?.meta?.total ?? 0).toLocaleString()} DATA UNIT</span>
 </div>
 </div>
 <button 
 ...
 <p className="text-sm font-bold text-emerald-700/40 tracking-wider text-xs font-semibold">Matriks Distribusi Mahasiswa &middot; {faculties?.meta?.total ?? 0} Unit</p>
 </div>
 </div>
 ...
 className="w-full h-16 pl-20 pr-8 bg-white border border-gray-200 rounded-xl text-sm font-bold text-black focus:border-emerald-500 outline-none transition-all placeholder:text-emerald-50/50 tracking-[0.1em]" 
 ...
 <tr key={faculty.id} className="hover:bg-emerald-50/30 transition-all duration-300 group">
 <td className="px-6 py-6">
 <span className="text-sm font-bold text-emerald-600 bg-emerald-50 border border-gray-200 px-6 py-2 rounded-xl tabular-nums shadow-sm">
 {faculty?.code || 'SYST_NONE'}
 </span>
 </td>
 <td className="px-6 py-6">
 <div className="flex flex-col">
 <span className="text-lg font-bold text-black group-hover:text-emerald-700 transition-colors leading-none tracking-tight truncate max-w-[400px]">{faculty?.name}</span>
 <span className="text-sm font-bold text-emerald-700/20 mt-3 font-semibold text-xs">Identitas Sistem &middot; #{faculty?.id}</span>
 </div>
 </td>
 <td className="px-6 py-6">
 <div className="flex flex-col items-center gap-2">
 <span className="text-base font-bold text-black tabular-nums tracking-tight group-hover:text-emerald-600 transition-colors">{(faculty?.students_count || 0).toLocaleString()} UNIT</span>
 <span className="text-sm font-bold text-emerald-700/30 tracking-wider text-xs font-semibold leading-none">{faculty?.programs_count || 0} PRODI TERKAIT</span>
 </div>
 </td>
 ...
 className="h-12 px-8 bg-white border border-emerald-100 text-emerald-200 hover:text-emerald-600 hover:border-emerald-200 transition-all rounded-2xl flex items-center text-sm font-bold tracking-wider text-xs font-semibold shadow-sm active:scale-95 no-underline"
 >
 <Edit2 size={16} strokeWidth={2.5} className="mr-3" /> KOREKSI
 </Link>
 <button 
 onClick={() => handleDelete(faculty?.id)} 
 className="h-12 w-12 bg-white border border-emerald-100 text-emerald-100/50 hover:text-rose-600 hover:border-rose-100 hover:bg-rose-50 transition-all rounded-2xl flex items-center justify-center shadow-sm active:scale-95 border-none"
 >
 ...
 <p className="text-xs font-bold tracking-[0.6em] leading-none opacity-30">Database Fakultas Kosong</p>
 </div>
 </td></tr>
 )}
 </tbody>
 ...
 <span className="text-sm font-bold text-emerald-700/20 tracking-wider text-xs font-semibold leading-none">Data Sistem &middot; {faculties?.meta?.total ?? 0} DATA TERDAFTAR</span>
...
function FacultyMetric({ label, value, icon: Icon }: { label: string, value: string | number, icon: any }) {
 ...
 <div className="h-14 w-14 bg-emerald-50 text-emerald-600 rounded-[1.2rem] flex items-center justify-center group-hover:rotate-6 transition-transform shadow-sm border-none"><Icon size={24} strokeWidth={2.5} /></div>
 <div className="flex flex-col z-10">
 <span className="text-sm font-bold text-emerald-700/40 tracking-wider text-xs font-semibold leading-none mb-2">{label}</span>
 <span className="text-3xl font-bold text-black font-bold text-center tabular-nums leading-none group-hover:text-emerald-600 transition-colors ">{value}</span>
 </div>
 </div>
 );
}
