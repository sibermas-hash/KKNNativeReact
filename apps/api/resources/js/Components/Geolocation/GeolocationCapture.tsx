...
const GeolocationCapture: React.FC<{ onSuccess?: () => void }> = ({ onSuccess }) => {
  ...
  const removeFromIndexedDB = useCallback(
    async (storeName: string, key: any) => {
      try {
        const db = await openIndexedDB();
        const tx = db.transaction(storeName, 'readwrite');
        const store = tx.objectStore(storeName);
  ...
  const addWatermark = (context: CanvasRenderingContext2D) => {
    ...
    // Add GPS coordinates
    if (geoData) {
      context.fillText(`${geoData.latitude.toFixed(6)}, ${geoData.longitude.toFixed(6)}`, 10, 55);
    }
  };
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        alert('📱 Data tersimpan secara lokal. Akan otomatis sinkronisasi saat koneksi kembali.');
      }
  ...
  return (
    <div className="space-y-4 bg-white rounded-lg p-6 shadow">
      ...
      <div>
        <label className="block text-sm font-medium text-emerald-950 mb-2">Jenis Aktivitas</label>
        <select
          value={activityType}
