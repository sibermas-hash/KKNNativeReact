...
export default function LocationsIndex({ locations, filters, summary, workflow, periods, active_periode_id, active_periode_name }: Props) {
  const [search, setSearch] = useState(filters.search ?? '');
  const [selectedPeriodId, setSelectedPeriodId] = useState(filters.period_id ?? String(active_periode_id ?? ''));
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (search !== (filters.search ?? '')) {
        router.get('/admin/locations', { search: search || undefined, period_id: selectedPeriodId || undefined }, { preserveState: true, replace: true, preserveScroll: true });
      }
    }, 400);
  ...
  useEffect(() => {
    if (selectedPeriodId !== (filters.period_id ?? '')) {
      router.get('/admin/locations', { period_id: selectedPeriodId || undefined, search: search || undefined }, { preserveState: true, replace: true, preserveScroll: true });
    }
  }, [selectedPeriodId]);
  ...
  function handleExport() {
    const url = route('locations.export', { search: search || undefined, period_id: selectedPeriodId || undefined });
    ...
    <AppLayout title="Manajemen Wilayah KKN">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pb-24 font-sans">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Titik Lokasi" value={summary.total_locations} icon={Building2} variant="gray" />
          <StatCard label="Unit Kelompok" value={summary.assigned_groups} icon={MapPinned} variant="success" />
          <StatCard label="Posko Aktif" value={summary.reported_posko} icon={House} variant="warning" />
          <StatCard 
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="lg:w-1/4 space-y-5 border-r border-gray-100 pr-8">
                ...
                <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100/30 flex items-start gap-3">
                   ...
                   <p className="text-[10px] font-bold text-emerald-800/60 uppercase tracking-tight leading-relaxed">
                     Pilih periode untuk memfilter lokasi yang memiliki penempatan kelompok pada periode tersebut.
                   </p>
                </div>
              </div>
              ...
              <form onSubmit={submitForm} className="flex-1">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Desa/Kelurahan</label>
                    <input
                      type="text"
                ...
                <div className="flex flex-col md:flex-row items-end gap-6 mt-6 pt-6 border-t border-gray-100">
                  <div className="flex-1 grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest pl-1">Kode Wilayah (BPS)</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest pl-1">Kapasitas Maksimal</label>
                      <input
                        type="number"
                        min="0"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {form.processing ? <RefreshCw size={14} className="animate-spin" /> : (editingLocation ? <Save size={14} /> : <Plus size={14} />)}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-emerald-950/40 uppercase tracking-widest tabular-nums">
                    Total {locations.meta.total} Titik Terdata &middot; Halaman {locations.meta.current_page} dari {locations.meta.last_page}
                  </span>
                </div>
          ...
          >
            <PremiumTable
              headers={['Desa/Kelurahan', 'Wilayah Administratif', 'Kode BPS', 'Okupansi & Kapasitas', 'Opsi']}
              isEmpty={locations.data.length === 0}
              emptyText="Data lokasi administratif tidak ditemukan."
            >
              {locations.data.map((l) => (
                <PremiumTableRow key={l.id} className={clsx("group", editingLocation?.id === l.id && "bg-emerald-50/50")}>
                  <PremiumTableCell>
                    <div className="flex flex-col py-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[13px] font-black text-emerald-950 uppercase tracking-tight leading-none group-hover:text-emerald-700 transition-colors">{l.village_name}</span>
                        {selectedPeriodId && l.is_used_in_period && (
                          <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 rounded-md text-[8px] font-black border border-emerald-100 uppercase tracking-widest">AKTIF</span>
                        )}
                      </div>
                      <span className="text-[10px] font-bold text-emerald-800/40 mt-1 uppercase tracking-tighter truncate max-w-[200px]">{l.full_name || '-'}</span>
                    </div>
                  </PremiumTableCell>
                  <PremiumTableCell>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[10px] font-black text-emerald-900 uppercase tracking-tight">{l.district_name || '-'}</span>
                      <span className="text-[9px] font-bold text-emerald-800/40 uppercase tracking-widest">{l.regency_name || '-'}</span>
                    </div>
                  </PremiumTableCell>
                  <PremiumTableCell>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col gap-2 w-28">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-tighter tabular-nums leading-none">
                        <div className="flex items-center gap-1.5">
                           ...
                           <span className={clsx(l.groups_count >= (l.capacity || 0) && (l.capacity || 0) > 0 ? 'text-amber-600 font-black' : 'text-emerald-950')}>
                            {l.groups_count}
                          </span>
                        </div>
                      ...
                      <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden border border-gray-50 shadow-inner">
                        <div 
                          className={clsx("h-full transition-all rounded-full", ((l.capacity ?? 0) > 0 && l.groups_count >= (l.capacity as number)) ? 'bg-gradient-to-r from-amber-500 to-rose-500' : 'bg-gradient-to-r from-emerald-500 to-emerald-400')} 
                  ...
                  <PremiumTableCell align="right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEditForm(l)} className="h-9 px-3 bg-white border border-gray-100 text-emerald-900 rounded-xl hover:bg-emerald-50 hover:border-emerald-200 transition-all shadow-sm active:scale-95 flex items-center gap-2 text-[10px] font-black uppercase" title="Edit">
