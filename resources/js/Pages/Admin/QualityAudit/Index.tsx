    ...
    return (
        <AppLayout title="Reports Analytics Core">
            ...
            <div className="space-y-10 pb-32">
                ...
                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8 border-b border-slate-100 pb-10">
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black text-rose-600 uppercase tracking-[0.4em] italic text-glow">INTELLIGENCE_LAYER_v4.2</span>
                        </div>
                        <h1 className="text-3xl md:text-4xl font-black text-slate-950 tracking-tighter flex items-center gap-4 italic uppercase">
                            ...
                            ACTIVITY <span className="text-rose-600">AUDITOR</span>
                        </h1>
                        <p className="text-sm font-bold text-slate-400 italic uppercase">Heuristic Analysis of Student Daily Reports & Operational Integrity.</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="px-8 py-5 bg-slate-950 border border-slate-800 rounded-[2rem] flex items-center gap-8 shadow-2xl relative overflow-hidden group">
                           <div className="absolute inset-0 bg-gradient-to-r from-rose-500/10 to-transparent translate-x-full group-hover:translate-x-0 transition-transform duration-1000" />
                           <div className="relative z-10 flex flex-col">
                               <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1.5">Anomaly Detected</span>
                               <div className="flex items-center gap-3">
                                   <ShieldAlert className="w-5 h-5 text-rose-500" />
                                   <span className="text-2xl font-black text-white italic tracking-tighter leading-none">{stats.high_risk_count}</span>
                               </div>
                           </div>
                           <div className="h-10 w-px bg-slate-800 relative z-10" />
                           <div className="relative z-10 flex flex-col">
                               <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1.5">Engine Status</span>
                               <div className="flex items-center gap-3">
                                   <Activity className="w-5 h-5 text-emerald-500" />
                                   <span className="text-lg font-black text-white italic tracking-tighter leading-none uppercase">Optimized</span>
                               </div>
                           </div>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-[3.5rem] border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg transition-all relative group/table">
                    <div className="px-12 py-10 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                        <div className="flex items-center gap-5">
                            <div className="p-4 bg-rose-600 text-white rounded-2xl shadow-[0_10px_30px_-10px_rgba(225,29,72,0.5)]">
                                <ScanLine className="w-6 h-6" />
                            </div>
                            <div>
                                <h2 className="text-sm font-black uppercase tracking-[0.4em] italic text-slate-950">ACTIVITY_INTEGRITY_SCANNER</h2>
                                <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase italic tracking-widest">Live feed analysis • Sync: Realtime</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                             <div className="h-2 w-2 rounded-full bg-emerald-500" />
                             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Core_Linked</span>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-white border-b border-slate-100">
                                    <th className="px-12 py-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Identity & Target</th>
                                    <th className="px-12 py-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Telemetry Data</th>
                                    <th className="px-12 py-8 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Risk Vector</th>
                                    <th className="px-12 py-8 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Audit Score</th>
                                    <th className="px-12 py-8 text-right text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] italic pr-16">Oversight</th>
                                </tr>
                            </thead>
                ...
                {/* Tactical Analytics Monitor Bottom */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="p-12 bg-slate-950 rounded-[4rem] border border-slate-800 relative overflow-hidden group shadow-3xl">
                        <div className="absolute top-0 right-0 w-full h-full bg-[radial-gradient(circle_at_70%_20%,rgba(225,29,72,0.15),transparent_60%)]" />
                        <div className="relative z-10 flex items-center gap-10">
                            <div className="p-6 bg-rose-500 shadow-[0_0_50px_rgba(225,29,72,0.3)] rounded-[2.5rem] rotate-3">
                                <Zap className="h-10 w-10 text-white animate-pulse" />
                            </div>
                            <div>
                                <h4 className="text-xl font-black text-white italic tracking-[0.3em] uppercase">Engine_Status: Live</h4>
                                <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mt-2 italic leading-relaxed">
                                    Auditor melakukan pemindaian heuristik pada database setiap mutasi data terjadi. <br/>
                                    <span className="text-rose-500 underline underline-offset-8 decoration-rose-500/20 underline-glow">Protocol: High_Risk_Alert_Enabled</span>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div className="p-12 bg-white rounded-[4rem] border border-slate-200 flex flex-wrap items-center justify-around gap-12 group hover:border-rose-500 transition-all duration-700 shadow-sm hover:shadow-2xl">
                         <div className="flex flex-col items-center">
                             <Fingerprint className="h-10 w-10 text-slate-200 group-hover:text-rose-600 transition-colors" />
                             <span className="text-[8px] font-black text-slate-300 uppercase mt-4 tracking-widest italic group-hover:text-rose-600">Bio_Hash</span>
                         </div>
                         <div className="flex flex-col items-center">
                             <Database className="w-10 h-10 text-slate-200 group-hover:text-emerald-500 transition-colors" />
                             <span className="text-[8px] font-black text-slate-300 uppercase mt-4 tracking-widest italic group-hover:text-emerald-500">Live_Grid</span>
                         </div>
                         <div className="flex flex-col items-center">
                             <ShieldCheck className="w-10 h-10 text-slate-200 group-hover:text-amber-500 transition-colors" />
                             <span className="text-[8px] font-black text-slate-300 uppercase mt-4 tracking-widest italic group-hover:text-amber-500">Sec_Core</span>
                         </div>
                    </div>
                </div>
