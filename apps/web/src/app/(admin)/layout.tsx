...
const getNavGroups = (pathname: string, roles: string[]) => {
  ...
  const operationalGroups = [
    ...
    { title: 'MANAJEMEN MAHASISWA', items: [
      ...
      { label: 'Kampus Eksternal', href: '/admin/external-universities', icon: Building2 },
...
export default function AdminLayout({ children }: { children: React.ReactNode }): React.JSX.Element {
  ...
  const portalHint = isSystem
    ...
    : isBlog
      ? 'Konten publik, pengumuman, profil, dan unduhan.'
      : 'Operasional harian KKN: peserta, pendaftaran, kelompok, laporan, nilai.';
      ...
      <aside className={clsx(
        ...
        <div className="h-28 px-6 flex flex-col justify-center sticky top-0 z-10 bg-[color:var(--profile-surface-strong)]">
          ...
          <div className="mt-4">
            <h1 className="text-base font-black leading-none tracking-tight flex items-center gap-2 font-display uppercase text-[color:var(--profile-text)]">
              {isBlog ? 'PORTAL KONTEN' : isSystem ? 'PORTAL SISTEM' : 'PORTAL KKN'}
            ...
            <p className="text-[9px] font-bold text-[color:var(--profile-muted)] mt-2 font-sans tracking-wider leading-relaxed uppercase">
              {isBlog ? 'Konten Publik' : isSystem ? 'Superadmin Sistem' : 'Admin Operasional'}
            </p>
          </div>
        </div>
        ...
        {!isSuperadmin && (
          <div className="px-4 py-2">
            <Link href="/profil" prefetch={false} onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-2xl bg-[color:var(--profile-soft)] border border-[color:var(--profile-border)] p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              ...
              <div className="min-w-0">
                <p className="text-[9px] font-black text-[color:var(--profile-soft-text)] uppercase tracking-[0.15em]">{roles[0] || 'admin'}</p>
          ...
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 hidden truncate text-[10px] font-bold uppercase tracking-[0.18em] text-[color:var(--profile-muted)] sm:block">
                {portalLabel} · {pageContext}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden max-w-[320px] rounded-xl border border-[color:var(--profile-border)] bg-[color:var(--profile-soft)] px-3 py-2 text-[color:var(--profile-soft-text)] shadow-sm xl:block">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[color:var(--profile-text)]">{portalLabel}</p>
              <p className="mt-1 truncate text-[9px] font-semibold text-[color:var(--profile-muted)]">{portalHint}</p>
            </div>
            <NotificationBell />
