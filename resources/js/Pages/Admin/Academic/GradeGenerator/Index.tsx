...
export default function GradeGeneratorIndex({ periods, groups }: Props) {
 ...
 <div className="h-14 px-8 bg-white border border-gray-200 rounded-xl flex items-center gap-5 shadow-sm">
 <Archive size={20} className="text-[#1a7a4a]"/>
 <div className="flex flex-col">
 <span className="text-xs font-bold text-gray-700 leading-none mb-1.5">Unit Terdaftar</span>
 <span className="text-lg font-semibold text-gray-900 tabular-nums leading-none">{activeGroups.length} KELOMPOK</span>
 </div>
 </div>
 <a 
 ...
 <div className="p-5 bg-gray-50 border-b-2 border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-6">
 ...
 className="w-full h-12 pl-12 pr-10 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-900 outline-none focus:border-[#1a7a4a] transition-all appearance-none cursor-pointer shadow-sm"
 >
 <option value="">TAMPILKAN SEMUA PERIODE</option>
 {periods.map(p => <option key={p.id} value={p.id}>{p.name.toUpperCase()}</option>)}
 </select>
 <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-600 pointer-events-none"size={16} />
 </div>
 </div>

 <div className="overflow-x-auto min-h-[400px]">
 <table className="w-full text-left">
 <thead className="bg-gray-50 border-b-2 border-gray-200 text-gray-900">
 <tr>
 ...
 <th className="px-8 py-5 text-xs font-bold text-center border-l border-gray-200">Dosen Pembimbing</th>
 ...
 <span className="text-base font-semibold text-gray-900 group-hover:text-gray-700 transition-colors leading-none font-mono">{group.code}</span>
 <span className="text-xs font-bold text-[#1a7a4a]">{group.name}</span>
 </div>
 </td>
 <td className="px-8 py-5">
 <div className="flex items-center gap-4">
 <div className="h-10 w-10 bg-gray-50 border border-gray-200 text-[#1a7a4a] rounded-xl flex items-center justify-center shadow-sm shrink-0"><MapPin size={16} strokeWidth={2.5} /></div>
 <div className="flex flex-col">
 ...
 <td className="px-8 py-5 text-center border-l border-gray-200/30">
 <div className="inline-flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-xl border border-gray-200/60 shadow-sm">
 <User size={12} className="text-[#1a7a4a]"/>
 <span className="text-xs font-bold text-gray-900 tracking-wide truncate max-w-[180px]">{group.dpl || 'DPL BELUM DITENTUKAN'}</span>
 </div>
 </td>
 ...
 <a href={`/admin/generator-nilai/${group.id}/export`} className="h-9 w-9 bg-white border border-gray-200 text-[#1a7a4a] hover:bg-gray-50 rounded-xl flex items-center justify-center transition-all shadow-sm active:scale-90"title="Ekspor Excel"><FileSpreadsheet size={16} /></a>
 ...
 <span className="text-sm font-bold text-gray-700 tracking-wide">Tidak Ada Kelompok</span>
 ...
 <div className="px-8 py-5 border-t-2 border-gray-200 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
 ...
 <div className="h-8 px-5 flex items-center bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-900 shadow-sm">Total: {activeGroups.length} Unit Kelompok</div>
 </div>
 </section>
 ...
 <div className="h-24 w-24 bg-gray-100 border border-emerald-800 text-gray-700 rounded-xl flex items-center justify-center shrink-0 shadow-inner"><Download size={48} strokeWidth={2} /></div>
 <div className="space-y-3">
 <h1 className="text-2xl font-bold text-white leading-none">Manajemen Dokumen Penilaian Terpusat</h1>
 <p className="text-sm font-semibold text-gray-700 leading-relaxed max-w-2xl">
 ...
 <ShieldCheck className="text-gray-700"size={18} />
 <span className="text-xs font-semibold text-gray-700">Otoritas Dokumen KKN</span>
 </div>
 </div>
 </div>
 </div>
...
function GeneratorMetric({ label, value, icon: Icon }: { label: string, value: string | number, icon: any }) {
 ...
 <div className="h-12 w-12 bg-gray-50 border border-gray-200 text-[#1a7a4a] rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-6 transition-all shadow-sm">
 <Icon size={20} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col z-20">
 <span className="text-xs font-bold text-gray-700 leading-none mb-2">{label}</span>
 <span className="text-xl font-semibold text-gray-900 tabular-nums leading-none group-hover:text-gray-700 transition-colors">{value}</span>
 </div>
 </div>
 );
}
