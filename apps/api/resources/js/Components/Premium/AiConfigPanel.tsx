...
export default function AiConfigPanel({ settings }: AiConfigPanelProps) {
  ...
  const [aiEnabled, setAiEnabled] = useState(
    enabledSetting?.value === '1' || enabledSetting?.value === 'true',
  );
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setValidationMessage(
        error.response?.data?.message || 'Gagal menghubungi server. Periksa koneksi jaringan Anda.',
      );
    }
  };
  ...
  const handleSave = () => {
    if (validationStatus === 'error') {
      if (
        !window.confirm(
          'API Key ini gagal divalidasi. Menyimpan key ini mungkin mematikan fungsi AI. Tetap lanjutkan?',
        )
      ) {
        return;
      }
    ...
    setIsSaving(true);
    router.patch(
      route('admin.pengaturan.sistem.ai.update'),
      {
    ...
    <div className="space-y-10">
      ...
      <div className="flex items-center justify-between bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100 shadow-inner">
        <div className="space-y-1">
          <h4 className="text-[10px] font-black text-emerald-950 uppercase tracking-[0.2em]">
            Aktifkan Bantuan AI
          </h4>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">
            Nyalakan ini agar sistem dapat memberikan rekomendasi otomatis.
          </p>
        </div>
        <button
          type="button"
      ...
      {!aiEnabled && (
        ...
        >
          ...
          <p className="text-[10px] font-black text-amber-800 uppercase tracking-tight leading-relaxed">
            Fitur AI sedang dimatikan. Mahasiswa dan DPL tidak akan menerima panduan otomatis dari
            sistem.
          </p>
        </motion.div>
      )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <label className="text-[10px] font-black text-emerald-950 uppercase tracking-[0.2em]">
              ...
              className={clsx(
                ...
                validationStatus === 'error'
                  ...
                  : validationStatus === 'success'
                    ? 'border-emerald-100 bg-emerald-50 text-emerald-900 focus:border-emerald-600'
        ...
        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed mt-2 pl-1">
          Kunci rahasia Anda akan disandikan dengan aman secara asimetris sebelum disimpan ke basis
          data.
        </p>
      </div>
        ...
        {apiKeySetting?.value ? (
          <button
            ...
            onClick={() => {
              if (
                window.confirm(
                  'Hapus Kunci Akses secara permanen? Fitur AI tidak akan bekerja sampai Anda memasukkan kunci baru.',
                )
              ) {
                setIsSaving(true);
