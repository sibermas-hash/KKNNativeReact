...
export default function MahasiswaIndex({ students, filters, faculties, programs, batchYears, stats, syncInfo }: Props) {
 ...
 <h1 className="text-2xl font-bold text-gray-900 leading-tight">Direktori Mahasiswa</h1>
 <p className="text-sm text-gray-700 max-w-2xl mt-1">Pengelolaan data kepesertaan dan sinkronisasi registrasi mahasiswa KKN.</p>
 </div>
 <div className="flex items-center gap-3 shrink-0">
 <div className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg shadow-sm text-xs text-gray-700">
 ...
 className="w-full h-10 pl-9 pr-4 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 focus:border-[#1a7a4a] focus:ring-[#1a7a4a] shadow-sm"
 ...
 className={clsx("h-10 px-4 rounded-lg text-sm font-medium flex items-center gap-2 border shadow-sm transition-colors", showFilters ?"bg-[#16a34a] text-white border-emerald-600":"bg-white border-gray-300 text-gray-700 hover:bg-gray-50")}
 >
 <Filter size={15} />
 ...
 <button onClick={resetFilters} className="text-sm text-gray-700 hover:text-rose-600 transition-colors font-medium">Reset Semua Filter</button>
 </div>
 </div>
 )}
 ...
 <tr>
 <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700">Data Mahasiswa</th>
 <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700">Afiliasi Studi</th>
 <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700">Performa Akademik</th>
 <th className="px-6 py-3 text-center text-xs font-semibold text-gray-700">Status Akun</th>
 <th className="px-6 py-3 text-right text-xs font-semibold text-gray-700">Aksi</th>
 </tr>
 </thead>
 <tbody className="bg-white divide-y divide-gray-200">
 {!students?.data ? (
 <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-gray-700">Memuat data...</td></tr>
 ) : students.data.length === 0 ? (
 <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-gray-700">Data mahasiswa tidak ditemukan.</td></tr>
 ) : students.data.map(s => (
 <tr key={s.id} className="hover:bg-gray-50 transition-colors">
 <td className="px-6 py-4">
 <div className="flex flex-col">
 <span className="text-sm font-semibold text-gray-900">{s.nama}</span>
 <span className="text-xs text-gray-700 mt-0.5">NIM: {s.nim}{s.account && ` · @${s.account.username}`}</span>
 </div>
 </td>
 <td className="px-6 py-4">
 <div className="flex flex-col">
 <span className="text-sm text-[#1f2937] truncate max-w-xs">{s.prodi?.nama || '—'}</span>
 <span className="text-xs text-gray-700 mt-0.5">{s.fakultas?.nama || '—'}</span>
 </div>
 </td>
 <td className="px-6 py-4">
 ...
 <span className={clsx("text-xs font-medium px-2 py-0.5 rounded w-fit", s.is_bta_ppi_passed ?"bg-[#e8f5ee] text-gray-700":"bg-rose-100 text-rose-800")}>
 ...
 {!s.account ? (
 <span className="text-xs text-gray-600">Tanpa Akun</span>
 ) : (
 <span className={clsx("inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold", s.account.is_active ?"bg-[#e8f5ee] text-gray-700":"bg-rose-100 text-rose-800")}>
 ...
 <button onClick={() => s.account && setConfirmReset(s)} disabled={!s.account} className="p-1.5 text-gray-600 hover:text-[#1a7a4a] hover:bg-gray-50 rounded-md transition-colors disabled:opacity-30"title="Reset Password">
 <KeyRound size={16} />
 </button>
 <button onClick={() => s.account && setConfirmToggle(s)} disabled={!s.account} className="p-1.5 text-gray-600 hover:text-amber-600 hover:bg-amber-50 rounded-md transition-colors disabled:opacity-30"title={s.account?.is_active ? 'Nonaktifkan' : 'Aktifkan'}>
 ...
 <span className="text-xs text-gray-700">Hal. {students.meta.current_page} — Total <strong>{students.meta.total}</strong> data</span>
...
function StatCard({ label, value, icon: Icon, color = 'slate' }: { label: string; value: number; icon: LucideIcon; color?: 'emerald' | 'slate' }) {
 ...
 <div className={clsx("h-9 w-9 rounded-lg flex items-center justify-center shrink-0", color === 'emerald' ? 'bg-gray-50 text-[#1a7a4a]' : 'bg-gray-100 text-gray-700')}>
...
function FilterGroup({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: Array<{ value: string; label: string }>; }) {
 ...
 <select value={value} onChange={e => onChange(e.target.value)} className="w-full h-10 pl-3 pr-8 rounded-lg border border-gray-300 bg-white text-sm text-gray-700 focus:border-[#1a7a4a] focus:ring-[#1a7a4a] appearance-none shadow-sm">
