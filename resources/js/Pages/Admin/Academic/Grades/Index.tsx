            ...
            {data.student_id && (
              <div className="flex items-center gap-6 pl-10 border-l-2 border-emerald-50/50">
                <div className="text-center">
                  <p className="text-[10px] font-black text-emerald-800 leading-none mb-2 uppercase tracking-widest font-display">
                    Skor Keseluruhan
                  </p>
                  <p className="text-4xl font-black text-emerald-950 leading-none font-display tabular-nums">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 p-8 bg-slate-50/30 border border-emerald-50 rounded-3xl shadow-sm">
          <div className="space-y-5">
            <label className="text-[10px] font-black text-emerald-900 flex items-center gap-4 uppercase tracking-[0.2em] font-display">
              <span className="h-7 w-7 bg-white text-emerald-700 rounded-xl flex items-center justify-center text-xs border-2 border-emerald-50 shadow-sm">
                01
              </span>
              LOKASI & KELOMPOK
            </label>
            ...
            <div className="py-24 flex flex-col items-center justify-center text-center space-y-6 bg-gray-50 rounded-xl border border-dashed border-gray-300">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs font-bold text-emerald-800">
                  Pilih Mahasiswa peserta KKN untuk memulai input nilai
                </p>
              </div>
            </div>
          ) : (
        ...
        <div className="mt-20 pt-10 border-t-2 border-emerald-50 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs font-semibold text-emerald-950">
              Protokol Kepatuhan Aktif • Sesuai Panduan Penilaian
            </p>
          </div>
          <p className="text-xs font-semibold text-emerald-800">
            Sistem Informasi <span className="text-sky-600">SIBER</span>
...
function Metric({ label, value, icon: Icon }: { label: string; value: string; icon: any }) {
  return (
    <div className="flex items-center gap-4">
      ...
      <div>
        <p className="text-[10px] font-black text-emerald-800 leading-none mb-1.5 uppercase tracking-widest font-display">
          {label}
        </p>
        <p className="text-xs font-black text-emerald-950 leading-none uppercase font-display">
          {value}
        </p>
      </div>
    </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-l-8 border-emerald-600 pl-6 py-4 bg-slate-50/50 rounded-r-3xl border border-y-emerald-50/50 border-r-emerald-50/50 shadow-sm">
        <div>
          ...
          <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest font-display">
            {subtitle}
          </p>
        </div>
        <div className="flex items-center gap-4 pr-6">
          <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest font-display">
            Bobot Total
          </span>
        ...
        {fields.map((field) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col gap-4 p-5 rounded-xl border transition-all duration-300 relative overflow-hidden ${
                isError
                  ...
                  : isFilled
                    ? 'border-[#0d9488] bg-white shadow-sm shadow-none/50'
            ...
            >
              ...
              {isError && <div className="absolute top-0 left-0 right-0 h-1.5 bg-rose-500" />}

              <div className="flex items-center justify-between z-10 pt-1">
                <label
                  className={`text-xs font-semibold flex items-center gap-2 ${isFilled ? 'text-emerald-950' : 'text-emerald-800'}`}
                >
                  <field.icon
                    size={14}
                    strokeWidth={isFilled ? 3 : 2}
                    className={isFilled && !isError ? 'text-[#0d9488]' : ''}
                  />
                  {field.label}
                </label>
                {isFilled && !isError && (
                ...
                {isError && (
                  <AlertCircle size={16} className="text-rose-600 animate-bounce" strokeWidth={3} />
                )}
              </div>
              ...
              <div className="relative z-10 mt-1">
                <input
                  ...
                  className={`w-full h-16 bg-white border-2 border-b-[6px] rounded-2xl flex items-center justify-center font-black text-4xl text-center focus:outline-none transition-all tabular-nums shadow-sm font-display ${
                    isError
                      ? 'border-rose-200 border-b-rose-500 text-rose-700 bg-rose-50 focus:border-rose-400 focus:border-b-rose-600'
                      : isFilled
                        ? 'border-emerald-100 border-b-emerald-600 text-emerald-950 focus:border-emerald-400 focus:border-b-emerald-700'
              ...
              {fieldError(field.id) || isError ? (
                ...
                <p
                  className={`text-xs font-semibold text-center mt-1 z-10 ${isFilled ? 'text-emerald-800' : 'text-emerald-800'}`}
                >
                  MASUKKAN SKOR
                </p>
              )}
