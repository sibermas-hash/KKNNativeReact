...
import type { PageProps, Student, DashboardStatProps, DashboardQuickLinkProps, ColorPalette } from '@/types';
...
export default function StudentDashboard({
    ...
    const phases = [
        { id: 1, label: 'Pendaftaran', desc: 'Registrasi Unit', isCompleted: isApproved, isActive: !registration || isPending },
        { id: 2, label: 'Persiapan', desc: 'Program Kerja', isCompleted: workProgramCount > 0, isActive: isApproved && workProgramCount === 0 },
        { id: 3, label: 'Pelaksanaan', desc: 'Laporan Harian', isCompleted: dailyReportCount >= (registration?.period?.min_logbook ?? 30), isActive: workProgramCount > 0 && dailyReportCount < (registration?.period?.min_logbook ?? 30) },
        { id: 4, label: 'Pelaporan', desc: 'Laporan Akhir', isCompleted: !!finalReport, isActive: dailyReportCount >= (registration?.period?.min_logbook ?? 30) && !finalReport },
        { id: 5, label: 'Penilaian', desc: 'Sertifikasi', isCompleted: grade?.is_finalized, isActive: !!finalReport && !grade?.is_finalized },
    ];

    const progressPercent = Math.floor((phases.filter((p) => p.isCompleted).length / phases.length) * 100);
            ...
            <AppLayout title="Portal Mahasiswa">
                ...
                <div className="space-y-4">
                    ...
                    <motion.div 
                        ...
                        className="bg-white rounded-xl border border-slate-100 p-5 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4"
                    >
                    ...
                    {!isGroupPinned ? (
                        <div className="bg-white rounded-xl border border-dashed border-slate-300 p-10 text-center space-y-4">
                            <div className="h-12 w-12 bg-slate-50 rounded-full flex items-center justify-center mx-auto text-slate-300 border border-slate-200">
                                <MapPin size={24} />
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            <div className="lg:col-span-2 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <StatBox icon={ClipboardList} label="Logbook Harian" value={`${dailyReportCount} Laporan`} color="emerald" />
                                    <StatBox icon={ScrollText} label="Laporan Akhir" value={finalReport ? 'Terkirim' : 'Belum Unggah'} color="blue" />
                                </div>

                                {/* Grade Alert */}
                                {grade?.is_finalized && (
                                    <div className="bg-white border border-emerald-100 rounded-xl p-6 shadow-sm flex items-center justify-between gap-4">
                                        <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-4">
                                <div className="bg-white rounded-xl border border-slate-100 p-5 shadow-sm">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <QuickLink href={route('student.laporan-harian.index')} icon={ClipboardList} label="Logbook" />
                                        <QuickLink href={route('student.program-kerja.index')} icon={Presentation} label="Proker" />
                                        <QuickLink href={route('student.posko.index')} icon={MapPin} label="Posko" />
                                        <QuickLink href={route('student.laporan-akhir.index')} icon={ScrollText} label="Laporan" />
                                    </div>
                                </div>
...
function StatBox({ icon: Icon, label, value, color = 'emerald' }: DashboardStatProps) {
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm flex items-center gap-4">
            <div className={clsx('h-10 w-10 rounded-lg flex items-center justify-center border', colors[color])}>
                <Icon size={18} />
            </div>
            <div>
...
function QuickLink({ href, icon: Icon, label }: DashboardQuickLinkProps) {
    return (
        ...
        >
            ...
            {label}
            <ArrowRight size={12} className="ml-auto text-slate-200 group-hover:text-emerald-500 transition-all group-hover:translate-x-1" />
        </Link>
    );
}
