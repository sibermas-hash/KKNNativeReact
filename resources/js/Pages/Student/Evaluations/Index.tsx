    ...
    <AppLayout title="Hasil Evaluasi">
      ...
      <div className="space-y-6">
        <section className="rounded-xl border border-emerald-50 bg-white p-8">
          <h1 className="text-2xl font-bold text-emerald-950 uppercase tracking-tight italic">
            Hasil Evaluasi
          </h1>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {evaluations.map((evaluation) => {
              ...
              return (
                <section
                  key={evaluation.id}
                  className="rounded-xl border border-emerald-50 bg-white p-6 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-bold text-emerald-950 tabular-nums italic">
                  ...
                  <div className="mt-6 space-y-3">
                    {items.length > 0 ? (
                      items.map((item, index) => (
                        ...
                        >
                          <span className="text-sm font-bold text-emerald-950 uppercase italic">
                            {item.criterion || '-'}
                          </span>
                  ...
                  {evaluation.notes && (
                    <div className="mt-6 rounded-xl border border-emerald-100 bg-emerald-50/50 p-4">
                      <h3 className="text-sm font-bold text-emerald-950 uppercase italic">
                        Catatan penilai
                      </h3>
                      <p className="mt-2 text-sm font-bold text-emerald-950 leading-relaxed italic">
                        {evaluation.notes}
                      </p>
                    </div>
                  )}
