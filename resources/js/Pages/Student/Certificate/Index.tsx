...
export default function CertificateIndex({ eligible, checks, score, laporan_akhir_status, certificate_url }: Props) {
        ...
        <AppLayout title="Sertifikat KKN">
            ...
            <div className="mx-auto max-w-6xl space-y-12 pb-20">
                ...
                <motion.div 
                    ...
                    className="relative overflow-hidden rounded-[3rem] bg-slate-900 p-12 lg:p-16 text-white shadow-2xl shadow-slate-950/20"
                >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-12">
                        <div className="space-y-8 max-w-xl">
                            <div className="flex items-center gap-4">
                                <div className="h-10 w-10 rounded-xl bg-emerald-500 flex items-center justify-center text-slate-900">
                                    <Award size={20} strokeWidth={3} />
                                </div>
                                <div className="h-px w-12 bg-emerald-500/30" />
                                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-500">Official Accreditation</span>
                            </div>
                            <div className="space-y-4">
                                ...
                                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest leading-relaxed opacity-80">
                                    Dokumen resmi tanda kelulusan dan kontribusi nyata Anda dalam program KKN UIN Saizu.
                                </p>
                            </div>
                        </div>
                ...
                <div className="grid gap-12 lg:grid-cols-5">
                    ...
                    <div className="lg:col-span-3 space-y-10">
                        ...
                        >
                            <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.5em] mb-12 flex items-center gap-3">
                            ...
                            <div className="space-y-12">
                                <CheckItem 
                                    ...
                                    status={checks.report_approved ? 'success' : (laporan_akhir_status === 'pending' ? 'pending' : 'failed')} 
                        ...
                        {!eligible && (
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <p className="text-xs font-black text-amber-900 uppercase tracking-widest">Protocol Warning</p>
                    ...
                    <div className="lg:col-span-2">
                        <div className="sticky top-10 rounded-[3rem] bg-white border border-slate-100 overflow-hidden shadow-2xl shadow-slate-200/50">
                            <div className="p-12 text-center space-y-10">
                                <h3 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em]">System Outcome</h3>
                                ...
                                {eligible ? (
                                    <div className="space-y-10">
                                        <div className="relative mx-auto h-32 w-32 flex items-center justify-center">
                                            ...
                                            <div className="h-28 w-28 items-center justify-center rounded-full bg-emerald-500 text-slate-900 ring-8 ring-emerald-500/5 z-10 flex">
                                                <Verified size={48} strokeWidth={2.5} />
                                            </div>
                                        </div>
                                        <div className="space-y-2">
                                            <p className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Approved.</p>
                                            <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em]">Access Granted for Download</p>
                                        </div>
                                        <a
                                            href={certificate_url ?? '#'}
                                            className="inline-flex w-full h-20 items-center justify-center gap-4 rounded-[2rem] bg-slate-900 text-white hover:bg-emerald-600 transition-all duration-500 shadow-2xl shadow-slate-900/20 active:scale-95 group/dl"
                                        >
                                            <Download className="h-6 w-6 group-hover:-translate-y-1 transition-transform" />
                                            <span className="text-xs font-black uppercase tracking-widest">Generate Certificate</span>
                                        </a>
                                    </div>
                                ) : (
                                    <div className="space-y-10">
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-3xl font-black text-slate-300 uppercase tracking-tighter">Pending.</p>
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Awaiting Authorization</p>
                                        </div>
                            ...
                            {eligible && score && (
                                <div className="bg-slate-900 px-12 py-10 space-y-6">
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Score Matrix</span>
                                        <span className="text-2xl font-black text-white tracking-tighter">{score.total_score}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Final Grade</span>
                                        <div className="px-5 py-2 bg-emerald-500 rounded-[1rem] shadow-lg shadow-emerald-500/20">
                                            <span className="text-sm font-black text-slate-900">{score.letter_grade}</span>
                                        </div>
                                    </div>
                                </div>
                            )}
...
function CheckItem({ label, status, desc, variants }: { label: string, status: 'success' | 'failed' | 'pending', desc: string, variants: any }) {
    return (
        <motion.div variants={variants} className="flex gap-10 group">
            <div className="shrink-0 mt-1">
                {status === 'success' ? (
                    <div className="h-10 w-10 rounded-2xl bg-emerald-500 flex items-center justify-center shadow-xl shadow-emerald-500/20">
                        <ShieldCheck className="h-6 w-6 text-slate-900" strokeWidth={3} />
                    </div>
                ) : status === 'failed' ? (
            ...
            <div className="space-y-2">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed opacity-60 group-hover:opacity-100 transition-opacity">
                    {desc}
                </p>
            </div>
        </motion.div>
