...
function openDatabase(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
        ...
        request.onerror = () => reject(request.error ?? new Error('Database offline tidak dapat dibuka.'));
...
export async function queueDailyReport(payload: PendingDailyReportPayload): Promise<PendingDailyReportRecord> {
    const record: PendingDailyReportRecord = {
        local_id: typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : String(Date.now()),
