...
export default function ExternalParticipantsPage(){
 ...
 const {data:periods=[]}=useQuery({queryKey:['external','periods'],queryFn:async()=>((await api.get('/admin/periode',{params:{per_page:100}})) as {data?:Period[]|{data?:Period[]}})?.data instanceof Array ? ((await api.get('/admin/periode',{params:{per_page:100}})) as {data?:Period[]}).data ?? [] : (((await api.get('/admin/periode',{params:{per_page:100}})) as {data?:{data?:Period[]}})?.data?.data ?? [])});
