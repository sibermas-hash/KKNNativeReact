        ...
        >
          ...
          >
            {reports.data.map((r) => (
              <PremiumTableRow
                key={r.id}
                className={clsx('group', r.risk_level === 'HIGH' ? 'bg-rose-50/10' : '')}
              >
                <PremiumTableCell>
                  <div className="flex items-center gap-4 py-1">
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-[11px] font-black text-emerald-950 uppercase tracking-tight leading-none group-hover:text-emerald-700 transition-colors">
                        {r.user_name}
                      </span>
                ...
                <PremiumTableCell>
                  <div className="flex flex-col gap-1">
                    <span className="text-[12px] font-bold text-emerald-950 line-clamp-1 leading-none mb-1">
                      {r.title}
                    </span>
                ...
                <PremiumTableCell>
                  <div className="flex flex-wrap gap-2">
                    {r.risk_flags.length > 0 ? (
                      r.risk_flags.map((flag, idx) => (
                        <span
                          key={idx}
                          className="h-6 px-3 bg-rose-50 border border-rose-100 text-rose-600 text-[9px] font-black uppercase tracking-widest rounded-lg flex items-center shadow-sm"
                        >
                          {flag.replace(/_/g, ' ')}
                        </span>
                      ))
                ...
                <PremiumTableCell align="center">
                  <div
                    className={clsx(
                      ...
                      r.risk_score >= 70
                        ? 'bg-rose-600 text-white border-rose-500 shadow-rose-200 rotate-6'
                        : r.risk_score >= 30
                          ? 'bg-amber-50 border-amber-200 text-amber-600 border-amber-200 shadow-amber-50'
                ...
                <PremiumTableCell align="right">
                  <button className="h-9 px-4 bg-white border border-gray-100 text-emerald-900 rounded-xl hover:bg-emerald-50 hover:border-emerald-200 transition-all shadow-sm active:scale-95 flex items-center gap-2 text-[10px] font-black uppercase">
                    <Eye size={14} /> Audit
                  </button>
                </PremiumTableCell>
              </PremiumTableRow>
