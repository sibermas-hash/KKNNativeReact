...
export default function AdminRekapitulasiIndex({ kelompok, rekapitulasi, dpl }: Props) {
 ...
 <h1 className="text-base font-bold tracking-tight leading-none">Activity Recap & Fiscal Audit</h1>
 ...
 <Link href={route('admin.kelompok.index')} className="h-14 px-8 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold shadow-xl shadow-emerald-100 flex items-center gap-3 text-sm transition-all active:scale-95 disabled:opacity-20 tracking-wider"><ArrowLeft size={14} /> Back</Link>
 <div className="h-10 w-[1px] bg-slate-200 mx-2" />
 <button onClick={() => window.print()} className="h-10 px-4 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 rounded-2xl flex items-center gap-2 text-sm font-bold font-semibold text-xs text-emerald-950 hover:border-emerald-500 hover:text-emerald-600 transition-all shadow-xl shadow-emerald-900/5 transition-all"><Printer size={14} /> Print_Node</button>
 ...
 {/* --- DOCUMENT CANVAS --- */}
 <section className="bg-white border border-emerald-100/60 rounded-xl overflow-hidden shadow-xl print:shadow-none print:border-0 print:m-0 print:p-0">
 ...
 {/* Info Registry */}
 <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 p-6 bg-emerald-50/30 border border-emerald-100/60 rounded-xl print:grid-cols-2 print:p-2 print:bg-white print:border-0 print:gap-y-1">
 ...
 <thead>
 <tr className="bg-emerald-50/60 text-black border-b border-slate-400">
 ...
 <th rowSpan={2} className="border border-slate-400 p-2 text-right font-bold w-24 bg-emerald-600 text-white">Total (Rp)</th>
 <th rowSpan={2} className="border border-slate-400 p-2 text-left font-bold w-32">Note</th>
 </tr>
 <tr className="bg-emerald-50/30 text-sm border-b border-slate-400">
 ...
 <tr key={item.id} className="hover:bg-emerald-50/30/50 transition-all font-medium text-black">
 ...
 <td className="border border-slate-400 p-2 text-right tabular-nums font-bold bg-emerald-50/30">{formatCurrency(item.jumlah)}</td>
...
function RekapMetric({ label, value, icon: Icon }: { label: string, value: string | number, icon: LucideIcon }) {
 ...
 <span className="text-lg font-bold text-black tracking-tight tabular-nums leading-none group-hover:text-emerald-600 transition-colors">{value}</span>
 </div>
 </div>
 );
}
...
function InfoItem({ label, value, icon: Icon }: { label: string, value: string, icon?:LucideIcon }) {
 ...
 <div className="w-24 text-sm font-bold text-emerald-950 tracking-wider text-xs font-semibold shrink-0">{label}</div>
