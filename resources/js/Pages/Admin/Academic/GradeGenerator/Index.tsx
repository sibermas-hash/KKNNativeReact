...
export default function GradeGeneratorIndex({ periods, groups }: Props) {
 ...
 Manajemen Ekstraksi Blanko Penilaian dan Manajemen Berkas Lapangan Terpadu Mahasiswa KKN
 </p>
 </div>
 ...
 <span className="text-sm font-bold text-emerald-950 font-semibold text-xs leading-none mb-1">Unit Aktif</span>
 <span className="text-sm font-bold text-black tabular-nums leading-none tracking-tight">{activeGroups.length} KELOMPOK</span>
 </div>
 </div>
 <a 
 href={selectedPeriodId ? `/admin/generator-nilai/export-zip?period_id=${selectedPeriodId}` : '/admin/generator-nilai/export-zip'} 
 className="h-14 px-6 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-bold transition-all shadow-xl shadow-emerald-100 flex items-center gap-3 active:scale-95 text-sm tracking-wider"
 >
 <Download size={18} />
 UNDUH SEMUA (ZIP)
 </a>
 ...
 <span className="text-sm font-bold text-black font-semibold text-xs">Grading Logic Manifest</span>
 </div>
 ...
 <select value={selectedPeriodId} onChange={(e) => setSelectedPeriodId(e.target.value)} className="w-full h-8 pl-8 pr-4 bg-emerald-50/30 border border-emerald-100/60 rounded-lg text-sm font-bold outline-none focus:bg-white transition-all appearance-none cursor-pointer">
 ...
 <tr key={group.id} className="group hover:bg-emerald-50/30/50 transition-all">
 <td className="px-6 py-4">
 <div className="flex flex-col gap-0.5">
 <span className="text-[14px] font-bold text-black group-hover:text-emerald-700 transition-colors leading-tight font-mono">{group.code}</span>
 <span className="text-sm font-bold text-emerald-950 font-semibold text-xs opacity-60">{group.name}</span>
 </div>
 </td>
 <td className="px-6 py-4">
 <div className="flex items-center gap-3">
 ...
 <div className="flex flex-col">
 <span className="text-sm font-bold text-emerald-700 leading-tight">{group.desa}</span>
 <span className="text-sm font-bold text-slate-300 font-semibold text-xs">{group.kecamatan}, {group.kabupaten}</span>
 </div>
 </div>
 </td>
 <td className="px-6 py-4 text-center">
 <div className="inline-flex items-center gap-2 bg-emerald-50/30 px-3 py-1 rounded-full border border-emerald-100/60">
 <User size={10} className="text-slate-300" />
 <span className="text-sm font-bold text-emerald-950 font-semibold text-xs truncate max-w-[150px]">{group.dpl || 'NULL_OVERSEER'}</span>
 </div>
 </td>
 ...
 <Link href={`/admin/grades?group_id=${group.id}`} className="h-8 px-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg flex items-center gap-2 text-sm font-bold font-semibold text-xs shadow-lg shadow-emerald-100 active:scale-95 group/btn">
 Grade_Auth <ChevronRight size={12} className="text-white group-hover/btn:translate-x-0.5 transition-transform" />
 </Link>
 <a href={`/admin/generator-nilai/${group.id}/export`} className="h-8 w-8 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 text-emerald-500 hover:bg-emerald-50 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-emerald-900/5 transition-all active:scale-90" title="Export XLSX"><FileSpreadsheet size={14} /></a>
 <a href={`/admin/generator-nilai/${group.id}/export-pdf`} className="h-8 w-8 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 text-rose-500 hover:bg-rose-50 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-emerald-900/5 transition-all active:scale-90" title="Export PDF"><FileText size={14} /></a>
 </div>
 </td>
 </tr>
 ))}
 {activeGroups.length === 0 && (
 <tr><td colSpan={4} className="py-20 text-center text-sm font-bold text-slate-300 tracking-normal">Logic buffer null. Select period node.</td></tr>
 )}
 </tbody>
 </table>
 </div>
 ...
 <span className="text-sm font-bold text-emerald-950 font-semibold text-xs leading-none">Transmission Nominal • Binary Sync Stable.</span>
 <div className="text-sm font-bold text-black ">Manifest Count: {activeGroups.length} Units</div>
 </div>
 </section>
 ...
 <div className="h-12 w-24 bg-white/20 rounded-xl flex items-center justify-center shrink-0 border border-white/20 shadow-sm backdrop-blur-md text-white"><Download size={48} strokeWidth={1.5} /></div>
 <div className="space-y-3">
 <h1 className="text-2xl font-bold tracking-tight text-white leading-none">Institutional Grade Authority</h1>
 <p className="text-sm font-bold text-emerald-50 font-semibold text-xs leading-relaxed max-w-xl opacity-80">Modul ini memfasilitasi ekstraksi blanko penilaian operasional ke dalam format binary (PDF/XLSX) untuk verifikasi lapangan oleh Dewan Pembimbing.</p>
 </div>
 </div>
 </div>
 </div>
