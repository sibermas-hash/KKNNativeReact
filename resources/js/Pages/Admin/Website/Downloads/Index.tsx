...
export default function DownloadIndex({ downloads }: Props) {
 ...
 <FileDown size={18} />
 <span className="text-xs font-bold tracking-[0.25em] opacity-80">Konten Publik & Repositori</span>
 </div>
 ...
 <span className="text-sm font-bold text-black tabular-nums leading-none tracking-tight">{downloads.meta?.total || 0} BERKAS</span>
 </div>
 </div>
 <button 
 ...
 <tr key={d.id} className="group hover:bg-emerald-50/30/50 transition-all">
 <td className="px-6 py-4">
 <div className="flex items-center gap-4">
 <div className="h-10 w-10 bg-emerald-50/30 border border-emerald-100/60 text-slate-300 rounded-lg flex items-center justify-center group-hover:bg-emerald-600 group-hover:text-white transition-all"><FileDown size={18} /></div>
 <div className="flex flex-col">
 <span className="text-sm font-bold text-black group-hover:text-emerald-700 transition-colors truncate max-w-[300px]">{d.title}</span>
 ...
 <div className={clsx('inline-flex h-6 items-center px-4 rounded-md text-sm font-bold tracking-normal border transition-all', d.file_path ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100')}>
 {d.file_path ? 'LOCAL_VAULT' : 'EXTERNAL_CDN'}
 </div>
 </td>
 <td className="px-6 py-4 text-center">
 <div className={clsx('inline-flex h-7 items-center px-4 rounded-full text-sm font-bold tracking-normal border transition-all', d.is_active ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-emerald-50/30 text-slate-300 border-emerald-100/60')}>
 ...
 <button onClick={() => openEditModal(d)} className="h-8 w-8 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 text-emerald-950 hover:text-emerald-600 hover:border-emerald-200 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-emerald-900/5 transition-all"><Edit2 size={14} /></button>
 <button onClick={() => { if(confirm('Hapus aset?')) router.delete(route('admin.unduhan.destroy', d.id)) }} className="h-8 w-8 bg-white/90 backdrop-blur-xl border-emerald-100/60 border-emerald-100/60 text-emerald-950 hover:text-rose-500 hover:border-rose-200 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-emerald-900/5 transition-all"><Trash2 size={14} /></button>
 </div>
 </td>
 </tr>
 ))}
 ...
 <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-lg bg-white shadow-2xl overflow-hidden rounded-xl border border-emerald-100/60 flex flex-col">
 <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-emerald-50/30/20">
 <div className="flex items-center gap-4">
 ...
 <input value={data.title} onChange={e => setData('title', e.target.value.toUpperCase())} className="w-full h-12 bg-emerald-50/30 border border-emerald-100/60 rounded-lg px-6 text-[14px] font-bold text-black focus:bg-white focus:border-emerald-500 outline-none transition-all " placeholder="OPERATIONAL_GUIDE_2026" required />
 </div>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="space-y-1">
 <label className="text-sm font-bold text-emerald-950 font-semibold text-xs ml-1">Binary_Source</label>
 <div className="relative h-12 w-full bg-emerald-50/30 rounded-lg border-2 border-dashed border-emerald-100/60 flex flex-col items-center justify-center gap-1 overflow-hidden group/img transition-colors hover:border-emerald-200">
 ...
 <div className="h-12 w-full bg-emerald-50/60 border border-emerald-100/60 rounded-lg p-3 space-y-2">
 <div className="flex items-center gap-2"><Globe size={10} className="text-emerald-500"/><span className="text-sm font-bold text-emerald-950 ">CDN_Link</span></div>
 <input value={data.external_url} onChange={e => setData('external_url', e.target.value)} className="w-full h-8 bg-white/50 border border-emerald-100/60 rounded-md px-3 text-sm font-bold outline-none focus:bg-white" placeholder="https://drive.google.com/..." />
 </div>
 </div>
 </div>
 <div className="flex items-center justify-between bg-emerald-50/30 p-3 rounded-lg border border-emerald-100/60">
 <div className="flex items-center gap-3">
 ...
 <input type="checkbox" checked={data.is_active} onChange={e => setData('is_active', e.target.checked)} className="h-5 w-5 rounded border-emerald-100/60 text-emerald-600 focus:ring-emerald-500" />
 </div>
 ...
 <Button type="submit" disabled={processing} className="h-10 px-8 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold font-semibold text-xs shadow-xl shadow-emerald-100 flex items-center gap-3 active:scale-95">{processing ? <RefreshCw size={14} className="animate-spin" /> : <Save size={14} className="text-white" />} COMMIT_DEP</Button>
 </div>
 </form>
 </motion.div>
 </div>
 ...
 <div className="space-y-1">
 <h4 className="text-lg font-bold tracking-tight leading-none">Institutional Binary Oversight</h4>
