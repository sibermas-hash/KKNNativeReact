...
export default function Assignment({
  ...
  const [deletingId, setDeletingId] = useState<{ type: 'period' | 'region'; id: number } | null>(
    null,
  );
  ...
  const handleDelete = () => {
    ...
    const url =
      type === 'period'
        ? route('admin.dpl.lepas-periode', id)
        : route('admin.dpl.lepas-wilayah', id);
    ...
    <AppLayout title="Penugasan DPL">
      ...
      <div className="max-w-[1600px] mx-auto space-y-8 pb-24 font-sans px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Total Unit"
            value={summary?.groups_total}
            icon={Briefcase}
            variant="gray"
          />
          <StatCard
            label="Butuh Plotting"
            value={summary?.active_groups_without_dpl}
            icon={AlertTriangle}
            variant={summary?.active_groups_without_dpl ? 'danger' : 'success'}
          />
          <StatCard
            label="Koordinator"
            value={summary?.district_coordinators}
            icon={MapPinned}
            variant="success"
          />
        </div>
          ...
          <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-24">
            <ContentPanel
              ...
              actions={
                activeFormMode === 'import' ? (
                  <button
                    onClick={() => setActiveFormMode('default')}
              ...
              {activeFormMode === 'import' && (
                <form onSubmit={submitImport} className="space-y-6">
                  ...
                  <button
                    ...
                    className="w-full h-12 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl shadow-lg shadow-emerald-600/10 transition-all flex items-center justify-center gap-2.5 text-[10px] uppercase tracking-[0.2em] disabled:opacity-50"
                  >
              ...
              {activeFormMode === 'default' && activeTab === 'assignments' && (
                <form onSubmit={submitPeriodAssignment} className="space-y-6">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1 font-display">
                      Dosen Pengampu
                    </label>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => periodForm.setData('max_groups', parseInt(e.target.value))}
                      className="w-full h-11 px-4 bg-white border border-gray-200 rounded-xl text-sm font-black text-emerald-950 focus:border-emerald-600 outline-none transition-all shadow-sm tabular-nums"
                      required
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full h-12 bg-emerald-950 hover:bg-black text-white font-black rounded-xl shadow-lg transition-all flex items-center justify-center gap-2.5 text-[10px] uppercase tracking-[0.2em] disabled:opacity-50"
                  >
              ...
              {activeFormMode === 'default' && activeTab === 'groups' && (
                <form onSubmit={submitGroupAssignment} className="space-y-6">
                    ...
                    <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100 flex items-center justify-between shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest leading-none mb-1 font-display">
                            Unit Terpilih
                          </span>
                          <span className="text-xs font-bold text-emerald-950 leading-tight font-display">
                            {selectedGroup.name}
                          </span>
                        </div>
                      </div>
                      <button
                        ...
                        className="h-8 w-8 rounded-lg hover:bg-rose-50 text-emerald-800 hover:text-rose-500 transition-colors flex items-center justify-center"
                      >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      <select
                        ...
                        className="w-full h-11 pl-4 pr-10 rounded-xl border border-gray-200 bg-white text-xs font-black text-emerald-950 focus:border-emerald-600 appearance-none shadow-sm transition-all outline-none uppercase disabled:opacity-50"
                        required
                      >
                        <option value="">PILIH DOSEN BERKUOTA...</option>
                        {availableAssignments?.map((a) => (
                          <option key={a.id} value={a.id}>
                            {a.dosen?.nama} &middot; [SISA {a.remaining_slots}]
                          </option>
                        ))}
                      </select>
                      <ChevronDown
                  ...
                  <button
                    type="submit"
                    disabled={
                      groupForm.processing || !selectedGroup || availableAssignments.length === 0
                    }
                    className="w-full h-12 bg-emerald-950 hover:bg-black text-white font-black rounded-xl shadow-lg transition-all flex items-center justify-center gap-2.5 text-[10px] uppercase tracking-[0.2em] disabled:opacity-50"
                  >
            ...
            <div className="bg-white border border-emerald-100 rounded-2xl shadow-sm flex flex-col overflow-hidden min-h-[600px]">
              ...
              <div className="p-6 border-b border-emerald-50 bg-gray-50/50 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                ...
                <form
                  ...
                  className="relative w-full xl:w-80 group/search"
                >
                  ...
                  {search && (
                    <button
                      ...
                      onClick={() => {
                        setSearch('');
                        router.get(route('admin.dpl.penugasan'), {}, { replace: true });
                      }}
                      ...
                      <PremiumTableRow key={a.id}>
                        <PremiumTableCell>
                          <div className="flex flex-col py-1">
                            <span className="text-sm font-black text-emerald-950 mb-1 font-display">
                              {a.dosen?.nama}
                            </span>
                            <span className="text-[10px] font-bold text-emerald-700/60 font-mono">
                              NIP: {a.dosen?.nip}
                            </span>
                          </div>
                        </PremiumTableCell>
                        <PremiumTableCell align="center">
                          <span className="inline-flex px-3 py-1 text-[9px] font-black rounded-lg bg-emerald-50 text-emerald-800 border border-emerald-100 uppercase tracking-widest shadow-sm font-display">
                            {a.period?.name}
                          </span>
                        </PremiumTableCell>
                        <PremiumTableCell>
                          <div className="flex flex-col gap-2 w-40">
                            ...
                            <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden border border-emerald-50/30">
                              <motion.div
                                initial={{ width: 0 }}
                                animate={{
                                  width: `${Math.min(100, (a.current_groups / a.max_groups) * 100)}%`,
                                }}
                                className={clsx(
                                  'h-full rounded-full transition-all duration-1000',
                                  a.current_groups >= a.max_groups
                                    ? 'bg-rose-500'
                                    : 'bg-emerald-600',
                                )}
                              />
                        ...
                        <PremiumTableCell align="right">
                          <button
                            ...
                            className="h-9 w-9 inline-flex items-center justify-center text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all active:scale-90"
                          >
                            <Trash2 size={16} />
                          </button>
                        </PremiumTableCell>
                  ...
                  >
                    {groups.map((g) => (
                      <PremiumTableRow
                        key={g.id}
                        className={clsx(
                          groupForm.data.group_id === String(g.id) && 'bg-emerald-50/70',
                        )}
                      >
                        <PremiumTableCell>
                          <div className="flex flex-col py-1">
                            ...
                            <span className="text-[10px] font-bold text-emerald-700/60 font-mono uppercase tracking-widest">
                              CODE: {g.code} &middot; {g.status}
                            </span>
                          </div>
                        </PremiumTableCell>
                        <PremiumTableCell>
                          <div className="flex flex-col gap-1">
                            <span className="text-[11px] font-black text-emerald-900 uppercase tracking-widest leading-none block font-display">
                              {g.location?.district_name || 'N/A'}
                            </span>
                            <span className="text-[9px] font-bold text-emerald-700/40 uppercase">
                              {g.location?.village_name}
                            </span>
                          </div>
                        </PremiumTableCell>
                        <PremiumTableCell>
                          {g.dpl ? (
                            <div className="flex flex-col">
                              <span className="text-xs font-black text-emerald-950 leading-tight mb-1 font-display">
                                {g.dpl.nama}
                              </span>
                              <span className="text-[9px] font-bold text-emerald-700/50 font-mono">
                                NIP: {g.dpl.nip}
                              </span>
                            </div>
                          ) : (
                        ...
                        <PremiumTableCell align="right">
                          <button
                            onClick={() => {
                              groupForm.setData('group_id', String(g.id));
                            ...
                            className={clsx(
                              ...
                              g.dpl
                                ...
                                : 'bg-emerald-950 text-white border-emerald-900 hover:bg-black',
                            )}
                          >
                        ...
                        <PremiumTableCell>
                          <div className="flex flex-col py-1">
                            ...
                            <span className="text-[10px] font-bold text-emerald-700/60 uppercase tracking-widest">
                              {c.regency_name || 'BANYUMAS'}
                            </span>
                          </div>
                        </PremiumTableCell>
                        <PremiumTableCell>
                          <div className="flex flex-col">
                            <span className="text-xs font-black text-emerald-950 mb-1 font-display">
                              {c.dosen?.nama}
                            </span>
                            <span className="text-[9px] font-bold text-emerald-700/50 font-mono">
                              NIP: {c.dosen?.nip}
