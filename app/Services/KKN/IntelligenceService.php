...
class IntelligenceService
{
    ...
    public function getHighRiskAnomalies(?int $facultyId = null): Collection
    {
        ...
        ->where(function($q) {
            $q->where(DB::raw('LENGTH(activity)'), '<', 30)
              ->orWhere('activity', 'like', '%lorem ipsum%')
              ->orWhere('activity', 'like', '%test%')
              ->orWhere('activity', 'like', '%asdf%')
              ->orWhere(function ($hourQuery) {
                  $hourQuery->where(DB::raw('CAST(EXTRACT(HOUR FROM created_at) AS INT)'), '>=', 23)
                            ->orWhere(DB::raw('CAST(EXTRACT(HOUR FROM created_at) AS INT)'), '<=', 4);
              });
        });
        ...
        return $query->latest()
            ...
            ->map(function ($report) {
                $reasons = [];
                if (strlen($report->activity) < 30) $reasons[] = 'Deskripsi terlalu singkat';
                if (preg_match('/lorem|test|asdf/i', $report->activity)) $reasons[] = 'Terindikasi teks placeholder';

                $hour = $report->created_at?->hour;
                if ($hour !== null && ($hour >= 23 || $hour <= 4)) $reasons[] = "Entri waktu mencurigakan ({$hour}:00)";
