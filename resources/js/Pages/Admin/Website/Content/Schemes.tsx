...
export default function SchemeContentPage({ content = { title: '', intro: '', items: [] } }: Props) {
  ...
  return (
    <AppLayout title="Skema KKN">
      <Head title="Manajemen Skema Operasional | SIBERDAYA" />
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-8 py-10 font-sans">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Active Clusters" value={data.schemes.length} icon={Box} variant="success" />
          ...
          >
            <div className="grid gap-8 py-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Judul Tampilan Skema</label>
                <input 
                  value={data.title} 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Introduksi Narasi</label>
                <textarea 
                  rows={3} 
                  value={data.intro} 
                ...
                >
                  <div className="px-8 py-4 bg-gray-50/50 border-b border-emerald-50 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="h-10 w-10 bg-emerald-600 text-white flex items-center justify-center rounded-xl text-[12px] font-black shadow-lg shadow-emerald-600/10 uppercase tabular-nums">{String(idx+1).padStart(2,'0')}</div>
                      <span className="text-[10px] font-black text-emerald-950 uppercase tracking-[0.2em]">Cluster_Node #{idx+1}</span>
                    </div>
                  ...
                  <div className="p-8 space-y-6">
                    <div className="space-y-2">
                      <label className="text-[9px] font-black text-emerald-950 uppercase tracking-widest pl-1">Identifier</label>
                      <input 
                        value={s.title} 
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-black text-emerald-950 uppercase tracking-widest pl-1">Description</label>
                      <textarea 
                        rows={3} 
                        value={s.description} 
                    ...
                    <div className="flex items-center justify-between gap-6 pt-2">
                      <div className="flex-1 space-y-2">
                        <label className="text-[9px] font-black text-emerald-950 uppercase tracking-widest pl-1">Proxy Color</label>
                        <div className="relative h-12">
                          <select 
                            value={s.color} 
                            onChange={e => updateScheme(idx, 'color', e.target.value as SchemeColor)} 
                          ...
                          <div className={clsx("absolute right-4 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full shadow-sm", colorOptions.find(c => c.value === s.color)?.dot)} />
                        </div>
                      </div>
                      <div className={clsx(
                        ...
                        s.color === 'emerald' ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 
                        s.color === 'blue' ? 'bg-blue-50 border-blue-200 text-blue-600' : 
                        s.color === 'amber' ? 'bg-amber-50 border-amber-200 text-amber-600' : 
