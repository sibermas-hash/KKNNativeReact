...
const DocIcon = ({ active, label, icon: Icon }: { active?: boolean; label: string; icon: any }) => (
  <div
    className={clsx('transition-all', active ? 'text-[#0d9488]' : 'text-gray-200')}
...
export default function RegistrationsIndex({ registrations, filters, stats, periods, app }: Props) {
  ...
  const handleExport = (type: 'standard' | 'biodata') => {
    const url =
      type === 'standard' ? '/admin/pendaftaran/ekspor' : '/admin/pendaftaran/ekspor-biodata';
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={clsx(
                      ...
                      app?.storage_disk === 's3' ? 'bg-blue-500' : 'bg-emerald-500',
                    )}
                  />
            ...
            <button
              ...
              className="h-12 px-5 bg-white border-2 border-gray-100 text-emerald-950 rounded-xl font-bold transition-all shadow-sm flex items-center gap-3 active:scale-95 text-xs uppercase tracking-widest hover:border-emerald-600"
            >
              <Download size={16} strokeWidth={2.5} /> Ekspor List
            </button>
            <button
              onClick={() => handleExport('biodata')}
              className="h-12 px-6 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-emerald-600/20 flex items-center gap-3 active:scale-95 text-xs uppercase tracking-widest"
            >
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            icon={Clock}
            variant={stats?.pending && stats.pending > 0 ? 'warning' : 'success'}
          />
          <StatCard
            label="Disetujui"
            ...
            <div className="flex-1 w-full space-y-1.5">
              ...
              <div className="relative group">
                <select
                  ...
                  className="w-full h-12 pl-4 pr-10 rounded-xl border border-gray-200 bg-white text-xs font-bold text-emerald-950 focus:border-emerald-600 appearance-none shadow-sm transition-all outline-none"
                >
            ...
            <div className="bg-emerald-950 rounded-2xl p-4 flex items-center justify-between gap-6 border border-emerald-800 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="h-10 px-5 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95 flex items-center gap-2"
                >
                  <CheckCheck size={14} /> Setujui Semua
                </button>
                <button
                  onClick={handleBulkReject}
                  className="h-10 px-5 bg-rose-600 hover:bg-rose-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all active:scale-95 flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="h-10 px-3 text-white/60 hover:text-white text-[10px] font-bold uppercase tracking-widest transition-all"
                >
                  Batal
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-emerald-950/40 uppercase tracking-widest tabular-nums">
                    Menampilkan {registrations?.data?.length || 0} entri • Halaman{' '}
                    {registrations?.meta?.current_page || 1} dari{' '}
                    {registrations?.meta?.last_page || 1}
                  </span>
                </div>
          ...
          >
            ...
            >
              {registrations?.data?.map((r) => (
                <PremiumTableRow
                  key={r.id}
                  className={clsx(selectedIds.includes(r.id) && 'bg-emerald-50/50')}
                >
                  ...
                  <PremiumTableCell>
                    <div className="flex flex-col gap-2 py-1">
                      <div className="flex flex-col">
                        <span className="text-[13px] font-black text-emerald-950 uppercase leading-tight tracking-tight font-display">
                          {r.student.name}
                        </span>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {r.student.is_bta_ppi_passed ? (
                          <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-md text-[9px] font-black border border-emerald-100 uppercase">
                            BTA/PPI OK
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 bg-rose-50 text-rose-700 rounded-md text-[9px] font-black border border-rose-100 uppercase animate-pulse">
                            BTA/PPI FAIL
                          </span>
                        )}
                        {!r.is_eligible && (
                          <span
                            className="px-2 py-0.5 bg-amber-50 text-amber-700 rounded-md text-[9px] font-black border border-amber-100 uppercase"
                            title={r.eligibility_issues?.join(', ')}
                          >
                            ISSUE
                          </span>
                        )}
                  ...
                  <PremiumTableCell>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[10px] font-black text-emerald-900 uppercase truncate max-w-[150px]">
                        {r.student.program?.name || 'UMUM'}
                      </span>
                      <span className="text-[9px] font-bold text-emerald-600/60 uppercase tracking-widest">
                        {r.student.faculty?.name || 'UIN SAIZU'}
                      </span>
                    </div>
                  </PremiumTableCell>
                  <PremiumTableCell>
                  ...
                  <PremiumTableCell>
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-2 text-emerald-950">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] font-bold text-emerald-600/60 tabular-nums">
                            {new Date(r.registration_date).toLocaleTimeString('id-ID', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                      ...
                      <div className="flex items-center gap-1.5 text-[9px] font-black text-emerald-800/40 uppercase tracking-tighter">
                        ...
                        <span className="truncate max-w-[80px]" title={r.period.name}>
                          {r.period.jenis || 'Reguler'}
                        </span>
                      </div>
                    </div>
                  </PremiumTableCell>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      {r.group && (
                        <div className="flex items-center gap-1 px-1.5 py-0.5 bg-gray-50 rounded border border-gray-100 w-fit">
                          ...
                          <span className="text-[9px] font-black text-gray-500 uppercase tracking-tighter truncate max-w-[100px]">
                            {r.group.name}
                          </span>
                        </div>
                      )}
                  ...
                  <PremiumTableCell align="right">
                    <Link
                      ...
                      className="h-9 px-4 bg-emerald-900 text-white hover:bg-emerald-800 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black transition-all active:scale-95 uppercase tracking-widest shadow-md"
                    >
