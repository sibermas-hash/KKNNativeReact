        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-emerald-50 pt-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-sm text-emerald-800">
              Tahun Akademik:{' '}
              <strong className="text-emerald-800">{period.academic_year.name}</strong>
            </span>
            <Link
              href="/admin/periode"
                      ...
                      <li key={idx} className="ml-6">
                        ...
                        <div
                          className={clsx(
                            ...
                            isActive ? 'bg-gray-50 border-gray-300' : 'bg-white border-emerald-50',
                          )}
                        >
                          <div className="flex items-center justify-between gap-4">
                            ...
                            {isActive && (
                              <span className="text-xs font-semibold text-[#0d9488] bg-[#f0fdfa] px-2 py-0.5 rounded-full whitespace-nowrap">
                                Fase Aktif
                              </span>
                            )}
                          </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="pt-2 border-t border-emerald-50">
                  <p className="text-xs text-emerald-800">
                    Seluruh akses fitur mahasiswa dan DPL dikontrol berdasarkan fase aktif periode
                    ini.
                  </p>
                </div>
              </div>
...
function StatCard({
  ...
  return (
    <div className="bg-white border border-emerald-50 rounded-xl shadow-sm p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
      <div className="h-10 w-10 rounded-lg bg-gray-50 text-[#0d9488] flex items-center justify-center shrink-0">
...
function QuickLink({ href, label }: { href: string; label: string }) {
  return (
    ...
    >
      <span className="text-sm font-medium text-emerald-800 group-hover:text-emerald-800">
        {label}
      </span>
      <ArrowRight
        size={16}
        className="text-emerald-800 group-hover:text-[#0d9488] transition-colors"
      />
    </Link>
  );
}
