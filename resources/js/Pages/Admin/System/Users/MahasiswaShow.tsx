              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm text-amber-900">
                    Password baru:{' '}
                    <code className="bg-white px-2 py-0.5 rounded border border-amber-200 font-mono font-bold select-all">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-5 text-center md:text-left">
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-3 gap-y-1 text-sm text-emerald-800/70 font-bold uppercase tracking-wide">
                  <span>
                    NIM: <strong className="text-emerald-900">{mahasiswa.nim}</strong>
                  </span>
                  <span className="hidden md:inline opacity-30">•</span>
                  <span>
                    Angkatan{' '}
                    <strong className="text-emerald-900">{mahasiswa.batch_year || '—'}</strong>
                  </span>
                  <span className="hidden md:inline opacity-30">•</span>
                  <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded text-[10px] font-black">
                    {mahasiswa.prodi?.nama || 'UMUM'}
                  </span>
                </div>
              </div>
            </div>
            {account && (
              <div className="flex items-center gap-3 shrink-0">
                ...
                >
                  {account.is_active ? (
                    <>
                      <Lock size={15} /> Kunci Akun
                    </>
                  ) : (
                    <>
                      <Unlock size={15} /> Aktifkan Akun
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="bg-white border border-emerald-50 rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="p-4 rounded-lg bg-gray-50 border border-emerald-50 text-center space-y-1">
                    <p className="text-2xl font-black font-display uppercase tracking-tighter text-emerald-950 tabular-nums">
                      {mahasiswa.sks_completed ?? 0}
                    </p>
                    <p className="text-xs text-emerald-800 font-medium">SKS Selesai</p>
                  </div>
                  <div className="p-4 rounded-lg bg-gray-50 border border-emerald-50 text-center space-y-1">
                    <p className="text-2xl font-black font-display uppercase tracking-tighter text-emerald-950 tabular-nums">
                      {Number(mahasiswa.gpa ?? 0).toFixed(2)}
                    </p>
                    ...
                    <div className="flex items-center justify-center gap-2">
                      {mahasiswa.is_bta_ppi_passed ? (
                        <>
                          ...
                          <p className="text-sm font-bold text-emerald-950">Lulus BTA-PPI</p>
                        </>
                      ) : (
                        <>
                          <XCircle size={18} className="text-rose-600" />
                          <p className="text-sm font-bold text-rose-800">Belum Lulus BTA-PPI</p>
                        </>
                      )}
                    </div>
                    <p className="text-xs text-emerald-800 uppercase tracking-wider font-bold text-center">
                      Prasyarat Utama
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-white border border-emerald-50 rounded-xl shadow-sm overflow-hidden">
                ...
                {registration ? (
                  <div className="p-5 grid grid-cols-2 sm:grid-cols-3 gap-6">
                    <div>
                      ...
                      <span
                        className={clsx(
                          ...
                          STATUS_BADGE[registration.status] || 'bg-gray-100 text-emerald-800',
                        )}
                      >
                    ...
                    {registration.status === 'rejected' && registration.rejection_reason && (
                      <div className="col-span-3 p-4 bg-rose-50 border border-rose-100 rounded-xl">
                        <p className="text-xs text-rose-600 font-bold uppercase tracking-tight">
                          Alasan Penolakan:
                        </p>
                    ...
                    <div className="col-span-3 pt-2 text-right">
                      <Link
                        ...
                        className="text-sm text-[#0d9488] hover:text-[#0d9488] font-bold flex items-center gap-1 justify-end transition-colors"
                      >
                  ...
                  <div className="p-8 text-center">
                    <ClipboardList
                      className="mx-auto h-10 w-10 text-gray-200 mb-2"
                      strokeWidth={1.5}
                    />
                    <p className="text-sm text-emerald-800 font-medium">
                      Mahasiswa ini belum melakukan pendaftaran KKN.
                    </p>
                  </div>
                )}
              </div>
                    ...
                    {group.location && (
                      <>
                        ...
                        <InfoField
                          ...
                          value={group.location.regency_name || '—'}
                        />
                      </>
                    )}
                  </div>
                ...
                {account ? (
                  <div className="p-5 space-y-4">
                    ...
                    <div className="pt-2 mt-2 border-t border-gray-100">
                      <InfoField
                        label="Sinkronisasi Terakhir LPPM"
                        value={formatDate(mahasiswa.master_synced_at)}
                      />
                    </div>
                  </div>
                ) : (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-xs text-emerald-800 font-medium leading-relaxed">
                      Akun sistem belum diinisialisasi melalui sinkronisasi master.
                    </p>
                  </div>
                )}
              </div>
                    ...
                    {dispensasi.map((d) => (
                      <div key={d.id} className="p-5 space-y-3">
                        ...
                        <p className="text-xs font-medium text-emerald-800 uppercase">
                          Periode: {d.periode?.name || 'Semua'} · {d.created_at}
                        </p>
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {(d.bypassed_requirements || []).map((r) => (
                            <span
                              key={r}
                              className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-100 rounded text-xs font-bold uppercase"
                            >
                              {r.replace('_', ' ')}
                            </span>
                          ))}
                        ...
                        <p className="text-xs text-emerald-800 font-medium">
                          Otorisator: <span className="text-emerald-800">{d.granted_by}</span>
                        </p>
                      </div>
                    ))}
                  </div>
      ...
      {account && (
        <>
          <ConfirmDialog
            ...
            onConfirm={() => {
              router.post(
                `/admin/pengguna/${account.id}/reset-password-sementara`,
                {},
                { preserveScroll: true },
              );
          ...
          <ConfirmDialog
            ...
            onConfirm={() => {
              router.patch(
                `/admin/pengguna/${account.id}/toggle-status`,
                {},
                { preserveScroll: true },
              );
...
function InfoField({
  ...
  return (
    <div className={span ? `col-span-${span}` : undefined}>
      <p className="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-1 opacity-70">
        {label}
      </p>
      <p
        className={clsx(
          'text-sm text-emerald-950 font-semibold break-words',
