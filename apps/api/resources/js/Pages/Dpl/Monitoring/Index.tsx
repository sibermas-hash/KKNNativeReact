      ...
      <div className="space-y-8">
        <section className="rounded-lg border border-emerald-50/60 bg-white p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-emerald-950">Monitoring Kunjungan DPL</h1>
        ...
        <section className="rounded-lg border border-emerald-50/60 bg-white p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {groups.map((group) => (
              <div
                key={group.id}
                className="rounded-lg border border-emerald-50/60 bg-emerald-50/30 p-4 hover:bg-white hover:border-emerald-300 transition-colors"
              >
                <div className="flex items-center gap-2 mb-2">
                  <MapPin className="h-4 w-4 text-emerald-950" />
                  <span className="text-sm font-semibold text-emerald-950">{group.nama}</span>
                </div>
                {group.periode && <p className="text-xs text-emerald-950 mb-3">{group.periode}</p>}
                <div className="flex items-center justify-between text-xs text-emerald-950">
                  <span>
                    Total: <strong className="text-emerald-950">{group.total_monitoring}</strong>{' '}
                    kunjungan
                  </span>
                  <span>
                    Terakhir:{' '}
                    <strong className="text-emerald-950">{group.terakhir_monitoring}</strong>
                  </span>
                </div>
                <Link
        ...
        <section className="rounded-lg border border-emerald-50/60 bg-white">
          <div className="border-b border-emerald-50/60 px-6 py-4">
            <h2 className="text-lg font-semibold text-emerald-950">Riwayat Monitoring</h2>
          </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        <p className="text-emerald-800">
                          <span className="font-medium text-emerald-950">Solusi:</span> {m.solusi}
                        </p>
                        {m.catatan_tambahan && (
                          <p className="text-xs text-emerald-950 ">Catatan: {m.catatan_tambahan}</p>
                        )}
                      </div>
                    </div>
                  </div>
