      ...
      <div className="space-y-12 pb-24 font-sans text-emerald-950">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 text-[#0d9488]">
              ...
              <span className="text-xs font-semibold opacity-80">
                Audit Struktur & Operasional Unit
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-6 pt-4">
                <div className="flex items-center gap-3 bg-white border border-emerald-50 px-5 py-2.5 rounded-xl shadow-sm">
                  ...
                  <span className="text-xs font-semibold text-emerald-950 font-mono">
                    TOKEN: {group.token || 'MISSING'}
                  </span>
                </div>
                <div className="flex items-center gap-3 bg-white border border-emerald-50 px-5 py-2.5 rounded-xl shadow-sm">
                  <Briefcase size={16} className="text-emerald-800" />
                  <span className="text-xs font-semibold text-emerald-950 leading-none">
                    {group.periode?.name || 'UMUM'}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          <MetricCard
            ...
            icon={ClipboardList}
            desc="Rencana Operasional"
          />
          <MetricCard
            label="Pembimbing"
            value={lecturerRows.length}
            icon={UserCheck}
            desc="Delegasi DPL"
          />
        </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-xs font-bold text-emerald-800">
                      Daftar Mahasiswa Terdaftar Dalam Unit
                    </p>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-xs font-semibold text-emerald-800 mb-1">Total Entitas</span>
                  <span className="text-3xl font-semibold text-white tabular-nums er leading-none">
                    {memberRows.length}
                  </span>
                </div>
              </div>
                ...
                <table className="min-w-full text-left border-collapse whitespace-nowrap">
                  <thead className="bg-gray-50 text-emerald-950 border-b border-emerald-50">
                    <tr>
                      ...
                      <th className="px-8 py-6 text-center text-xs font-semibold">
                        Peran Otoritas
                      </th>
                  ...
                  <tbody className="divide-y divide-[#f3f4f6]">
                    {memberRows.length === 0 ? (
                      <EmptyTable
                        icon={Users}
                        label="Manifest Personel Kosong"
                        desc="Belum ada mahasiswa yang terdaftar pada unit ini."
                      />
                    ) : (
                      memberRows.map((m) => (
                        <tr key={m.id} className="group hover:bg-gray-50 transition-all">
                          <td className="px-10 py-8">
                            <div className="flex flex-col gap-3">
                              <span className="text-sm font-semibold text-emerald-950 leading-none group-hover:text-emerald-800 transition-colors">
                                {m.mahasiswa?.nama || 'PESERTA'}
                              </span>
                              <span className="text-xs font-semibold text-[#0d9488] font-mono">
                                NIM: {m.mahasiswa?.nim || 'UNKNOWN'}
                              </span>
                            </div>
                          </td>
            ...
            <section className="bg-white border border-emerald-50 rounded-xl overflow-hidden shadow-sm flex flex-col">
              ...
              <div className="p-10 space-y-6">
                {workPrograms.length === 0 ? (
                  <EmptyBox
                    icon={ClipboardList}
                    label="Data Proker Kosong"
                    desc="Unit belum melakukan unggah rencana program kerja."
                  />
                ) : (
                  workPrograms.map((p) => (
                    <div
                      key={p.id}
                      className="group/pro p-8 bg-gray-50 border border-emerald-50 rounded-xl hover:bg-white hover:border-[#0d9488] transition-all duration-500 flex items-center justify-between gap-8 shadow-sm"
                    >
                      <div className="flex items-center gap-6">
                        ...
                        <div className="flex flex-col gap-3">
                          <span className="text-lg font-semibold text-emerald-950 group-hover/pro:text-emerald-800 transition-colors leading-tight truncate max-w-[400px]">
                            {p.title || 'Draft Proker'}
                          </span>
                          <span className="text-xs font-semibold text-emerald-800 font-mono">
                            ID: #{p.id.toString().padStart(6, '0')}
                          </span>
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  {lecturerRows.length === 0 ? (
                    ...
                    lecturerRows.map((l) => (
                      <div
                        key={l.id}
                        className="p-8 bg-emerald-700/50 border border-[#0d9488] rounded-xl hover:bg-white hover:text-emerald-950 transition-all duration-500 group/card shadow-sm"
                      >
                        <span className="text-xs font-semibold text-emerald-800 block mb-1">
                          Ketua Pembimbing
                        </span>
                        <h4 className="text-lg font-semibold leading-tight mb-4">
                          {l.nama || '-'}
                        </h4>
                        <div className="h-px w-full bg-[#0d9488] group-hover/card:bg-[#f0fdfa] mb-4 transition-colors" />
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-semibold font-mono opacity-60 group-hover/card:text-[#0d9488]">
                            NIP: {l.nip || '-'}
                          </span>
            ...
            <section className="bg-white border border-emerald-50 rounded-xl p-10 space-y-10 shadow-sm relative overflow-hidden group/loc">
              ...
              <div className="space-y-8">
                <div className="p-8 bg-gray-50 border border-emerald-50 rounded-xl space-y-4">
                  <span className="text-xs font-semibold text-emerald-800 leading-none">
                    ZONA PENEMPATAN
                  </span>
                  <div className="flex flex-col gap-2">
                    <span className="text-xl font-semibold text-emerald-950 leading-tight er">
                      {group.lokasi?.full_name || group.lokasi?.village_name || 'BELUM TERPETAKAN'}
                    </span>
                    {group.lokasi?.address && (
                      <p className="text-xs font-bold text-emerald-800 leading-relaxed opacity-60">
                        {group.lokasi.address}
                      </p>
                    )}
                  </div>
                ...
                {group.posko ? (
                  <div className="space-y-6">
                    {group.posko.photo_url && (
                      <div className="relative h-64 rounded-xl overflow-hidden border border-emerald-50 shadow-sm group/img">
                        <img
                          ...
                          alt="Foto posko"
                          className="absolute inset-0 w-full h-full object-cover grayscale group-hover/img:grayscale-0 group-hover/img:scale-110 transition-all duration-1000"
                        />
                    ...
                    <div className="p-8 bg-white text-white rounded-xl space-y-8 shadow-sm relative overflow-hidden group/gmaps">
                      <div className="absolute top-0 right-0 p-8 opacity-5 rotate-12 -mr-12 -mt-12 group-hover/gmaps:rotate-[45deg] transition-all duration-1000">
                        <Globe size={150} />
                      </div>
            ...
            <div className="space-y-6 flex-1">
              <div className="flex items-center gap-6">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-semibold text-emerald-800 opacity-80">
                    Protokol Akuntabilitas Lapangan
                  </span>
                </div>
              </div>
              <p className="text-sm font-bold text-emerald-800 leading-relaxed max-w-4xl">
                Seluruh parameter yang terdata pada unit operasional ini merupakan representasi
                entitas valid dari program <span className="text-sky-600">SIBER</span>
                <span className="text-emerald-600">MAS</span>. Perubahan pada struktur kepemimpinan,
                proker, maupun titik koordinat posko akan terekam dalam audit trail transparan untuk
                menjamin integritas monitoring dan evaluasi terpusat.
              </p>
            </div>
            <div className="h-20 w-px bg-white/10 hidden lg:block" />
            <div className="flex flex-col items-end shrink-0 hidden lg:flex">
              <span className="text-xs font-semibold text-emerald-800 mb-1 opacity-60">
                KEAMANAN DATA
              </span>
  ...
  return (
    <div className="bg-white border border-emerald-50 rounded-xl p-6 flex items-center gap-5 shadow-sm hover:border-emerald-50 transition-all group overflow-hidden relative">
      ...
      <div className="flex flex-col relative z-20">
        ...
        <span className="text-2xl font-semibold text-emerald-950 er leading-none group-hover:text-emerald-800 transition-colors mb-1.5">
          {value}
        </span>
...
function EmptyBox({ icon: Icon, label, desc }: { icon: any; label: string; desc: string }) {
  return (
    <div className="py-24 border border-dashed border-emerald-50 rounded-xl flex flex-col items-center justify-center gap-4 group/none hover:border-gray-300 transition-all">
      <Icon
        size={64}
        className="text-emerald-50 group-hover/none:text-emerald-800 transition-colors"
        strokeWidth={1}
      />
