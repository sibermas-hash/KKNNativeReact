              ...
              animate-in slide-in-from-right-8 fade-in duration-500 transition-all`}
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-white text-sm font-black tracking-tight">{t.title}</p>
                                {t.message && (
                                    <p className="text-slate-400 text-[11px] mt-1 font-medium leading-relaxed">{t.message}</p>
                                )}
                                {t.action && (
                                    <a href={t.action.href}
                                        className={`inline-block mt-3 text-xs font-bold underline decoration-2 underline-offset-4 ${cfg.iconColor} hover:brightness-125 transition-all`}>
                                        {t.action.label} →
                                    </a>
                                )}
                            </div>
                            <button onClick={() => onDismiss(t.id)}
                                className="flex-shrink-0 p-1 rounded-lg text-slate-500 hover:text-white hover:bg-white/10 transition-all self-start">
                                <XMarkIcon className="w-4 h-4" />
                            </button>
                        </div>
                        {/* Auto-progress bar (visual only) */}
                        <div className={`absolute bottom-0 left-0 h-1 ${cfg.iconColor.replace('text', 'bg')} opacity-20`}
                            style={{ width: '100%', animation: `shrink ${t.duration ?? 5000}ms linear forwards` }} />
                    </div>
                )
            })}
            <style>{`
