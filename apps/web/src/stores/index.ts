...
async function fetchCurrentUserSilently(): Promise<CurrentUserProbeResult> {
  try {
    const response = await fetch(apiUrl('/auth/user'), {
      ...
      const payload = await response.json() as { data?: unknown; authenticated?: boolean } | User | null;
      ...
      // Backend returns { authenticated: false, user: null } for unauthenticated users
      if (maybeUser && typeof maybeUser === 'object' && 'authenticated' in maybeUser && (maybeUser as Record<string, unknown>).authenticated === false) {
