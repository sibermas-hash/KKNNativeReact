...
export default function PesertaEksternalPage(): React.JSX.Element {
  ...
  return <main className="space-y-6 p-1">
    ...
    <section className="grid gap-4 md:grid-cols-3">
      ...
      {batches.map(b => <div key={b.id} className="group rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-lg">
    ...
    <section className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-cyan-50 p-4"><div className="flex items-center gap-2 font-bold text-slate-800"><Users size={18}/> Daftar Peserta Eksternal {listQ.isFetching && <Loader2 size={14} className="animate-spin text-slate-400" />}</div><div className="flex flex-wrap items-center gap-2"><label className="relative"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Cari nama/NIM/kampus" className="w-56 rounded-xl border border-slate-200 bg-white/80 py-2 pl-9 pr-3 text-xs font-semibold outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100" /></label><a className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-black text-white transition hover:-translate-y-0.5 hover:bg-slate-800 hover:shadow-lg" href="/api/v1/admin/peserta-eksternal/export"><Download size={14}/> Export CSV</a></div></div>
      <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="w-14 px-4 py-3 text-center">No</th><th className="px-4 py-3">Nama</th><th className="px-4 py-3">NIM Asal</th><th className="px-4 py-3">Kampus</th><th className="px-4 py-3">Fak/Prodi Asal</th><th className="px-4 py-3">Kelompok</th></tr></thead><tbody>{rows.length === 0 ? <tr><td className="px-4 py-6 text-center text-slate-500" colSpan={6}>Belum ada peserta eksternal.</td></tr> : rows.map((r, idx)=>{const p=r.mahasiswa?.peserta?.[0]; return <tr key={r.id} className="border-t transition-colors hover:bg-cyan-50/40"><td className="px-4 py-3 text-center text-slate-500">{idx + 1}</td><td className="px-4 py-3 font-medium">{r.mahasiswa?.nama ?? '-'}</td><td className="px-4 py-3">{r.external_nim}</td><td className="px-4 py-3">{r.home_university}</td><td className="px-4 py-3">{r.external_faculty ?? '-'} / {r.external_study_program ?? '-'}</td><td className="px-4 py-3">{p?.kelompok?.nama_kelompok ?? 'Belum ditempatkan'} {p?.kelompok?.lokasi?.regency_name ? `(${p.kelompok.lokasi.regency_name})` : ''}</td></tr>})}</tbody></table></div>
    </section>
  </main>;
}
