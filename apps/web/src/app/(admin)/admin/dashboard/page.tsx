...
export default function AdminDashboardPage(): React.JSX.Element {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['admin', 'dashboard', 'v2', { periode_id: selPeriod }],
    queryFn: async () => {
  ...
  const periodWarning = d?.period_warning as string | undefined;
  ...
  return (
    ...
    >
      ...
      {periodWarning && (
        <motion.div variants={ENTER} className="rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          {periodWarning}
        </motion.div>
      )}

      {!isLoading && !stats && (
        <motion.div variants={ENTER} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
          Statistik dashboard belum tersedia untuk periode ini. Pilih periode lain jika diperlukan.
        </motion.div>
      )}
      ...
      <motion.div variants={ENTER} className="rounded-2xl border border-white/70 bg-white/80 px-5 py-4 shadow-sm backdrop-blur flex items-center">
        {PHASES.map((p, i) => (
          <div key={p.id} className="flex items-center flex-1 min-w-0">
            <Tip content={p.label}>
              <button
                disabled={!isSuperadmin || !periodId || phaseMut.isPending}
                onClick={() => isSuperadmin && periodId && phaseMut.mutate({ periode_id: periodId, phase: p.id })}
                title={isSuperadmin ? p.label : 'Hanya Super Admin yang boleh mengubah fase'}
