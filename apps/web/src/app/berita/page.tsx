        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {announcements.map((item) => (
              <Link key={String(item.id)} href={`/berita/${String(item.slug || '')}`} className="group rounded-2xl bg-white p-6 shadow-sm transition hover:shadow-md">
                {item.image_url ? (
                  <div className="mb-4 h-40 overflow-hidden rounded-xl bg-slate-100">
                    <img src={String(item.image_url)} alt={String(item.title || '')} className="h-full w-full object-cover" />
                  </div>
                ) : null}
                <p className="text-xs text-slate-500">{String(item.published_at || '')}</p>
                <p className="mt-2 font-semibold text-slate-800 group-hover:text-teal-600">{String(item.title || '')}</p>
                <p className="mt-2 line-clamp-3 text-sm text-slate-600">{String(item.excerpt || '')}</p>
              </Link>
            ))}
          </div>
        )}
