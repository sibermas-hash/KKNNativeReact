      ...
      <div className="max-w-5xl mx-auto space-y-10 py-12 px-4">
        ...
        <div className="relative overflow-hidden rounded-[3rem] bg-emerald-950 p-12 text-white shadow-2xl">
          ...
          <div className="relative z-10 space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/20 rounded-full border border-emerald-500/30">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-emerald-300">Pusat Pengembangan SDM</span>
            </div>
          ...
          <div className={clsx(
            ...
            <div className="flex-1 space-y-2 text-center md:text-left">
              <h3 className={clsx(
                ...
                dpl_eligibility.eligible ? "text-emerald-900" : "text-rose-900"
              )}>
            ...
            {dpl_eligibility.eligible && (
              <div className="px-6 py-2 bg-emerald-50 rounded-xl border border-emerald-100 text-[10px] font-black text-emerald-600 uppercase tracking-widest animate-pulse">
                Terverifikasi
              </div>
            )}
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-4 px-4">
            <h2 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">Jadwal Tersedia</h2>
            <div className="h-px flex-1 bg-slate-100" />
          </div>

          <div className="grid grid-cols-1 gap-6">
            {workshops.length === 0 ? (
              <div className="py-20 bg-white rounded-[3rem] border border-slate-100 flex flex-col items-center justify-center text-slate-300">
              ...
              workshops.map((w) => (
                <div key={w.id} className="group bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-10 shadow-sm hover:shadow-2xl hover:shadow-emerald-900/5 transition-all duration-500 relative overflow-hidden">
                  {w.is_registered && (
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-3">
                          <span className={clsx(
                            ...
                            w.is_registered ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-50 text-slate-500 border-slate-100"
                          )}>
                        ...
                        <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase leading-none">{w.title}</h3>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest line-clamp-1">{w.description || 'Tidak ada deskripsi tambahan.'}</p>
                      </div>

                      <div className="flex flex-wrap gap-x-8 gap-y-4">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-500 transition-colors">
                            <Calendar size={18} />
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tanggal</span>
                            <span className="text-xs font-black text-slate-700 uppercase">{w.date}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-500 transition-colors">
                          ...
                          <div className="flex flex-col">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Waktu</span>
                            <span className="text-xs font-black text-slate-700 uppercase">{w.time}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-500 transition-colors">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-black text-slate-700 uppercase truncate max-w-[200px]">{w.location || 'LPPM'}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="shrink-0 flex flex-col items-center md:items-end gap-4">
                      <div className="text-right hidden md:block">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Kapasitas</span>
                      ...
                      {w.is_registered ? (
                        <div className="flex items-center gap-2 px-8 py-4 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100">
                          <CheckCircle2 size={18} strokeWidth={3} />
                          <span className="text-[11px] font-black uppercase tracking-widest">Sudah Terdaftar</span>
                        </div>
                      ) : (
                        <Button
                          disabled={!!w.is_full || !!(dpl_eligibility && !dpl_eligibility.eligible)}
                          onClick={() => handleRegister(w.id)}
                          className="h-14 px-10 rounded-2xl uppercase text-[11px] font-black tracking-widest shadow-xl shadow-emerald-600/20"
