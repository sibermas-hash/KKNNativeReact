        ...
        <AppLayout title="Preview Nilai">
            ...
            <div className="max-w-6xl mx-auto pb-20 p-6">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Konfirmasi Import Nilai</h1>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 overflow-hidden relative">
                            <div className="relative z-10">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Ringkasan File</p>
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-bold text-slate-600">Total Baris</span>
                                        <span className="text-lg font-black text-slate-900">{preview.length}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-bold text-slate-600 text-emerald-600">Siap Import</span>
                                        <Badge variant="success" className="text-xs font-black px-3">{readyCount}</Badge>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-bold text-slate-600 text-rose-600">Bermasalah</span>
                                        <Badge variant="danger" className="text-xs font-black px-3">{preview.length - readyCount}</Badge>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-slate-900 rounded-[2rem] p-6 text-white shadow-xl">
                            ...
                            <p className="text-2xl font-black tracking-tight mb-1">{group.name}</p>
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{group.village_name}</p>
                        </div>
                        ...
                        <Button
                            ...
                            size="lg"
                            className="w-full h-14 rounded-2xl shadow-xl shadow-indigo-500/20 font-black tracking-tight"
                        >
                    ...
                    <div className="lg:col-span-3">
                        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100 font-black uppercase text-[10px] text-slate-500 tracking-widest">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {preview.map((item, idx) => (
                                        <tr key={idx} className={`group hover:bg-slate-50/50 transition-colors ${item.status !== 'READY' ? 'bg-rose-50/20' : ''}`}>
                                            ...
                                            <td className="px-6 py-4 text-center font-black text-slate-700">{item.discipline}</td>
                                            <td className="px-6 py-4 text-center font-black text-slate-700">{item.attitude}</td>
                                            <td className="px-6 py-4">
                                                {item.status === 'READY' ? (
