      ...
      <div className="max-w-[1600px] mx-auto space-y-8 pb-24 font-sans px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pt-10">
          ...
          <div className="bg-white border border-emerald-50 rounded-xl p-4 flex items-center gap-4 shadow-sm shadow-emerald-900/5">
             ...
             <div>
                ...
                <h4 className="text-xl font-bold text-emerald-950 tabular-nums">{configs?.length} Node</h4>
             </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white border border-emerald-50 rounded-xl overflow-hidden shadow-sm">
              <div className="p-6 border-b border-[#f3f4f6] bg-gray-50 flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-bold text-emerald-950 uppercase tracking-wider">Form Narasi</h3>
                  <p className="text-xs font-bold text-emerald-800">OFFICIAL NARRATIVE SCRIPT</p>
                </div>
              </div>
              
              <div className="p-8 space-y-6">
                {/* Dynamic Injectors */}
                <div className="bg-gray-50 border border-emerald-50 rounded-xl p-5 mb-4">
                  <div className="flex items-center gap-3 mb-3">
                    <Binary size={16} className="text-[#1a7a4a]" />
                    <h4 className="text-xs font-bold text-emerald-950 uppercase tracking-widest">Metadata Tags Tersedia</h4>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {(['[Nama]', '[NIM]', '[Fakultas]', '[Kelompok]', '[Lokasi]']).map((tag) => (
                      <span key={tag} className="px-2 py-1 bg-white border border-emerald-50 text-[#1a7a4a] rounded text-[9px] font-bold shadow-sm">{tag}</span>
                    ))}
                  </div>
                </div>
                ...
                {textConfigs.map((config) => (
                  <div key={config.id} className="space-y-2">
                    <label htmlFor={`cert-config-${config.id}`} className="text-xs font-bold text-emerald-950 uppercase tracking-widest pl-1">
                      {config.label}
                    </label>
                    {config.type === 'longtext' ? (
                      <textarea
                        ...
                        className="w-full px-5 py-4 bg-gray-50 border border-emerald-50 rounded-xl text-sm font-bold text-emerald-950 focus:bg-white focus:border-[#f3f4f6]0 outline-none transition-all placeholder:text-black resize-none"
          ...
          <div className="lg:col-span-8 flex flex-col h-full">
            <div className="bg-white border border-emerald-50 rounded-xl overflow-hidden shadow-sm flex flex-col h-full min-h-[500px]">
              ...
              <div className="p-6 border-b border-[#f3f4f6] bg-gray-50 flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-bold text-emerald-950 uppercase tracking-wider">Identitas Visual</h3>
                  <p className="text-xs font-bold text-emerald-800">GRAPHIC ASSETS & RESOURCES</p>
                </div>
              </div>
              ...
              <div className="p-8 space-y-8 flex-1">
                <div className="grid grid-cols-1 gap-6">
                  {imageConfigs.map((config) => (
                    <div key={config.id} className="space-y-2">
                      <label htmlFor={`cert-img-${config.id}`} className="text-xs font-bold text-emerald-950 uppercase tracking-widest pl-1">
                        {config.label}
                      </label>
                      <div className="relative">
                        <ImageIcon size={18} className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-800"/>
                        <input
                          ...
                          className="w-full h-12 pl-12 pr-5 bg-gray-50 border border-emerald-50 rounded-xl text-sm font-bold text-emerald-950 focus:bg-white focus:border-[#f3f4f6]0 outline-none transition-all font-mono placeholder:text-black"
                        />
                      </div>
                    </div>
                  ))}
                ...
                <div className="py-6 flex justify-center">
                   <div className="w-full max-w-sm aspect-[4/3] bg-gray-50 rounded-xl border-2 border-dashed border-emerald-200 flex flex-col items-center justify-center text-center gap-4 relative">
                     <Target size={40} className="text-emerald-800" strokeWidth={1.5} />
                     <div className="space-y-1">
                        <p className="text-xs font-bold text-[#1a7a4a] uppercase tracking-widest">Preview Dinonaktifkan</p>
                        <p className="text-xs font-bold text-emerald-800 opacity-80 max-w-[200px]">Asset akan dimuat dan disusun secara runtime saat file PDF di-generate oleh backend.</p>
                     </div>
                   </div>
                </div>
              </div>
