...
export default function DosenIndex({ users, filters, title }: Props) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (search !== (filters.search || '')) {
                router.get('/admin/dpl', { search }, { preserveState: true, replace: true });
            }
        }, 300);
            ...
            <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                    <p className="hidden lg:block text-sm font-bold text-slate-500 bg-white px-4 py-2 rounded-xl border border-slate-200">
                        {users.meta?.total || 0} Dosen
                    </p>
                    <Link href="/admin/users/create?role=dpl">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                            <tr>
                                <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Identitas Dosen</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Username/Email</th>
                                <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Status Akun</th>
