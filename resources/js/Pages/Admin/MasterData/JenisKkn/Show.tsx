...
export default function JenisKknShow({ jenisKkn, stats, registrations, filters }: Props) {
 ...
 className="h-12 w-12 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-gray-500 hover:text-[#1a7a4a] hover:border-gray-300 transition-all shadow-sm"
 >
 ...
 <p className="text-sm font-bold text-gray-700/40 text-xs font-semibold mt-4">KODE PROTOKOL: {jenisKkn.code}</p>
 </div>
 <Link 
 ...
 <span className="text-sm font-bold text-gray-700/40 text-xs font-semibold mt-1">Kriteria Minimum Peserta</span>
 </div>
 </div>
 
 <div className="flex flex-wrap items-center gap-x-16 gap-y-6">
 <div className="flex flex-col">
 <span className="text-sm font-bold text-gray-700/20 font-semibold text-xs leading-none mb-3">Ambang Batas Kredit</span>
 <span className="text-lg font-bold text-black tabular-nums">{jenisKkn.min_sks} UNIT SKS</span>
 </div>
 <div className="flex flex-col">
 <span className="text-sm font-bold text-gray-700/20 font-semibold text-xs leading-none mb-3">Ambang Batas IPK</span>
 <span className="text-lg font-bold text-black tabular-nums">{jenisKkn.min_gpa} INDEKS</span>
 </div>
 <div className="flex flex-col">
 <span className="text-sm font-bold text-gray-700/20 font-semibold text-xs leading-none mb-3">Aturan Registrasi</span>
 <span className="text-lg font-bold text-black">{jenisKkn.registration_mode_label}</span>
 </div>
 <div className="flex flex-col">
 <span className="text-sm font-bold text-gray-700/20 font-semibold text-xs leading-none mb-3">Nomenklatur Deskripsi</span>
 <span className="text-sm font-bold text-gray-700/40 max-w-sm truncate"title={jenisKkn.description || ''}>{jenisKkn.description || 'NIHIL DESKRIPSI PROTOKOL.'}</span>
 </div>
 </div>
 </div>
 </section>
 ...
 <div className="px-6 py-6 border-b border-gray-200/50 flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-gray-50/50">
 ...
 <span className="text-sm font-bold text-gray-700/40 text-xs font-semibold mt-1">Log transmisi data registrasi terpadu.</span>
 </div>
 </div>
 <form onSubmit={handleSearch} className="relative w-full lg:w-[450px] group">
 <Search size={22} className="absolute left-8 top-1/2 -translated-y-1/2 text-gray-500 group-focus-within:text-[#1a7a4a] transition-colors"strokeWidth={3} />
 <input 
 type="text"
 placeholder="CARI DATA PESERTA..."
 value={search}
 onChange={(e) => setSearch(e.target.value)}
 className="w-full h-18 pl-20 pr-8 bg-white border border-gray-200 rounded-xl text-sm font-bold text-black focus:border-[#1a7a4a] outline-none transition-all placeholder:text-emerald-50/50"
 />
 </form>
 </div>
 ...
 <div className="flex flex-col">
 <span className="text-lg font-bold text-black group-hover:text-gray-700 transition-colors leading-none truncate max-w-[300px]">{reg.mahasiswa.nama}</span>
 <span className="text-sm font-bold text-gray-700/20 mt-3 font-semibold text-xs leading-none">{reg.mahasiswa.nim}</span>
 </div>
 </div>
 </td>
 <td className="px-6 py-6">
 <div className="flex items-center gap-4">
 <GraduationCap size={16} className="text-gray-500"strokeWidth={2.5} />
 <span className="text-sm font-bold text-black">{reg.mahasiswa.fakultas.nama}</span>
 </div>
 </td>
 <td className="px-6 py-6 text-center">
 <span className="inline-flex items-center px-6 py-2 rounded-xl bg-gray-50 text-gray-700 text-sm font-bold tracking-normal border border-gray-200 shadow-sm">
 Siklus #{reg.periode.periode}
 </span>
 </td>
 <td className="px-6 py-6 text-center">
 <div className="flex flex-col items-center gap-2">
 <div className="flex items-center justify-center gap-3 text-sm font-bold text-black tabular-nums">
 <Calendar size={14} className="text-gray-600"strokeWidth={2.5} />
 ...
 <span className="text-sm font-bold text-gray-700/10 text-xs font-semibold leading-none">Belum Terdistribusi</span>
 )}
 </td>
 </tr>
 )) : (
...
function MetricCard({ label, value, icon: Icon, color, isText = false, desc }: { label: string; value: string | number; icon: any; color: 'emerald' | 'rose' | 'amber' | 'slate'; isText?: boolean, desc: string }) {
 const colorMap = {
 emerald: 'bg-gray-50 text-[#1a7a4a] border-gray-200 hover:border-[#1a7a4a]',
 ...
 slate: 'bg-gray-50 text-gray-700/40 border-gray-200 hover:border-[#1a7a4a]'
 };
 return (
 ...
 <div className="text-sm font-bold text-gray-700/30 text-xs font-semibold">{desc}</div>
 </div>
 <div className="mt-8 space-y-4 relative z-10 transition-all group-hover:translate-x-1">
 <p className="text-sm font-bold text-gray-700/40 text-xs font-semibold leading-none">{label}</p>
 <p className={clsx('font-bold text-black tabular-nums leading-none ', isText ? 'text-2xl' : 'text-2xl')}>{value.toLocaleString()}</p>
 </div>
 </div>
 );
}
