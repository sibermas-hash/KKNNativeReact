      ...
      <div className="max-w-7xl mx-auto space-y-8 pb-24 text-slate-900 font-sans">
        ...
        <div className="space-y-4">
            <div className="flex items-center gap-3 text-emerald-600">
                <Layers3 size={18} />
                <span className="text-xs font-bold uppercase tracking-[0.25em] opacity-80">Konten Publik & Operasional</span>
            </div>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    <button 
                        ...
                        className="h-14 px-8 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold shadow-xl shadow-emerald-100 flex items-center gap-3 text-sm transition-all active:scale-95 disabled:opacity-20 uppercase tracking-wider"
                    >
           ...
           <section className="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm">
                <div className="p-3 bg-slate-50/20 border-b border-slate-50 flex items-center justify-between">
                     <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest italic">Interface Global Parameters</span>
                     </div>
                </div>
                <div className="p-6 space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Interface_Title_Node</label>
                        <input value={data.title} onChange={e => setData('title', e.target.value)} className="w-full h-12 bg-slate-50 border border-slate-100 rounded-lg px-6 text-[14px] font-black text-slate-900 focus:bg-white focus:border-emerald-500 transition-all uppercase italic outline-none" required />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Narrative_Intro_Vector</label>
                        <textarea rows={3} value={data.intro} onChange={e => setData('intro', e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-lg px-6 py-4 text-[12px] font-bold text-slate-900 focus:bg-white focus:border-emerald-500 transition-all leading-relaxed italic outline-none" required />
                    </div>
                </div>
           </section>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <AnimatePresence>
                    {(data.schemes || []).map((s, idx) => (
                        <motion.div key={`scheme-${idx}`} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm group">
                            <div className="px-4 py-3 bg-slate-50/50 border-b border-slate-50 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="h-6 w-6 rounded bg-emerald-600 text-white flex items-center justify-center text-[10px] font-black italic shadow-sm">{String(idx+1).padStart(2,'0')}</div>
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Cluster_Node #{idx+1}</span>
                                </div>
                                {data.schemes.length > 1 && (
                                    <button type="button" onClick={() => removeScheme(idx)} className="h-7 w-7 bg-white border border-slate-100 text-rose-300 hover:text-rose-600 rounded flex items-center justify-center transition-all"><Trash2 size={12} /></button>
                                )}
                            </div>
                            <div className="p-4 space-y-4">
                                <div className="space-y-1">
                                    <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-1">Identifier</label>
                                    <input value={s.title} onChange={e => updateScheme(idx, 'title', e.target.value)} className="w-full h-10 bg-slate-50 border border-slate-100 rounded-lg px-4 text-[12px] font-black italic focus:border-emerald-500 outline-none transition-all uppercase" required />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-1">Description</label>
                                    <textarea rows={3} value={s.description} onChange={e => updateScheme(idx, 'description', e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-lg px-4 py-3 text-[11px] font-bold italic focus:border-emerald-500 outline-none transition-all" required />
                                </div>
                                <div className="flex items-center justify-between gap-4 pt-2">
                                     <div className="flex-1 space-y-1">
                                        <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-1">Proxy_Color</label>
                                        <select value={s.color} onChange={e => updateScheme(idx, 'color', e.target.value as SchemeColor)} className="w-full h-9 bg-slate-50 border border-slate-100 rounded-lg px-3 text-[10px] font-black uppercase italic outline-none">
                                            {colorOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                                        </select>
                                     </div>
                                     <div className={clsx('h-16 w-16 rounded-xl border-2 border-dashed flex items-center justify-center transition-all shrink-0', s.color === 'emerald' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : s.color === 'blue' ? 'bg-blue-50 border-blue-100 text-blue-600' : s.color === 'amber' ? 'bg-amber-50 border-amber-100 text-amber-600' : 'bg-slate-50 border-slate-100 text-slate-400')}>
           ...
           <div className="bg-emerald-600 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xl shadow-emerald-100">
                <div className="flex items-center gap-4 ml-2">
                    ...
                    <div className="space-y-0.5">
                        <h4 className="text-[12px] font-black uppercase text-white italic leading-none">Scheme Commitment Protocol</h4>
                        <span className="text-[8px] font-bold text-emerald-100 uppercase tracking-[0.2em] italic">Deploying {data.schemes.length} ACTIVE_DOMAINS to public interface</span>
                    </div>
                </div>
