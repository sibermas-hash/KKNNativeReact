...
interface LaporanAkhir {
  ...
  artifacts?: Array<{ key: string; label: string; available: boolean; download_url: string | null }>;
  submitted_at?: string;
...
export default function AdminLaporanAkhirDetailPage(): React.JSX.Element {
  ...
  const updateMutation = useMutation({
    ...
    onError: () => toast.error('Gagal memperbarui status laporan'),
  });
  ...
  const artifacts = data.artifacts ?? [
    { key: 'file', label: 'Laporan Utama', available: Boolean(data.file_path), download_url: data.file_path ? `${apiUrl(`/admin/laporan/akhir/${id}/unduh`)}?asset=${encodeURIComponent(data.file_path)}` : null },
    { key: 'article_1', label: 'Artikel Ilmiah 1', available: Boolean(data.article_1_path), download_url: data.article_1_path ? `${apiUrl(`/admin/laporan/akhir/${id}/unduh`)}?asset=${encodeURIComponent(data.article_1_path)}` : null },
    { key: 'article_2', label: 'Artikel Ilmiah 2', available: Boolean(data.article_2_path), download_url: data.article_2_path ? `${apiUrl(`/admin/laporan/akhir/${id}/unduh`)}?asset=${encodeURIComponent(data.article_2_path)}` : null },
    { key: 'poster_1', label: 'Poster Peta 1', available: Boolean(data.poster_1_path), download_url: data.poster_1_path ? `${apiUrl(`/admin/laporan/akhir/${id}/unduh`)}?asset=${encodeURIComponent(data.poster_1_path)}` : null },
    { key: 'poster_2', label: 'Poster Peta 2', available: Boolean(data.poster_2_path), download_url: data.poster_2_path ? `${apiUrl(`/admin/laporan/akhir/${id}/unduh`)}?asset=${encodeURIComponent(data.poster_2_path)}` : null },
    { key: 'poster_3', label: 'Poster Peta 3', available: Boolean(data.poster_3_path), download_url: data.poster_3_path ? `${apiUrl(`/admin/laporan/akhir/${id}/unduh`)}?asset=${encodeURIComponent(data.poster_3_path)}` : null },
  ];
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {artifacts.map((item, i) => (
                <div key={i} className={`p-4 rounded-xl border-2 flex flex-col gap-3 ${item.available ? 'border-slate-100 bg-white hover:border-emerald-300' : 'border-dashed border-slate-100 bg-slate-50 opacity-50'}`}>
                  <div className="flex items-center gap-2">
                    {item.key.startsWith('poster') ? <ImageIcon size={14} className={item.available ? 'text-emerald-600' : 'text-slate-400'} /> : <FileText size={14} className={item.available ? 'text-emerald-600' : 'text-slate-400'} />}
