...
export default function GeotaggingMap({ lat, lng, label = 'Lokasi Laporan', zoom = 15, height = '300px' }: Props) {
    ...
    if (!isMounted) return <div style={{ height, backgroundColor: '#f8fafc' }} className="rounded-2xl animate-pulse" />;

    const position: [number, number] = [lat, lng];

    return (
        <div className="relative w-full overflow-hidden rounded-[2rem] border border-emerald-100 shadow-xl group/map" style={{ height }}>
            <MapContainer 
                center={position} 
                zoom={zoom} 
