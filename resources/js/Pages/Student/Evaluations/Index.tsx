...
export default function StudentEvaluationsIndex({ evaluations }: Props) {
 ...
 return (
 <section key={evaluation.id} className="rounded-lg border border-gray-200/60 bg-white p-6">
 ...
 <p className="mt-1 text-sm font-medium text-gray-900">Grade {evaluation.grade || '-'}</p>
 </div>
 </div>
 ...
 <div
 key={item.id ?? index}
 className="grid grid-cols-[1fr_auto_auto] gap-3 rounded-lg border border-gray-200/60 px-4 py-3"
 >
 <span className="text-sm font-medium text-black">{item.criterion || '-'}</span>
 <span className="text-sm text-gray-900">{item.weight ?? 0}%</span>
 <span className="text-sm font-semibold text-black">{item.score ?? 0}</span>
 </div>
 ))
 ) : (
 <p className="text-sm text-gray-900">Belum ada rincian item evaluasi.</p>
 )}
 </div>

 {evaluation.notes && (
 <div className="mt-6 rounded-lg border border-gray-200/60 bg-emerald-50/30 p-4">
 <h3 className="text-sm font-semibold text-black">Catatan penilai</h3>
 <p className="mt-2 text-sm text-gray-900">{evaluation.notes}</p>
 </div>
 )}
 </section>
 );
 ...
 <section className="rounded-lg border border-gray-200/60 bg-white px-6 py-6 text-center text-sm text-gray-900">
