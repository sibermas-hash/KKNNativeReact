        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <ContentPanel
              ...
              description={editingItem ? `Memperbarui konfigurasi node #${editingItem.id}` : "Tentukan parameter logika validasi pendaftaran."}
              icon={editingItem ? Pencil : Plus}
              padding={true}
              headerAction={editingItem && (
            ...
            >
              <form onSubmit={submit} className="space-y-6">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Nama Protokol</label>
                  <input
                    type="text"
                    value={data.name}
                    ...
                    required
                  />
                  {errors.name && <p className="text-[10px] font-bold text-rose-600 mt-1 uppercase tracking-tight">{errors.name}</p>}
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Sumber Data</label>
                    <div className="relative group">
                      ...
                      >
                        ...
                        {availableColumns.map(c => <option key={c.value} value={c.value}>{c.label.toUpperCase()}</option>)}
                      </select>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Operator</label>
                    <div className="relative group">
                      <select
                        value={data.operator}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Nilai Ekspektasi</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-emerald-950 uppercase tracking-widest pl-1">Pesan Penolakan (Output)</label>
                  <textarea
                ...
                <div className="flex items-center gap-3 px-4 py-3 bg-emerald-50 rounded-xl border border-emerald-100/50">
                  ...
                  <div className="flex flex-col">
                    <label htmlFor="is_active" className="text-[10px] font-black text-emerald-950 uppercase tracking-widest cursor-pointer leading-none mb-1">Aktivasi Node</label>
                    <p className="text-[9px] font-bold text-emerald-700/60 uppercase tracking-tight">Aturan akan langsung dievaluasi pendaftar.</p>
                  </div>
                </div>
          ...
          <div className="lg:col-span-8">
            ...
            >
              <PremiumTable
                headers={['Protocol Identity', 'Logic Condition', 'Return Message', 'Status', 'Opsi']}
                isEmpty={requirements.length === 0}
                emptyText="Belum ada aturan validasi yang terdaftar."
              >
                {requirements.map((r) => (
                  <PremiumTableRow key={r.id} className={clsx(editingItem?.id === r.id && "bg-emerald-50/50")}>
                    <PremiumTableCell>
                      <div className="flex flex-col py-1">
                        <span className="text-[13px] font-black text-emerald-950 uppercase tracking-tight leading-none group-hover:text-emerald-700 transition-colors">{r.name}</span>
                        <span className="text-[9px] font-bold text-slate-400 mt-2 uppercase tracking-widest">NODE_ID_AFFIRMED #{r.id}</span>
                      </div>
                    </PremiumTableCell>
                    <PremiumTableCell>
                      <div className="inline-flex flex-col gap-1 px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl">
                        <span className="text-[9px] font-black text-emerald-800 font-mono tracking-widest uppercase">{r.column_name}</span>
                        <div className="flex items-center gap-1.5">
                           <span className="text-emerald-600 font-black text-xs">{r.operator}</span>
                           <span className="text-[11px] font-black text-emerald-950">"{r.expected_value}"</span>
                        </div>
                      </div>
                    </PremiumTableCell>
                    <PremiumTableCell>
                      <p className="text-[11px] font-bold text-slate-600 leading-tight line-clamp-2 max-w-[200px] italic">"{r.error_message}"</p>
                    </PremiumTableCell>
                    <PremiumTableCell align="center">
                       <button onClick={() => toggleStatus(r.id)} className="transition-all active:scale-95">
                          <StatusTag status={r.is_active ? 'active' : 'draft'} label={r.is_active ? 'ENABLED' : 'DISABLED'} size="sm" />
                       </button>
                    </PremiumTableCell>
                    <PremiumTableCell align="right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => startEdit(r)} className="h-8 w-8 flex items-center justify-center text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all" title="Edit">
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => deleteItem(r.id)} className="h-8 w-8 flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Hapus">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </PremiumTableCell>
