...
export default function MetricCard({ 
  ...
  const colorClasses = {
    ...
    amber: "border-amber-100 bg-amber-50/20 hover:bg-amber-50 text-amber-700 icon-bg-amber-100 icon-text-amber-600",
    ...
    >
      ...
      >
        <div className="font-sans">
          <p className="text-[11px] font-semibold text-slate-500 mb-2 uppercase tracking-wider">{title}</p>
          <AnimatedCounter 
            value={numericValue} 
            className={clsx(
        ...
        <div className={clsx(
          ...
          selectedColor.split(' ').find(c => c.startsWith('icon-bg-'))?.replace('icon-bg-', 'bg-'),
          selectedColor.split(' ').find(c => c.startsWith('icon-text-'))?.replace('icon-text-', 'text-')
        )}>
