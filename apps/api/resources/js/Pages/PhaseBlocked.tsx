...
const phaseInfo: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  upcoming: {
    ...
    icon: Clock,
    color: 'text-[#0d9488] bg-emerald-50 border-emerald-50',
  },
  registration: {
  ...
  finished: {
    label: 'KKN Selesai',
    icon: Lock,
    color: 'text-emerald-400 bg-emerald-50 border-emerald-50',
  },
  inactive: {
    label: 'Tidak Aktif',
    icon: Lock,
      ...
      <div className="min-h-[60vh] flex items-center justify-center px-4 bg-white">
        <div className="max-w-md w-full text-center space-y-8 p-8 rounded-3xl border border-gray-100 shadow-xl shadow-emerald-50/20 bg-white">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-emerald-800/70 leading-relaxed font-medium">{message}</p>
          </div>

          {/* Current Phase Badge */}
          <div
            className={`inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl border font-bold tracking-tight shadow-sm ${info.color}`}
          >
            <PhaseIcon size={18} />
