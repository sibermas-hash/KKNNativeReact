...
function normalizeStatus(status?: string): string | undefined {
  ...
  if (['approved', 'disetujui', 'verifikasi_pusat', 'interview_passed'].includes(s)) return 'approved';
  ...
  if (s === 'interview_scheduled') return 'interview_scheduled';
  if (s === 'interview_failed') return 'interview_failed';
  return status;
}
...
export default function StudentDashboard(): React.JSX.Element {
  ...
  const isInterviewScheduled = normalizedStatus === 'interview_scheduled';
  const isInterviewFailed = normalizedStatus === 'interview_failed';
  ...
  const phases = [
    ...
    { id: 4, label: 'Pelaksanaan', done: dailyReportCount >= minLogbook, active: workProgramCount > 0 && dailyReportCount < minLogbook },
    { id: 5, label: 'Pelaporan', done: !!finalReport, active: dailyReportCount >= minLogbook && !finalReport },
                ...
                <div className="relative z-10 space-y-6">
                  <div className="space-y-2">
                    <h3 className="text-xl font-black uppercase tracking-tight">
                      {isRejected ? 'Perbaikan Berkas Diperlukan' : isPending ? 'Audit Pendaftaran Berjalan' : isInterviewScheduled ? 'Menunggu Wawancara' : 'Belum Terdaftar?'}
                    </h3>
                    <p className="text-sm font-medium text-slate-400 max-w-xl leading-relaxed">
                      {isRejected
                        ...
                        : isPending
                          ...
                          ? 'Pendaftaran Anda telah disetujui. Anda wajib mengikuti wawancara sebelum resmi menjadi peserta KKN. Silakan cek jadwal di menu Wawancara.'
