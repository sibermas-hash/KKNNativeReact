...
export default function StudentTransfer({ students, targetPeriods, filters }: Props) {
  ...
  const handleTransfer = () => {
    ...
    if (
      confirm(
        `Mutasi Peserta: ${selectedStudent.mahasiswa.nama}? Pastikan data tujuan sudah benar.`,
      )
    ) {
      router.post(
        '/admin/peserta/pindah',
        {
  ...
  return (
    <AppLayout title="Transfer Peserta">
      <Head title="Manajemen Mutasi Peserta | SIBERMAS" />
        ...
        <div className="bg-white border-2 border-gray-100 rounded-[2rem] p-8 relative overflow-hidden flex flex-col md:flex-row items-start justify-between gap-8 group">
          ...
          <div className="relative z-10 flex gap-6 items-start">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm font-bold text-emerald-800 max-w-xl leading-relaxed">
                Protokol administrasi instan untuk menggeser penempatan personil KKN lintas kelompok
                atau siklus periode secara aman dan akurat.
              </p>
            </div>
          </div>
          ...
          <div className="relative z-10 hidden md:flex items-center gap-6 bg-emerald-50 px-8 py-6 rounded-[2rem] shrink-0 border border-emerald-100">
            <div className="flex flex-col text-right">
              <span className="text-xs font-black text-emerald-700 uppercase tracking-widest mb-1">
                Total Peserta Valid
              </span>
              <span className="text-4xl font-black text-emerald-950 leading-none tabular-nums">
                {students.meta.total.toLocaleString('id-ID')}
              </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-gray-100 custom-scrollbar p-6">
                ...
                >
                  <span
                    className={clsx(
                      ...
                      selectedStudent?.id === s.id ? 'text-emerald-200' : 'text-emerald-600',
                    )}
                  >
                    {s.mahasiswa.nim}
                  </span>
              ...
              {students.data.length === 0 && (
                <div className="py-24 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="text-xs font-black text-emerald-950 uppercase tracking-widest mb-1">
                    Data Kosong
                  </span>
            ...
            {selectedStudent ? (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-xs font-black text-emerald-500 uppercase tracking-widest">
                        Sistem Mutasi Terpadu
                      </h2>
                      <h3 className="text-xl font-black text-white uppercase tracking-tight">
                        Konfigurasi Tujuan Penempatan
                      </h3>
                    </div>
                  </div>
                  <button
                ...
                <div className="flex-1 p-10 space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-8 border border-gray-200 rounded-xl flex flex-col gap-3 relative overflow-hidden group">
                      <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity text-emerald-950">
                        <UserCircle size={100} />
                      </div>
                      <span className="text-xs font-black text-emerald-600 uppercase tracking-widest relative z-10">
                        Identitas Terpilih
                      </span>
                      <div className="relative z-10">
                        <strong className="text-2xl font-black text-emerald-950 uppercase shrink-0 leading-none block">
                          {selectedStudent.mahasiswa.nama}
                        </strong>
                        <span className="text-xs font-bold text-emerald-600 block mt-2 uppercase tracking-widest tabular-nums">
                    ...
                    <div className="p-8 bg-emerald-50/50 border border-emerald-100 rounded-xl flex flex-col gap-3 relative overflow-hidden group">
                      ...
                      <span className="text-xs font-black text-emerald-700 uppercase tracking-widest relative z-10">
                        Penempatan Saat Ini
                      </span>
                      <div className="relative z-10">
                        <strong className="text-2xl font-black text-emerald-900 leading-none block uppercase">
                          {selectedStudent.kelompok?.nama_kelompok ||
                            selectedStudent.kelompok?.code ||
                            'BELUM DITEMPATKAN'}
                        </strong>
                  ...
                  <div className="space-y-8 bg-white rounded-xl border border-gray-200 p-8 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          <option value="">
                            {isLoadingGroups ? 'LOADING...' : '-- SELEKSI UNIT (OPSIONAL) --'}
                          </option>
                          {groups
                ...
                <div className="px-10 py-6 bg-gray-50 border-t border-emerald-100 flex flex-col sm:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs font-bold text-emerald-700 max-w-[250px] leading-relaxed uppercase tracking-wider">
                      Operasi ini bersifat langsung dan akan tercatat permanen di dalam log audit
                      kelayakan.
                    </span>
                  </div>
                  <Button
              ...
              <div className="flex-1 flex flex-col bg-gray-50/50 border-2 border-dashed border-emerald-100 rounded-xl items-center justify-center p-20 text-center h-full group transition-all hover:bg-gray-50">
                <div className="h-32 w-32 bg-white rounded-[2rem] shadow-sm border border-emerald-100 flex items-center justify-center mb-10 group-hover:scale-110 group-hover:rotate-12 transition-transform duration-700">
                  <ArrowRightLeft
                    size={48}
                    className="text-emerald-700 group-hover:text-emerald-600 transition-colors"
                    strokeWidth={1.5}
                  />
                </div>
                <h3 className="text-2xl font-black text-emerald-950 mb-4 uppercase tracking-tight">
                  Kondisi Siaga
                </h3>
                <p className="text-xs font-bold text-emerald-800 max-w-sm leading-relaxed uppercase tracking-widest">
                  Silakan seleksi identitas peserta dari kolom sebelah kiri untuk mengaktifkan
                  antarmuka protokol perpindahan dan mutasi.
                </p>
              </div>
            )}
          </div>
