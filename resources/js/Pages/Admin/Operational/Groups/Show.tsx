...
export default function GroupShow({ group, members = [] }: Props) {
 ...
 <Link
 href="/admin/kelompok"
 className="h-14 w-14 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-600 hover:text-[#1a7a4a] hover:border-gray-300 transition-all shadow-sm group/back active:scale-90"
 >
 ...
 <p className="text-2xl font-semibold text-gray-900/40 leading-tight max-w-2xl">
 {group.nama_kelompok}
 </p>
 <div className="flex items-center gap-6 pt-4">
 <div className="flex items-center gap-3 bg-white border border-gray-200 px-5 py-2.5 rounded-xl shadow-sm">
 <Lock size={16} className="text-gray-700"/>
 <span className="text-xs font-semibold text-gray-900 font-mono">TOKEN: {group.token || 'MISSING'}</span>
 </div>
 <div className="flex items-center gap-3 bg-white border border-gray-200 px-5 py-2.5 rounded-xl shadow-sm">
 <Briefcase size={16} className="text-gray-700"/>
 <span className="text-xs font-semibold text-gray-900 leading-none">{group.periode?.name || 'UMUM'}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 {/* PERSONNEL LEDGER */}
 <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm flex flex-col">
 ...
 <Users size={32} className="text-gray-700"strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <h3 className="text-xl font-semibold leading-none mb-1.5">Manifest Personel</h3>
 <p className="text-xs font-bold text-gray-700">Daftar Mahasiswa Terdaftar Dalam Unit</p>
 </div>
 </div>
 <div className="flex flex-col items-end">
 <span className="text-xs font-semibold text-gray-700 mb-1">Total Entitas</span>
 ...
 <span className="text-sm font-semibold text-gray-900 leading-none group-hover:text-gray-700 transition-colors">{m.mahasiswa?.nama || 'PESERTA'}</span>
 <span className="text-xs font-semibold text-[#1a7a4a] font-mono">NIM: {m.mahasiswa?.nim || 'UNKNOWN'}</span>
 </div>
 </td>
 <td className="px-8 py-8">
 <div className="flex flex-col gap-2">
 <span className="text-xs font-bold text-gray-700 leading-tight">{m.mahasiswa?.prodi?.nama || '-'}</span>
 <span className="text-xs font-semibold text-gray-600">{m.mahasiswa?.fakultas?.nama || '-'}</span>
 </div>
 </td>
...
"h-10 px-6 rounded-xl flex items-center justify-center gap-3 text-xs font-semibold border shadow-sm inline-flex mx-auto",
 m.role === 'Ketua' ? 'bg-[#16a34a] border-[#1a7a4a] text-white' : 'bg-white border-gray-200 text-gray-700'
 )}>
 ...
 className="h-10 px-5 bg-white border border-gray-200 text-gray-900 hover:bg-white hover:text-white rounded-xl text-xs font-semibold transition-all active:scale-95"
 >
 AUDIT
 </Link>
 ...
 method="post"
 className="h-10 w-10 bg-white border border-gray-200 text-gray-700 hover:bg-[#16a34a] hover:text-white rounded-xl flex items-center justify-center transition-all active:scale-90"
 ...
 <div className="flex items-center gap-6">
 <div className="h-16 w-16 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-[#1a7a4a] shadow-sm">
 <ClipboardList size={32} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <h3 className="text-xl font-semibold leading-none mb-1.5">Manifest Program Kerja</h3>
 <p className="text-xs font-bold text-gray-700 leading-none">Rencana Operasional & Pengabdian</p>
 </div>
 </div>
 <div className="h-14 w-24 bg-white text-gray-700 rounded-xl flex flex-col items-center justify-center shadow-sm border border-emerald-800">
 ...
 workPrograms.map((p) => (
 <div key={p.id} className="group/pro p-8 bg-gray-50 border border-gray-200 rounded-xl hover:bg-white hover:border-[#1a7a4a] transition-all duration-500 flex items-center justify-between gap-8 shadow-sm">
 <div className="flex items-center gap-6">
 <div className="h-16 w-16 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-600 group-hover/pro:bg-[#16a34a] group-hover/pro:text-white transition-all duration-500 scale-90 group-hover/pro:scale-100">
 <Zap size={28} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col gap-3">
 <span className="text-lg font-semibold text-gray-900 group-hover/pro:text-gray-700 transition-colors leading-tight truncate max-w-[400px]">{p.title || 'Draft Proker'}</span>
 <span className="text-xs font-semibold text-gray-700 font-mono">ID: #{p.id.toString().padStart(6, '0')}</span>
 </div>
 </div>
 ...
 <div className="flex items-center gap-6">
 <div className="h-16 w-16 bg-gray-100 border border-emerald-800 rounded-xl flex items-center justify-center shadow-inner">
 <UserCheck size={32} className="text-gray-700"strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <h3 className="text-xl font-semibold leading-none mb-1.5">Otoritas DPL</h3>
 <p className="text-xs font-bold text-gray-600 leading-none">Dosen Pembimbing Lapangan</p>
 </div>
 </div>
 ...
 lecturerRows.map((l) => (
 <div key={l.id} className="p-8 bg-emerald-700/50 border border-[#1a7a4a] rounded-xl hover:bg-white hover:text-gray-900 transition-all duration-500 group/card shadow-sm">
 <span className="text-xs font-semibold text-gray-700 block mb-1">Ketua Pembimbing</span>
 ...
 {/* LOKASI POSKO */}
 <section className="bg-white border border-gray-200 rounded-xl p-10 space-y-10 shadow-sm relative overflow-hidden group/loc">
 <div className="flex items-center gap-6">
 <div className="h-16 w-16 bg-gray-50 text-rose-500 border border-gray-200 rounded-xl flex items-center justify-center shadow-sm group-hover/loc:bg-[#16a34a] group-hover/loc:text-white group-hover/loc:rotate-12 transition-all duration-500">
 <MapPinned size={32} strokeWidth={2.5} />
 </div>
 <div className="flex flex-col">
 <h3 className="text-xl font-semibold leading-none mb-1.5">Pusat Navigasi</h3>
 <p className="text-xs font-bold text-gray-700 leading-none">Geo-Intelligence Posko</p>
 </div>
 </div>

 <div className="space-y-8">
 <div className="p-8 bg-gray-50 border border-gray-200 rounded-xl space-y-4">
 <span className="text-xs font-semibold text-gray-700 leading-none">ZONA PENEMPATAN</span>
 <div className="flex flex-col gap-2">
 <span className="text-xl font-semibold text-gray-900 leading-tight er">{group.lokasi?.full_name || group.lokasi?.village_name || 'BELUM TERPETAKAN'}</span>
 {group.lokasi?.address && (
 <p className="text-xs font-bold text-gray-700 leading-relaxed opacity-60">{group.lokasi.address}</p>
 )}
 </div>
 </div>
 ...
 <div className="relative h-64 rounded-xl overflow-hidden border border-gray-200 shadow-sm group/img">
 ...
 <div>
 <span className="text-xs font-semibold text-gray-700 block mb-2">LINTANG</span>
 <span className="text-sm font-semibold font-mono">{group.posko.latitude || '0.000'}</span>
 </div>
 <div>
 <span className="text-xs font-semibold text-gray-700 block mb-2">BUJUR</span>
 ...
 <div className="py-20 border border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center gap-4 opacity-30">
 <MapPin size={40} className="text-gray-500"strokeWidth={1} />
 ...
 <span className="text-xs font-semibold text-gray-700 opacity-80">Protokol Akuntabilitas Lapangan</span>
 </div>
 </div>
 ...
 <span className="text-xs font-semibold text-gray-700 mb-1 opacity-60">KEAMANAN DATA</span>
...
function MetricCard({ label, value, icon: Icon, type, desc }: { label: string; value: any; icon: any; type?: 'success' | 'warning'; desc: string }) {
 ...
 type === 'warning' ? 'bg-amber-50 text-amber-500 border-amber-100' : 'bg-gray-50 text-[#1a7a4a] border-gray-200'
 )}>
...
function EmptyTable({ icon: Icon, label, desc }: { icon: any; label: string; desc: string }) {
 ...
 <p className="text-xs font-semibold text-gray-700 leading-none opacity-60">{desc}</p>
 </div>
 </td>
 </tr>
 );
...
function EmptyBox({ icon: Icon, label, desc }: { icon: any; label: string; desc: string }) {
 return (
 <div className="py-24 border border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center gap-4 group/none hover:border-gray-300 transition-all">
 <Icon size={64} className="text-emerald-50 group-hover/none:text-gray-700 transition-colors"strokeWidth={1} />
 <div className="text-center space-y-2">
 <span className="text-sm font-semibold text-gray-900 block">{label}</span>
 <p className="text-xs font-semibold text-gray-700 opacity-60">{desc}</p>
 </div>
 </div>
 );
}
