      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:px-6 lg:px-8 font-sans pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 border-b border-gray-200 pt-6">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight leading-tight">Direktori Mahasiswa</h1>
            <p className="text-sm text-gray-500 max-w-2xl mt-1">Pengelolaan data kepesertaan dan sinkronisasi registrasi mahasiswa KKN.</p>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <div className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg shadow-sm text-xs text-gray-500">
              Sinkron: <strong className="text-gray-700">{formatDateTime(syncInfo.last_synced_at)}</strong>
            </div>
            <Link href="/admin/mahasiswa/sinkron" className="h-10 px-4 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-medium shadow-sm hover:bg-gray-50 transition-colors flex items-center gap-2">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-gray-200 bg-gray-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={clsx("h-10 px-4 rounded-lg text-sm font-medium flex items-center gap-2 border shadow-sm transition-colors", showFilters ? "bg-emerald-600 text-white border-emerald-600" : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50")}
              >
                <Filter size={15} />
                {activeFilterCount > 0 ? `Filter (${activeFilterCount})` : 'Filter'}
              </button>
              <button onClick={submitFilters} className="h-10 px-4 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 transition-colors shadow-sm">
                Terapkan
              </button>
            </div>
          </div>
          ...
          {/* FILTER PANEL */}
          {showFilters && (
            <div className="px-5 py-4 border-b border-gray-200 bg-gray-50 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="sm:col-span-2 lg:col-span-4 flex justify-end pt-2 border-t border-gray-200">
                <button onClick={resetFilters} className="text-sm text-gray-500 hover:text-rose-600 transition-colors font-medium">Reset Semua Filter</button>
              </div>
            </div>
          )}
          ...
          <div className="overflow-x-auto min-h-[300px]">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Data Mahasiswa</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Afiliasi Studi</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Performa Akademik</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider">Status Akun</th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">Aksi</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {!students?.data ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-gray-500">Memuat data...</td></tr>
                ) : students.data.length === 0 ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-gray-500">Data mahasiswa tidak ditemukan.</td></tr>
                ) : students.data.map(s => (
                  <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold text-gray-900">{s.nama}</span>
                        <span className="text-xs text-gray-500 mt-0.5">NIM: {s.nim}{s.account && ` · @${s.account.username}`}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm text-gray-800 truncate max-w-xs">{s.prodi?.nama || '—'}</span>
                        <span className="text-xs text-gray-500 mt-0.5">{s.fakultas?.nama || '—'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <span className={clsx("text-xs font-medium px-2 py-0.5 rounded w-fit", s.is_bta_ppi_passed ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800")}>
                    ...
                    <td className="px-6 py-4 text-center">
                      {!s.account ? (
                        <span className="text-xs text-gray-400">Tanpa Akun</span>
                      ) : (
                        <span className={clsx("inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold", s.account.is_active ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800")}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => s.account && setConfirmReset(s)} disabled={!s.account} className="p-1.5 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-md transition-colors disabled:opacity-30" title="Reset Password">
                          <KeyRound size={16} />
                        </button>
                        <button onClick={() => s.account && setConfirmToggle(s)} disabled={!s.account} className="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-md transition-colors disabled:opacity-30" title={s.account?.is_active ? 'Nonaktifkan' : 'Aktifkan'}>
                          {s.account?.is_active ? <Lock size={16} /> : <Unlock size={16} />}
                        </button>
                        <Link href={`/admin/mahasiswa/${s.id}`} className="px-3 py-1.5 bg-white text-gray-700 hover:bg-gray-50 border border-gray-300 rounded-md text-xs font-medium transition-colors">
                          Detail
                        </Link>
                      </div>
                    </td>
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
            <span className="text-xs text-gray-500">Hal. {students.meta.current_page} — Total <strong>{students.meta.total}</strong> data</span>
...
function StatCard({ label, value, icon: Icon, color = 'slate' }: { label: string; value: number; icon: LucideIcon; color?: 'emerald' | 'slate' }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
      ...
      <div>
        <p className="text-lg font-bold text-gray-900 leading-tight tabular-nums">{value.toLocaleString('id-ID')}</p>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <select value={value} onChange={e => onChange(e.target.value)} className="w-full h-10 pl-3 pr-8 rounded-lg border border-gray-300 bg-white text-sm text-gray-700 focus:border-emerald-500 focus:ring-emerald-500 appearance-none shadow-sm">
