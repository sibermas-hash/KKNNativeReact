...
class AuthController extends Controller
{
    ...
    public function googleRedirect(Request $request): RedirectResponse|JsonResponse
    {
        ...
        if ($clientId === '' || $redirectUri === '') {
            return $this->error('GOOGLE_NOT_CONFIGURED', 'Google Login belum dikonfigurasi.', 503);
        }
        $state = Str::random(40);
    ...
    public function googleCallback(Request $request): RedirectResponse
    {
        ...
        if ($state === '' || ! Cache::pull('google_oauth_state:'.$state) || $code === '') {
            return redirect()->away($frontend.'/login?google_error=invalid_state');
        }
        try {
            ...
            if ($email === '' || ! (bool) ($profile['email_verified'] ?? false)) {
                return redirect()->away($frontend.'/login?google_error=failed');
            }
            $user = User::whereRaw('LOWER(email) = ?', [$email])->where('is_active', true)->first();
            if (! $user) {
                ActivityLogger::log('login_google', 'failed', null, ['email' => $email, 'reason' => 'not_registered']);
...
Berlaku 5 menit. Jangan bagikan kode ini.", fn ($m) => $m->to($user->email)->subject('OTP Login Google SIBERMAS'));
            ...
            return redirect()->away($frontend.'/login?google_error=server');
        }
    }
    ...
    public function googleOtpVerify(Request $request): JsonResponse
    {
        ...
        if (! is_array($challenge)) {
            return $this->error('OTP_EXPIRED', 'Kode OTP kedaluwarsa.', 422);
        }
        if (($challenge['attempts'] ?? 0) >= 5) {
            Cache::forget($key);

            return $this->error('OTP_LOCKED', 'Terlalu banyak percobaan OTP.', 423);
        }
        if (! Hash::check($data['code'], (string) $challenge['otp_hash'])) {
            $challenge['attempts'] = ((int) ($challenge['attempts'] ?? 0)) + 1;
            $ttl = max(1, (int) (($challenge['expires_at'] ?? now()->timestamp) - now()->timestamp));
            Cache::put($key, $challenge, now()->addSeconds($ttl));

            return $this->error('OTP_INVALID', 'Kode OTP salah.', 422);
        }
        Cache::forget($key);
        $user = User::find((int) $challenge['user_id']);
        if (! $user || ! $user->is_active) {
            return $this->error('USER_INACTIVE', 'Akun tidak aktif.', 403);
        }
