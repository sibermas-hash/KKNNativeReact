...
export default function DplFinalReportsIndex({ reports }: Props) {
 ...
 <p className="text-xs text-slate-500">{report.submitted_at ?? 'Belum tercatat'}</p>
 </td>
 <td className="px-6 py-4">
 <p className="text-sm text-slate-800">{report.mahasiswa?.nama ?? '-'}</p>
 <p className="text-xs text-slate-500">{report.mahasiswa?.nim ?? '-'}</p>
 </td>
 <td className="px-6 py-4 text-sm text-slate-600">{report.kelompok?.nama_kelompok ?? '-'}</td>
 <td className="px-6 py-4">
