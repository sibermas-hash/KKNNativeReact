...
export default function PublicLayout({
    ...
    const getDashboardRoute = () => {
        ...
        const roles = (auth.user.roles as Array<string | { name: string }> | undefined)?.map((role) =>
    ...
    return (
        <div className="min-h-screen bg-white text-emerald-950">
            <nav className={overlayNav ? 'fixed inset-x-0 top-0 z-50' : 'sticky top-0 z-50 border-b border-emerald-100 bg-white/95 backdrop-blur-xl'}>
                ...
                <div className="mx-auto grid max-w-7xl gap-8 px-6 sm:px-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)_minmax(0,0.8fr)] lg:px-8">
                    <div className="space-y-4">
                        ...
                        <p className="max-w-lg text-sm leading-6 text-slate-600">
                            Portal publik KKN UIN SAIZU untuk menyajikan berita, dokumen resmi, dan informasi lokasi pengabdian
                            secara terbuka dan mudah dipantau.
                        </p>
                    </div>

                    <div className="space-y-4">
                        <p className="font-display text-sm font-semibold uppercase tracking-[0.18em] text-emerald-950">Kontak</p>
                    ...
                    <div className="space-y-4">
                        <p className="font-display text-sm font-semibold uppercase tracking-[0.18em] text-emerald-950">Tautan Cepat</p>
                        <div className="grid gap-2.5 text-sm text-slate-600">
                            {[...navItems, dashboardItem].map((item) => (
                                <Link key={item.label} href={item.href} className="no-underline transition-colors hover:text-emerald-700">
                                    {item.label}
                                </Link>
                            ))}
                        </div>
