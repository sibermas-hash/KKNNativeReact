...
export default function DailyReportIndex({ reports, groups, filters }: Props) {
  ...
  const handleFilterChange = (status: string, groupId: string) => {
    router.get(
      route('dosen.daily-reports.index'),
      { status, kelompok_id: groupId },
      {
        preserveState: true,
        replace: true,
      },
  ...
  return (
    <AppLayout title="Logbook Mahasisiswa">
      ...
      <div className="max-w-[1600px] mx-auto space-y-8 pb-20 font-sans">
        {/* Header Section */}
        <div className="bg-white rounded-[2.5rem] border border-emerald-50/60 shadow-sm p-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-50 rounded-full -mr-32 -mt-32 opacity-20 blur-3xl" />
          <div className="relative space-y-3">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100/50 text-emerald-800 text-sm font-bold font-semibold uppercase text-xs ring-1 ring-emerald-200">
              Monitoring Progres
            </span>
            <h1 className="text-2xl font-[900] text-emerald-950 tracking-tight">
              Logbook Mahasiswa
            </h1>
            <p className="text-emerald-950 font-medium max-w-xl">
              Pantau dan verifikasi aktivitas harian mahasiswa bimbingan Bapak secara real-time.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 px-2">
            <button
              ...
              className={clsx(
                ...
                groupIdFilter === ''
                  ...
                  : 'bg-white border-emerald-50/60 text-emerald-950 hover:border-emerald-200 hover:text-emerald-600',
              )}
            >
            ...
            {groups.map((group) => (
              <button
                key={group.id}
                onClick={() => {
                  setGroupIdFilter(group.id.toString());
                  handleFilterChange(statusFilter, group.id.toString());
                }}
                className={clsx(
                  ...
                  groupIdFilter === group.id.toString()
                    ...
                    : 'bg-white border-emerald-50/60 text-emerald-950 hover:border-emerald-200 hover:text-emerald-600 shadow-sm',
                )}
              >
                <div
                  className={clsx(
        ...
        <div className="bg-white rounded-xl border border-emerald-50/60 shadow-xl overflow-hidden min-h-[600px] flex flex-col">
          <div className="px-6 py-8 border-b border-emerald-50/60 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-emerald-50/30/30">
            <div className="flex flex-wrap items-center gap-2">
              {['', 'submitted', 'approved', 'revision'].map((s) => (
                <button
                  ...
                  onClick={() => {
                    setStatusFilter(s);
                    handleFilterChange(s, groupIdFilter);
                  }}
                  className={clsx(
                    'h-10 px-6 rounded-xl text-xs font-bold transition-all',
                    statusFilter === s
                      ? 'bg-emerald-900 text-white shadow-lg'
                      : 'bg-white text-emerald-950 hover:text-emerald-950 hover:bg-white/80 border border-emerald-50/60',
                  )}
                >
            ...
            <div className="flex items-center gap-4 bg-white p-1 rounded-2xl border border-emerald-50/60 shadow-inner w-full md:w-auto">
          ...
          <div className="flex-1 overflow-x-auto overflow-y-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-emerald-50/30/50 border-b border-emerald-50/60">
                  <th className="px-6 py-5 text-sm font-bold text-emerald-950 font-semibold uppercase text-xs">
                    Waktu & Judul Kegiatan
                  </th>
                  <th className="px-6 py-5 text-sm font-bold text-emerald-950 font-semibold uppercase text-xs">
                    Identitas Mahasiswa
                  </th>
                  <th className="px-6 py-5 text-sm font-bold text-emerald-950 font-semibold uppercase text-xs text-center">
                    Wilayah / Desa
                  </th>
                  <th className="px-6 py-5 text-sm font-bold text-emerald-950 font-semibold uppercase text-xs text-center">
                    AI Audit
                  ...
                  <th className="px-6 py-5 text-sm font-bold text-emerald-950 font-semibold uppercase text-xs text-right">
                    Opsi
                  </th>
                </tr>
              </thead>
                    ...
                    <tr key={report.id} className="hover:bg-emerald-50/30/80 transition-all group">
                      <td className="px-6 py-6">
                        <div className="flex items-center gap-4">
                          <div className="h-12 w-12 rounded-2xl bg-white border border-emerald-50/60 shadow-sm flex flex-col items-center justify-center shrink-0">
                          ...
                          <div className="space-y-1">
                            <p className="text-sm font-extrabold text-emerald-950 line-clamp-1">
                              {report.title}
                            </p>
                      ...
                      <td className="px-6 py-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-bold text-emerald-950 font-bold text-center mt-0.5">
                              {report.student.nim}
                            </p>
                          </div>
                        </div>
                        ...
                        {report.ai_analysis ? (
                          <div className="flex flex-col items-center gap-1 group/ai cursor-help relative">
                            <div
                              className={clsx(
                                ...
                                report.ai_analysis.abcd_compliance >= 8
                                  ...
                                  : report.ai_analysis.abcd_compliance >= 5
                                    ? 'bg-amber-50 border-amber-100 text-amber-600'
                                    : 'bg-rose-50 border-rose-100 text-rose-600',
                              )}
                            >
                              <span className="text-sm font-bold">
                                {report.ai_analysis.abcd_compliance}/10
                              </span>
                            ...
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 p-3 bg-emerald-900 text-white text-sm rounded-xl opacity-0 invisible group-hover/ai:opacity-100 group-hover/ai:visible transition-all z-20 shadow-2xl">
                              <p className="font-bold text-emerald-400 mb-1 font-semibold uppercase text-xs">
                                AI Audit Summary
                              </p>
                              ...
                              <div className="flex flex-wrap gap-1">
                                {report.ai_analysis.tags.slice(0, 2).map((tag, idx) => (
                                  <span
                                    key={idx}
                                    className="px-1.5 py-0.5 bg-white/10 rounded-md text-white/50 lowercase"
                                  >
                                    #{tag}
                                  </span>
                                ))}
                        ...
                        >
                          <div
                            className={clsx(
                              ...
                              statusColors[report.status].replace('text-', 'bg-').split(' ')[1],
                            )}
                          />
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-32 text-center">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <p className="text-xl font-bold text-slate-300 font-semibold uppercase text-xs leading-none">
                            Antrian Bersih
                          </p>
          ...
          {reports.total > 15 && (
            <div className="px-6 py-8 border-t border-emerald-50/60 flex items-center justify-between bg-emerald-50/30/10">
              <p className="text-xs font-bold text-emerald-950 font-semibold uppercase text-xs">
                Menampilkan{' '}
                <span className="text-emerald-950 font-bold">{reports.data.length}</span> dari{' '}
                <span className="text-emerald-950 font-bold">{reports.total}</span> Laporan
              </p>
                ...
                {reports.links.map((link, i) => (
                  <Link
                    ...
                    className={clsx(
                      ...
                      link.active
                        ? 'bg-emerald-900 text-white shadow-lg'
                        : 'bg-white border border-emerald-50/60 text-emerald-950 hover:border-emerald-200 hover:text-emerald-600',
                    )}
                  />
                ))}
              </div>
