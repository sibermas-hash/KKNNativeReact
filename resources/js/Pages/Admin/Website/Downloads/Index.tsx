...
export default function DownloadIndex({ downloads }: Props) {
  ...
  const openEditModal = (d: Download) => {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const url = editingDownload
      ? route('admin.unduhan.update', editingDownload.id)
      : route('admin.unduhan.store');
    router.post(
      url,
      { ...data, _method: editingDownload ? 'PATCH' : 'POST', is_active: data.is_active ? 1 : 0 },
      {
        forceFormData: true,
        onSuccess: () => {
    ...
    <AppLayout title="Pusat Unduhan">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 space-y-8 py-10 font-sans">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Vaults Aktif"
            value={downloads.data.filter((d) => d.is_active).length}
            icon={Database}
            variant="success"
          />
          ...
          >
            {downloads.data.map((d) => (
              <PremiumTableRow key={d.id} className="group">
                  ...
                  <div className="flex items-center gap-4 py-1">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-[9px] font-black text-emerald-950/40 font-mono tracking-tighter uppercase">
                        ID_{String(d.id).padStart(4, '0')} &middot;{' '}
                ...
                <PremiumTableCell align="center">
                  <StatusTag
                    status={d.is_active ? 'active' : 'draft'}
                    label={d.is_active ? 'RELEASED' : 'DRAFT_STG'}
                    size="sm"
                  />
                </PremiumTableCell>
                <PremiumTableCell align="right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      onClick={() => {
                        if (confirm('Hapus aset?'))
                          router.delete(route('admin.unduhan.destroy', d.id));
                      }}
            ...
            >
              <div className="px-10 py-6 border-b border-emerald-50 flex items-center justify-between bg-gray-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-emerald-950 uppercase tracking-[0.2em]">
                      {editingDownload ? 'Revisi Aset' : 'Injeksi Binary'}
                    </h3>
                    <p className="text-[10px] font-bold text-emerald-700/50 uppercase tracking-widest">
                      Panel Repositori Digital
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="h-10 w-10 flex items-center justify-center rounded-xl hover:bg-rose-50 text-emerald-900 hover:text-rose-600 transition-all"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={submit} className="p-10 space-y-8">
                    ...
                    <div className="relative h-14 w-full bg-[#F8FAF9] rounded-2xl border-2 border-dashed border-emerald-100 flex flex-col items-center justify-center gap-1 overflow-hidden group/img transition-all hover:border-emerald-400">
                      ...
                      {data.file ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] font-black text-emerald-950 uppercase tracking-tight truncate w-24">
                            {data.file.name}
                          </span>
                        </div>
                      ) : (
                        <>
                          ...
                          />
                          <span className="text-[9px] font-black text-emerald-800/30 uppercase tracking-widest">
                            Local Binary
                          </span>
                        </>
                      )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-14 w-full bg-[#F8FAF9] border-2 border-slate-50 rounded-2xl px-4 flex items-center gap-3">
                      ...
                      <input
                        ...
                        className="w-full bg-transparent border-none p-0 focus:ring-0 text-[10px] font-black text-emerald-950 placeholder:text-gray-200"
                ...
                <div className="flex items-center justify-between bg-emerald-50 px-6 py-4 rounded-2xl border border-emerald-100/50">
                  <div className="flex items-center gap-4">
                    <ShieldCheck
                      size={20}
                      className={clsx(data.is_active ? 'text-emerald-600' : 'text-emerald-200')}
                    />
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-emerald-950 uppercase tracking-widest leading-none mb-1">
                        Status Visibilitas
                      </span>
                      <span className="text-[9px] font-bold text-emerald-700/50 uppercase tracking-tight">
                        Siap dipublikasi ke portal.
                      </span>
                    </div>
                  </div>
                  <input
                ...
                <div className="flex items-center gap-4 pt-4">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="flex-1 h-14 text-[10px] font-black text-emerald-950 uppercase tracking-widest hover:bg-gray-50 rounded-2xl transition-all"
                  >
                    Batalkan
                  </button>
                  <button
