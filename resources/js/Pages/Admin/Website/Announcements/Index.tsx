...
export default function AnnouncementIndex({ announcements }: Props) {
 ...
 <Megaphone size={18} />
 <span className="text-xs font-bold opacity-80">Konten Publik & Komunikasi</span>
 </div>
 ...
 <div className="px-6 py-4 border-t border-slate-50 flex items-center justify-between bg-gray-50/20 shrink-0">
 <span className="text-sm font-bold text-slate-300 font-semibold text-xs">Narrative Stream Buffer v2.5</span>
 <Button form="cms-form"type="submit"disabled={processing} className="h-10 px-8 bg-[#16a34a] hover:bg-[#15803d] text-white rounded-lg text-sm font-bold font-semibold text-xs shadow-sm shadow-none flex items-center gap-3 group active:scale-95">{processing ? <RefreshCw size={14} className="animate-spin"/> : <Save size={14} className="text-white"/>} COMMIT_BROADCAST</Button>
 </div>
 </motion.div>
 </div>
 )}
